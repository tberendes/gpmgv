...
;
; skip_orig      - Binary parameter.  If set, then the full-vertical-resolution PR
...
pro gen_pr_and_geo_match_x_sections, ncfilepr, use_db, skip_orig_in, $
...
IF ( skip_orig NE 1 ) THEN BEGIN
   ...
   dprFileMatch=[filesmeta.FILE_1C21, filesmeta.FILE_2A23, $
                 filesmeta.FILE_2A25, filesmeta.FILE_2B31 ]
  ...
  ; and, for now, set a default instrumentID and scan type
   ...
   GPMDATA_ROOT = '/data/gpmgv/GPMtest'
   dir_2Adpr='/.' & dir_2Aku='/.' & dir_2Aka='/.' & dir_comb='/.'
...
IF ( skip_orig NE 1 ) THEN print, 'and full-vertical-resolution (250 m) PR 2A-25 data,'
...
;                         IF ( skip_orig NE 1 ) THEN WDELETE, 7
                         WDELETE, 5
                         WDELETE, 6
                         tvlct, rr, gg, bb
                      ENDIF
...
pro dpr_and_geo_match_x_sections, ELEV2SHOW=elev2show, SITE=sitefilter, $
                                  ...
                                  SKIP_ORIG=skip_orig, PCT_ABV_THRESH=pctAbvThresh, $
...
print
print, "###############################################"
print, "#  DPR_AND_GEO_MATCH_X_SECTIONS: Version 1.0  #"
print, "#  NASA/GSFC/GPM Ground Validation, Jul. 2013 #"
print, "###############################################"
print
...
IF N_ELEMENTS( skip_orig ) EQ 0 THEN skip_orig = 1 $
ELSE skip_orig = KEYWORD_SET( skip_orig )
   ...
   if nf eq 0 then begin
      ...
      for fnum = 0, nf-1 do begin
        ...
        ;
         ...
         gen_pr_and_geo_match_x_sections, ncfilepr, use_db, skip_orig, $
                        ELEV2SHOW=elev2show, PCT_ABV_THRESH=pctAbvThresh, $

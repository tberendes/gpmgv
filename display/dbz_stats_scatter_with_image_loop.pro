...
; All Rights Reserved.
;
; dbz_stats_scatter_with_image_loop.pro
;
...
pro dbz_stats_scatter_with_image_loop
...
;              plots, [0.14,0.18], [0.955,0.955], COLOR = 100, /NORMAL
	    ...
	    endelse
         ...
         TVLCT, 255,255,255,255 ; set the 1:1 line color to white
	;Build the scatter plot(s) for this level
	 if ( have55match eq 1 ) then begin
	    ...
	    ymax = xmax
	    ymin = 15.0 & xmin = ymin
	    plot, dbzpr2, dbzgv2, PSYM = 1, COLOR=100, TITLE = "Reflectivity Scatter", $
	          XTITLE = "PR (dBZ)", YTITLE = "GV (dBZ)", XRANGE=[xmin,xmax], YRANGE=[ymin,ymax]
	    plots, [xmin, xmax], [ymin, ymax], color=255, LINESTYLE=0
	    fitted = linfit(dbzpr2, dbzgv2)
	    plots, [xmin, xmax], [fitted[0]+fitted[1]*xmin,fitted[0]+fitted[1]*xmax], $
	           color=100, LINESTYLE=1, THICK = 1.5
	 endif
	 if ( haveREOmatch eq 1 ) then begin
            if ( have55match eq 1 ) then begin
	       oplot, dbzpr3, dbzgv3, PSYM = 5, COLOR=150
	       colorfitted=150
	    endif else begin
	       xmax = max(dbzgv3) > max(dbzpr3)
	       ymax = xmax
	       ymin = 15.0 & xmin = ymin
               plot, dbzpr3, dbzgv3, PSYM = 5, COLOR=100, $
	       TITLE = "Reflectivity Scatter", XTITLE = "PR (dBZ)", YTITLE = "GV (dBZ)", $
	    ...
	    plots, [xmin, xmax], [fitted[0]+fitted[1]*xmin,fitted[0]+fitted[1]*xmax], $
	           color=colorfitted, LINESTYLE=2, THICK = 1.5
	 endif

PRO agu_nexrad_bias_maps_pr_dpr2, PR_DPR_diffs_file, PSNAMEADD=psNameAdd
...
; File format is like one of these two patterns:
;
; RADAR_ID|PRbias|numPR|DPRbias|numDPR
; RADAR_ID|PRbias|numPR|DPRbias|numDPR|radarLatitude|radarLongitude
...
max_sym = (MAX(ABS(prediffs)) > MAX(ABS(postdiffs))) > MAX(ABS(diffchg)) + 0.05
...
for isource = src1,src2 do begin
   CASE isource OF
      0 : begin
          ...
          val2 = prediffs
          print, "Plotting PR bias"
          cbarTitle = 'GR-PR mean differences (dBZ)
          end
      1 : begin
          sym2 = sym_string_post
          val2 = postdiffs

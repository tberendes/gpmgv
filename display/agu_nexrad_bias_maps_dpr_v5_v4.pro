PRO agu_nexrad_bias_maps_dpr_V5_v4, DPR_V5V4_diffs_file, PSNAMEADD=psNameAdd

; this IDL program plots the locations and identifiers of the
; nexrad radars in the GPM GV validation network, along with the
; V4 and V5 mean GR-DPR bias, Ku-adjusted if indicated
;
...
outfile = '/data/tmp/map_V4_V5_DPR_bias'+nameAdd+'.ps'
...
nrads = FILE_LINES( DPR_V5V4_diffs_file )
radar_names_array = STRARR(nrads)
V5diffs = FLTARR(nrads)
...
OPENR, unit, DPR_V5V4_diffs_file, /GET_LUN
filestr = ''
READF, unit, filestr
...
; File format is like:
; RADAR_ID|V5bias|numV5|V4bias|numV4|radarLatitude|radarLongitude
...
WHILE ~ EOF(unit) DO BEGIN
   ...
   V5diffs[num] = FLOAT(parsed[1])
   V4diffs[num] = FLOAT(parsed[3])
   diffchg[num] = V4diffs[num]-V5diffs[num]
...
sym_string_pre = STRCOMPRESS(STRING(V5diffs),/REMOVE_ALL)
sym_string_post = STRCOMPRESS(STRING(V4diffs),/REMOVE_ALL)
sym_string_chg = STRCOMPRESS(STRING(diffchg),/REMOVE_ALL)

max_sym = (MAX(ABS(V5diffs)) > MAX(ABS(V4diffs))) > MAX(ABS(diffchg)) + 0.05
...
for isource = src1,src2 do begin
   CASE isource OF
      0 : begin
          sym2 = sym_string_pre
          val2 = V5diffs
          print, "Plotting V5 bias"
          cbarTitle = 'GR-DPR V5 mean differences (dBZ)
          end
      1 : begin
          sym2 = sym_string_post
          val2 = V4diffs
          print, "Plotting V4 bias"
          cbarTitle = 'GR-DPR V4 mean differences (dBZ)'
          end
      2 : begin
          sym2 = sym_string_chg
          val2 = diffchg
          print, "Plotting V5-V4 bias change"
          cbarTitle = 'GR-DPR bias change (dBZ) from V5 to V4'
          end
      3 : begin
          sym2 = sym_string_pre
          val2 = V5diffs

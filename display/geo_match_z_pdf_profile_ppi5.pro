...
; All Rights Reserved.
;
; geo_match_z_pdf_profile_ppi3.pro         Morris/SAIC/GPM_GV      November 2008
;
; DESCRIPTION
; -----------
...
;-
;===============================================================================
pro geo_match_z_pdf_profile_ppi5, PDF_LEVEL=pdf_level, SPEED=looprate, $
...
FUNCTION geo_match_z_plots, ncfilepr, height, looprate, elevs2show, PPIorient, $
                            windowsize, pctabvthresh, PPIbyThresh
;
; DESCRIPTION
; -----------
...
print, ' Level |  PR-GV    NumPts   PR-GV(s)  NumPts   PR-GV(c)  NumPts    AvgDist   PR MaxZ   GV MaxZ '
print, ' -----   -------   ------   -------   ------   -------   ------    -------   -------   ------- '
...
;                  TITLE = strmid( bname, 7, prlen-10), CHARSIZE=1
                  TITLE = strmid( bname, 7, 17)+"  "+'!m'+STRING("142B)+pctString+"% of bins required", CHARSIZE=1
...
IF ( counthavezs GT 0 ) THEN BEGIN
   oplot, prmnz2plot[idxhavezs], h2plot[idxhavezs], COLOR = 100, LINESTYLE=1, thick=3
   oplot, gvmnz2plot[idxhavezs], h2plot[idxhavezs], COLOR = 200, LINESTYLE=1, thick=3
ENDIF
...
idxhavezs = WHERE( prmnz2plot GT 0.0 and gvmnz2plot GT 0.0, counthavezs )
IF ( counthavezs GT 0 ) THEN BEGIN
   oplot, prmnz2plot[idxhavezs], h2plot[idxhavezs], COLOR = 100, LINESTYLE=2, thick=2
   oplot, gvmnz2plot[idxhavezs], h2plot[idxhavezs], COLOR = 200, LINESTYLE=2, thick=2
ENDIF
...
XYOutS, 39.5, 15.9, 'Any Type', COLOR = 100, CHARSIZE=1

yvalsleg2 = 15
plots, xvalsleg1, yvalsleg2, COLOR = 100, LINESTYLE=1, THICK=3
XYOutS, 39.5, 14.9, 'Stratiform', COLOR = 100, CHARSIZE=1

yvalsleg2 = 14
plots, xvalsleg1, yvalsleg2, COLOR = 100, LINESTYLE=2, THICK=2
...
IF ( PPIbyThresh ) THEN BEGIN
   prtitle = "PR, points with "+'!m'+STRING("142B)+pctString+"% of bins above Z threshold"
   myprbuf2 = plot_sweep_2_zbuf( zcor_in2, mysite.site_lat, mysite.site_lon, xCorner, $
                             yCorner, mygeometa.num_footprints, ifram, $
                             WINSIZ=windowsize, TITLE=prtitle )
   gvtitle = mysite.site_ID+", points with "$
             +'!m'+STRING("142B)+pctString+"% of bins above Z threshold"

...
if(cpstatus eq 'OK') then begin
 ...
 ; create data field arrays of correct dimensions and read data fields
  ...
  status = read_geo_match_netcdf_csu( ncfile1,  dbzgv=gvz, dbzcor=zcor, $
               grAtten=grAtten, grMaxAtten=grMaxAtten, $
...
WHILE ( !Mouse.Button EQ 1 ) DO BEGIN
   ...
   IF ( scanNum GT 2 AND scanNum LT 251B ) THEN BEGIN  ; account for +3 offset and hot corner values
     ...
     ; grab the subarrays of geo-match data for the rays to be plotted in the
     ; cross section so that we can print them out for Bringi
      prsubarray = zcor[idxcurscan,*]
      rawsubarray = zraw[idxcurscan,*]
      grsubarray = gvz[idxcurscan,*]
      hgtsubarray = (top[idxcurscan,*] + botm[idxcurscan,*])/2.0
      attensubarray = grAtten[idxcurscan,*]
      pctprsubarray = pctgoodpr[idxcurscan,*]
      pctgrsubarray = pctgoodgv[idxcurscan,*]
      help, nframes
      help, prsubarray
      help, grsubarray
      help, attensubarray
      ...
      IF ( no_2a25 NE 1 ) THEN BEGIN
        ; generate the PR full-resolution vertical cross section plot
         plot_pr_xsection, scanNumpr, raystartpr, rayendpr, dbz_2a25, meanbb, $
                        DBZSCALE2A25, TITLE=caseTitle25
        ; output the corresponding PIA values to the terminal
        ; (per Bringi request Nov'09)
         print, ''
         print, 'Raynumber, Latitude, Longitude, PIA, Hgt AGL, PR Zcor, PR Zraw, GR Z, GR Atten, PRpctAbv, GRpctAbv field from Point A to Point B:'
         for ray2print = raystartpr, rayendpr do begin
            prrayprofile = prsubarray[ray2print-raystartpr,*]
	    rawrayprofile = rawsubarray[ray2print-raystartpr,*]
	    grrayprofile = grsubarray[ray2print-raystartpr,*]
	    hgtrayprofile = hgtsubarray[ray2print-raystartpr,*]
            attenrayprofile = attensubarray[ray2print-raystartpr,*]
            pctprrayprofile = pctprsubarray[ray2print-raystartpr,*]
            pctgrrayprofile = pctgrsubarray[ray2print-raystartpr,*]
	    for lev2print = 0, nframes-1 do begin
	       print, ray2print, geolocation[0,ray2print,scanNumpr], $
                geolocation[1,ray2print,scanNumpr], pia[0,ray2print,scanNumpr], $
		hgtrayprofile[lev2print], prrayprofile[lev2print], $
		rawrayprofile[lev2print], grrayprofile[lev2print], attenrayprofile[lev2print], $
                pctprrayprofile[lev2print], pctgrrayprofile[lev2print]
	    endfor
         endfor
         print, ''
      ENDIF
...
pro pr_and_geo_match_x_sections_csu, ELEV2SHOW=elev2show, SITE=sitefilter, $

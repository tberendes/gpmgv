...
; All Rights Reserved.
;
; pr2tmi_rainrate_comparisons.pro
; - Morris/SAIC/GPM_GV  Dec. 2012
;
; DESCRIPTION
; -----------
; Performs a case-by-case statistical analysis of orbit-match TMI and PR
...
; INTERNAL MODULES
; ----------------
; 1) pr2tmi_rainrate_comparisons - Main procedure called by user.  Checks
;                                  input parameters and sets defaults.
;
; 2) pr2tmi_rr_plots - Workhorse procedure to read data, compute statistics,
...
; - Created from tmi2gr2pr_rainrate_comparisons.pro
; 03/01/13 Morris, GPM GV, SAIC
; - Added default value for ANIMATE parameter to avoid undefined variable error.
; 04/04/13 Morris, GPM GV, SAIC
; - Added BLANK_REJECTS binary keyword and logic to blank out below-threshold
;   footprints from animation images.
; - Added logic to blank out rainy V7 ocean samples with PoP below threshold.
...
;===============================================================================
;
; MODULE 1:  pr2tmi_rr_plots
;
; DESCRIPTION
; -----------
; Reads TMI and PR rainrate, and spatial fields from a user-selected PR2TMI
; matchup netCDF file, builds index arrays of categories rain type. 
; Computes mean TMI-PR rainrate differences for each of the 3 underlying
...
; of TMI and PR rainrate for each of these 3 surface types where data exists
; for that type, for each of 3 rain type categories: Any, Stratiform, and
; Convective. 
;
; Also produces a single image of TMI rainrate plotted on a map background,
; or an animation loop of TMI, PR, and Combined TMI/PR rainrate images either
...
FUNCTION pr2tmi_rr_plots, mygeomatchfile, looprate, windowsize, $
...
orbit = parsed[2]
version = parsed[3]
; set 4-digit year for yymmdd in V6 case
IF version EQ '6' THEN $
   if strmid(yymmdd,0,2) GT 90 THEN yymmdd = '19'+yymmdd ELSE yymmdd = '20'+yymmdd
...
; read the PR-TMI matchup data from netCDF file into a structure
the_data=READ_PR2TMI_MATCHUP(mygeomatchfile)
...
IF ( the_data.matchupmeta.trmm_version EQ 7 ) THEN BEGIN
   idxpopmiss = WHERE( the_data.PoP GT 100, npopmiss )
   IF npopmiss GT 0 THEN the_data.PoP[idxpopmiss] = -99   ; restore PoP "Missing value"
   idxpopok = WHERE( the_data.PoP GE pop_threshold, countpopok )
   idxtmirain = WHERE( the_data.tmirain GE RRcut, nsfcrainy )
   print, ''
   FMTSTR='("#TMI footprints GE ",I0,"% PoP = ", I0, ", #footprints > "'+ $
          ',F0.2," mm/h: ", I0, ",  #footprints = ", I0)'
   print, pop_threshold, countpopok, RRcut, nsfcrainy, N_ELEMENTS(the_data.PoP), $
          FORMAT=FMTSTR
;   print, PoP
ENDIF
...
XY_km = map_proj_forward( the_data.TMIlongitude, the_data.TMIlatitude, map_structure=smap ) / 1000.
...
IF ( do_ps EQ 1 ) THEN BEGIN
  ; set up postscript plot params. and file path/name
   ...
   PSTITLE = 'TMIvsPR.'+yymmdd+"."+orbit+".V"+version
...
;   num_over_SfcType = LONARR(4)
   ...
   ; blank out version 7 TMI over-ocean rainrate where PoP is below threshold
   IF ( the_data.matchupmeta.trmm_version EQ 7 ) THEN BEGIN
...
IF (do_ps EQ 1) THEN BEGIN
   ...
   printf, tempunit, '*'
ENDIF
textout = '* TMI-PR Rain Rate difference statistics (mm/h) - ' $
          +'   Orbit: '+orbit+'   V'+version
print, textout & IF (do_ps EQ 1) THEN printf, tempunit, textout
print, '*' & IF (do_ps EQ 1) THEN printf, tempunit, '*'

IF ( the_data.matchupmeta.trmm_version EQ 7 ) THEN $
   PRINT, "* Number of TMI/ocean rainy points with POP < ", $
...
print, textout & IF (do_ps EQ 1) THEN printf, tempunit, textout
textout = '*  type  | TMI-PR    NumPts | TMI-PR    NumPts | TMI-PR    NumPts  |  TMIMaxRR  PR MaxRR '
print, textout & IF (do_ps EQ 1) THEN printf, tempunit, textout
textout = '*  ----- | -------   ------ | -------   ------ | -------   ------  |  --------  -------- '
print, textout & IF (do_ps EQ 1) THEN printf, tempunit, textout
      ...
      if(havematch eq 1) then begin
        ...
        ; compute histogram of log range category, ignoring the lowest (below 0.1 mm/h)
         ...
         IF ( ~ hideTotals ) THEN BEGIN
            ...
            xyouts, 0.34, 0.925, 'PR (all)', COLOR=GV_COLR, /NORMAL, $
                    CHARSIZE=1*CHARadj
         ...
         mndifhline = 'TMI/PR Vol. Bias: ' + mndifhstr
        ...
        ; OVERLAY CONVECTIVE RAINTYPE PDFS, IF ANY POINTS
         ...
         IF ( nconv GT 0 ) THEN BEGIN
           ...
           xyouts, 0.34, 0.825, 'PR (Conv)', COLOR=GV_COLR, /NORMAL, CHARSIZE=1*CHARadj
        ...
        ; OVERLAY STRATIFORM RAINTYPE PDFS, IF ANY POINTS
         ...
         IF ( nstrat GT 0 ) THEN BEGIN
           ...
           xyouts, 0.34, 0.875, 'PR (Strat)', COLOR=GV_COLR, /NORMAL, CHARSIZE=1*CHARadj
...
IF ( do_ps EQ 1 ) THEN BEGIN
   ...
   plot_scatter_by_sfc_type3x3_ps, PSFILEpdf, PDFTITLE, 'PR', sfcRain, gvrr, $
                            rnType, sfcCat, num4hist3, idx4hist3, $
                            MIN_XY=0.5, MAX_XY=150.0, UNITS='mm/h'
ENDIF ELSE BEGIN
   plot_scatter_by_sfc_type3x3, PDFTITLE, 'PR', sfcRain, gvrr, rnType, sfcCat, $
...
ENDIF ELSE BEGIN
   ...
   IF KEYWORD_SET(blank_rejects) THEN BEGIN
      ; find points where any of PR, TMI, or COM are non-zero
      idxrrnonzero = WHERE( the_data.tmiRain GT 0.0 $
      ...
      ; find out where any/all of TMI, PR, or COM rain rate is below RRcut
      idxrrbelowthresh = WHERE( the_data.tmiRain[idxrrnonzero] LT RRcut $
      ...
      IF num_rr_blo_thresh GT 0 THEN BEGIN
         ...
         blankedTMI=the_data.tmiRain[idxrrnonzero[idxrrbelowthresh]]
         ...
         the_data.tmiRain[idxrrnonzero[idxrrbelowthresh]] = 0.0
...
CASE loopit OF
  0 : plot_rainrate_swath, the_data, ANIMATE=loopit, /PRECUT, DO_PS=do_ps
  1 : plot_rainrate_swath, the_data, ANIMATE=loopit, /PRECUT, DO_PS=do_ps
  2 : animate_rainrate_bscan, the_data, /PRECUT
ENDCASE

IF KEYWORD_SET(blank_rejects) THEN BEGIN
   IF num_rr_blo_thresh GT 0 THEN BEGIN
      ; restore the blanked-out values
      the_data.tmiRain[idxrrnonzero[idxrrbelowthresh]] = blankedTMI
...
;===============================================================================
;
; MODULE 2:  pr2tmi_rainrate_comparisons
;
; DESCRIPTION
; -----------
...
;
;                 0 - (default) display a static image of TMI rain rate on a map
;                     backgound
;                 1 - display an animation sequence of TMI, PR, and TMI-PR
;                     Combined rain rate images on a map background
;                 2 - as for (1), but in ray vs. scan coordinates rather than on
;                     a map.
;

pro pr2tmi_rainrate_comparisons, SPEED=looprate, $
                                 NCPATH=ncpath, $
...
IF ( N_ELEMENTS(filefilter) NE 1 ) THEN BEGIN
   print, "Defaulting to PRtoTMI* for file pattern."
   ncfilepatt = 'PRtoTMI*'
...
;         action = 0
         action = pr2tmi_rr_plots( mygeomatchfile, looprate, win_size, $
                                   POP_THRESHOLD=pop_threshold, RR_CUT=RRcut, $
...
ENDIF ELSE BEGIN
   print, ''
   print, 'Select a PRtoTMI file from the file selector, or Cancel to exit.'
   mygeomatchfile = dialog_pickfile(path=pathpr, filter = ncfilepatt, $
                                    TITLE='Select a PRtoTMI file')
...
;      action = 0
      if (action) then break
      print, ''
      print, 'Select another PRtoTMI file from the file selector, or Cancel to exit.'
      mygeomatchfile = dialog_pickfile(path=pathpr, filter = ncfilepatt, $
                                       TITLE='Select a PRtoTMI file')
   endwhile
ENDELSE

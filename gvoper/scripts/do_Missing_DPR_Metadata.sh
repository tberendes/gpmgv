...
#
# Computes missing rain event metadata from 2AKu or 2ADPR data for any or all
# ground radar sites.  Assumes that the overpass event information for the sites
# is present in the gpmgv database.  Has hard-coded defaults for PPS version,
# Instrument, and Algorithm that can be overridden by command line
# parameters.
#
###############################################################################
...
||to_char(d.filedate,'DD')||'/'||d.filename, d.orbit, c.radar_id, count(*)\
     ...
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
        AND c.subset = d.subset AND c.sat_id='$SAT_ID' and c.subset IN ('CONUS') \
...
||d.subset||'/'||to_char(d.filedate,'YYYY')||'/'||to_char(d.filedate,'MM')||'/'\
||to_char(d.filedate,'DD')||'/'||d.filename, d.orbit, count(*)\
     from eventsatsubrad_vw c\
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
        AND c.subset = d.subset AND c.sat_id='$SAT_ID' and c.subset IN ('CONUS') \
        AND d.product_type = '${ALGORITHM}' and c.nearest_distance<=${MAX_DIST} \
...
AND C.ORBIT != 8107 \
       and 11 > (select count(*) from event_meta_numeric \
       where event_num=c.event_num) group by 1,2 \
     order by 2;" | psql gpmgv  | tee -a $LOG_FILE 2>&1
...
tail $filelist
echo ""
#exit  # uncomment if just testing for missed metadata processing

if [ -s $filelist ]
  then
       for thisrow in `cat $filelist`
#       for thisrow in `head -50 $filelist`
         do
...
#           echo ""  | tee -a $LOG_FILE
    ...
    if [ -s $outfileall ]
      then
       ...
       #   so we need to remove all values with this tag from appstatus table.
        ...
        if [ $? = 1 ]
          then
            echo "get2ADPRMeta4date.sh status = MISSING for $yymmdd" \
                 | tee -a $LOG_FILE 2>&1
            ...
            echo "set status to SUCCESS." | tee -a $LOG_FILE
            echo "" | tee -a $LOG_FILE
        fi
      else
        echo "" | tee -a $LOG_FILE
    ...
    echo "No missing metadata 2AKu files found, quitting." | tee -a $LOG_FILE
fi

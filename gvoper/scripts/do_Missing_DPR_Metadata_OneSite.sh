...
###############################################################################
#
# do_Missing_DPR_Metadata_OneSite.sh    Morris/SAIC/GPM GV    Jan 2015
#
...
# GR site, and Instrument that can be overridden by command line parameters.
#
###############################################################################
...
||to_char(d.filedate,'DD')||'/'||d.filename, d.orbit, c.radar_id, count(*)\
     ...
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
        AND c.subset = d.subset AND c.sat_id='$SAT_ID' and c.subset = \
           (select subset from siteproductsubset where sat_id='$SAT_ID' and radar_id='$GRSITE') \
    ...
    if [ -s $outfileall ]
      then
       ...
       #   so we need to remove all values with this tag from appstatus table.
        echo "delete from appstatus where app_id = 'get2ADPRMeta' and datestamp = '${yymmdd}';" \

...
#
# DESCRIPTION
#   Runs IDL in batch mode to extract metadata from 2AKu or 2ADPR data files
#   for NEXRAD sites overpassed in the orbit.  Files, orbits, and overpassed
#   NEXRAD sites are listed in the well-known filename file2aDPRsites.YYMMDD.txt
#   in TMP_DIR whose YYMMDD datestamp is provided as the single argument to
...
#
# FILES
#   file2aDPRsites.YYMMDD.txt (INPUT) - text file listing 2A* files, orbits, and
...
THISDATE=`date -u +%y%m%d`
if [ $# != 1 ] 
  then
#     THISRUN=`date -u +%y%m%d`
     LOG_FILE=${META_LOG_DIR}/get2ADPRMeta4date.${THISDATE}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2ADPRMeta4date.${THISDATE}.log
echo "Processing 2ADPR metadata on date ${THISDATE}" | tee -a $LOG_FILE
echo "New data datestamp: ${THISRUN}" | tee -a $LOG_FILE
echo "+++++++++++++++++++++++++++++++++++++++++++" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
# Add this run's yymmdd to to-do list, whether or not a control file for this
# date was already processed successfully.  We may get file(s) for this
# datestamp on more than one date in the new PPS procedures. 
# Let loadmetadata.sh deal with any duplicate metadata loading attempts.
...
#    fi
  ;;
  $FAILED )
    echo "Status indicates yymmdd = ${THISRUN} had prior fatals."\
     | tee -a $LOG_FILE
    echo $THISRUN >> $META2ADPR
#    if [ $have_retries = 'f' ]
#      then
...
for yymmdd in `cat $META2ADPR`
  do
    ...
    if [ -s $GETMYMETA ]
      then
        RUNDATE=$yymmdd
       	export RUNDATE            # IDL getmetadata2adpr procedure needs this
       	export GETMYMETA          # IDL getmetadata2adpr procedure needs this
       	echo "" | tee -a $LOG_FILE
       	...
       	echo "" | tee -a $LOG_FILE
       	echo "=============================================" | tee -a $LOG_FILE
       	echo "Calling IDL for yymmdd = ${yymmdd}, control file = $GETMYMETA" \
            	 | tee -a $LOG_FILE
       	echo "" | tee -a $LOG_FILE

...
################################################################################
#
#  get_PPS_CS_v4testdata.sh     Morris/SAIC/GPM GV     February 2014
#
#  DESCRIPTION
...
#    file types not of interest to the GPM Validation Network.  Despite the
#    name of this family of scripts, should work for any new ITE data versions
#    posted by the PPS, not just those for V04 development.
#
#  ROUTINES CALLED
#
#  FILES
                       ...
                       do
                         ...
                         if [ -s $2/$thisPPSfile ]
                           then
                             ...
                             # hard-code the test version specification, ignoring the file path (NO!)
                             #Version="V4ITE"
                             ...
                             if [ ! -s $targetVersDir ]
                               then
                                 ...
                                 update_CS_dirs_v4test.sh  ${satellite}/${Instrument}/${Algo}  ${Version}\
                                   | tee -a $LOG_FILE
                             fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT * FROM appstatus WHERE \
 app_id = 'get_ITE_V4' AND datestamp = '$rundate';" | psql -a -d gpmgv \
 | tee -a $LOG_FILE 2>&1
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus(app_id,datestamp,status) VALUES \
      ('get_ITE_V4','$rundate','$UNTRIED');" | psql -a -d gpmgv \
      | tee -a $LOG_FILE 2>&1
fi
...
echo "" | tee -a $LOG_FILE
echo "UPDATE appstatus SET ntries=ntries+1 WHERE app_id = 'get_ITE_V4' AND \
...
if [ -s $FILES2DO ]
  then
    ...
    ${BIN_DIR}/getPPSftpListings_v4test.sh
    if [ -s ${LOG_DIR}/PPSftpListings_v4test.${rundate}.log ]
      then
        ...
        echo "No valid ftp_url files downloaded by getPPSftpListings_v4test.sh, exiting."
        echo "Mark incomplete in database:" | tee -a $LOG_FILE
        echo "" | tee -a $LOG_FILE
        echo "UPDATE appstatus SET status = '$MISSING' WHERE \
	       app_id = 'get_ITE_V4' AND datestamp = '$rundate';" \
            | psql -a -d gpmgv | tee -a $LOG_FILE 2>&1
        exit 1
    fi
...
echo "Set status to FAILED where this is the 5th failure for any downloads:"\
 | tee -a $LOG_FILE
echo "UPDATE appstatus SET status='$FAILED' WHERE app_id = 'get_ITE_V4' AND \

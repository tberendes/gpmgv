...
datelist=${TMP_DIR}/doAllGrid_dates_temp.txt
...
for thisdate in `cat $datelist`
  do
   ...
   # is run-date-specific.
    filelist=${TMP_DIR}/PR_filelist4gvMeta_temp.txt
    outfile=${TMP_DIR}/PR_files_sites4gvMeta_temp.txt
    outfileall=${TMP_DIR}/PR_files_sites4gvMeta.${yymmdd}.txt
    ...
    # 1/17/2014, Morris GPM GV - added 'distinct' constraint to query, got messed
    #   up in case where two 1CUF files were present within the collatedgvproducts
    ...
    for row in `cat $filelist`
      do
        ...
        echo "\t \a \f '|' \o $outfile \\\ select distinct a.event_num, a.radar_id, \
...
#exit
    ...
    if [ -s $outfileall ]
      then
        ...
        DBOUTFILE=${TMP_DIR}/gvMetadata.${yymmdd}.unl
        export DBOUTFILE
        if [ -f $DBOUTFILE ]
          then
            rm -v $DBOUTFILE

...
#    October 2015 - Morris - Added a 'sort' when running through the list of
#                            files in GTsToGet.
#    June 2015 - Morris - Added yearnow computation to set PREDICT_DIR to
#                         GT7 subdirectory for current year.
#
################################################################################
...
yearnow=`date -u +%Y`
PREDICT_DIR=/data/tmp/ground_track_7day/${yearnow}
...
LAST_FILE=`cat $files7day | sort | head -1`  # initialize "prior" 7-day file
WORKINGDATE=`echo $LAST_FILE | cut -f3 -d '.'`  # date of prior 7-day
echo "In extract_daily_predicts.sh, first file to consider: $LAST_FILE"
...
for file in `cat $files7day | sort`
  do

...
################################################################################
#
#  do_DPRGMI_geo_matchup4date.sh    Morris/SAIC/GPM GV    April 2014
#
#
#  DESCRIPTION
#
...
#
#   polar2dprgmi_V.bat (EXTERNAL) - Batch file of IDL commands to be run, where
#                                   V is defined by the value of $PARAMETER_SET,
#                                   e.g., polar2dprgmi_1.bat.
#
#
#  DATABASE
#
...
#                         value to define the version of the polar2dprgmi ".bat"
...
export IDL_PRO_DIR                # IDL polar2dprgmi_V.bat needs this
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/do_DPRGMI_geo_matchup4date.${THISRUN}.fatal.log
...
LOG_FILE=${LOG_DIR}/do_DPRGMI_geo_matchup4date.${THISRUN}.log
...
if [ -s $CONTROLFILE ]
  then
    #export THISRUN              # IDL polar2dprgmi.bat needs this
    export CONTROLFILE          # IDL polar2dprgmi.bat needs this
  else
    echo "ERROR in do_DPRGMI_geo_matchup4date.sh, control file $CONTROLFILE not found."\
     | tee -a $LOG_FILE
    exit 1
fi
...
# polar2dprgmi batch file to use
if [ ! -s ${IDL_PRO_DIR}/polar2dprgmi_${PARAMETER_SET}.bat ]
  then
     echo "" | tee -a $LOG_FILE
     echo "FATAL: Script ${IDL_PRO_DIR}/polar2dprgmi_${PARAMETER_SET}.bat not found, " \
       | tee -a $LOG_FILE
...
if [ -s $CONTROLFILE ]   # legacy if check, moved error logic to top of script
  then
    ...
    echo "Calling IDL for yymmdd = ${THISRUN}, file = $CONTROLFILE" \
     | tee -a $LOG_FILE
    echo "using batch file ${IDL_PRO_DIR}/polar2dprgmi_${PARAMETER_SET}.bat" \
     | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
       
    $IDL < ${IDL_PRO_DIR}/polar2dprgmi_${PARAMETER_SET}.bat | tee -a $LOG_FILE 2>&1
...
echo "" | tee -a $LOG_FILE
echo "do_DPRGMI_geo_matchup4date.sh complete, exiting." | tee -a $LOG_FILE
exit 0

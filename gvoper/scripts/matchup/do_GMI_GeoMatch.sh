...
while read thisdate
  do
   ...
   # 'outfileall', which is run-date-specific.
    ...
    outfileall=${TMP_DIR}/GMI_files_sites4geoMatch.${yymmdd}.txt
...
'${SAT_ID}/${INSTRUMENT_ID}/${ALGORITHM1C}/${PPS_VERSION}/'||x.subset||'/'||to_char(x.filedate,'YYYY')||'/'\

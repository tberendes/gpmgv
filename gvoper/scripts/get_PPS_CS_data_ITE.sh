...
################################################################################
#
#  get_PPS_CS_dataITE.sh     Morris/SAIC/GPM GV     February 2014
#
#  DESCRIPTION
...
export CS_BASE
TMP_CS_DATA=/data/tmp/PPS_ITE
export TMP_CS_DATA
BIN_DIR=${GV_BASE_DIR}/scripts

LOG_DIR=/data/logs
rundate=`date -u +%y%m%d`
LOG_FILE=${LOG_DIR}/get_PPS_CS_data_ITE.${rundate}.log       # script log file
export LOG_FILE
...
# check for the presence of ftp_url file "$1" in TMP_CS_DATA ($2)
 ...
 # for current ITE run, here is the format:
 #ftps://todd.a.berendes%40nasa.gov:todd.a.berendes%40nasa.gov@arthurhou.pps.eosdis.nasa.gov/itedata/iteorder/todd.a.berendes@nasa.gov/V757/2019/11/23/radar/2A-CS-CONUS.GPM.Ka.V9-20210415.20191123-S223238-E224108.032595.ITE757.HDF5
 if [ "$foundman" = "y" ]
   then
           ...
           do
             for datatype in `echo $datatypes`
               do
                 ...
                 grep $filepre $2/$1 | grep -v 'TRMM.PR.2A21' | cut -f4-12 -d '/' \
...
if [ ${pgproccount} -lt 3 ]
  then
    thistime=`date -u`
    echo "Message from get_PPS_CS_data_ITE.sh cron job on ${thistime}:" \
...
#    ${BIN_DIR}/getPPSftpListings.sh
    ${BIN_DIR}/getPPSftpListings_wget_ITE.sh
...
#        grep Got ${LOG_DIR}/PPSftpListings_ITE.${rundate}.log
        grep ftps_url_ ${LOG_DIR}/PPSftpListings_ITE.${rundate}.log
        if [ $? -eq 0 ]
          then
...
# TAB 8/6/20 modified to handle wget output
            URLFILES=`grep ftps_url_ ${LOG_DIR}/PPSftpListings_ITE.${rundate}.log | cut -f4 -d" "`
        fi
      else
        echo "No valid ftps_url files downloaded by getPPSftpListings_ITE.sh, exiting."

#!/bin/sh
#
# getPPSftpListings_ITE.sh    Morris/SAIC/GPM GV    April 2014
#
# DESCRIPTION:
#
# Uses Perl script 'mirror' to create a local mirror of the PPS ftp location
# where ftp_url_YYMMDDHHMM.txt listing files for new Internal Test and
# Evaluation (ITExxx, xxx=001,002,...) PPS products are located.  Makes up to 10
...
#
# arthurhou.pps.eosdis.nasa.gov.ITE - Package file, holds configuration for
#                           mirror script to mirror ITE files from PPS ftp site.
#
...
#
#  DATABASE
#    Catalogs PPS subset file metadata in orbit_subset_product table in 'gpmgv'
...
#    2015, Morris, SAIC, GPM GV
#    - Created from getPPSftpListings.sh
#    4/27/2017, Morris, SAIC, GPM GV
#    - Created from getPPSftpListings_v4test.sh.  Changed name of MIR_PACKAGE,
#      LOG_FILE, and renaming of MIR_LOG_FILE.  Changed app_id value used in
#      database entries from getV4tesList to get_ITE_List.
#
################################################################################
...
MIR_DATA_DIR=${TMP_DIR}/PPS_v4test
...
MIR_PACKAGE=${MIR_PACKAGES_DIR}/arthurhou.pps.eosdis.nasa.gov.ITE
...
if [ ${pgproccount} -lt 3 ]
  then
    ...
    mailx -s 'postgresql down on ${HOST}' \
      -c kenneth.r.morris@nasa.gov,todd.a.berendes@nasa.gov,berendt@uah.edu \

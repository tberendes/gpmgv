...
###############################################################################
#
# do_DPR2GR_GeoMatch.sh    Morris/SAIC/GPM GV    March 2016
#
...
#        -f option (e.g., "do_DPR2GR_GeoMatch.sh -f"), then the status of prior
...
LOG_FILE=${LOG_DIR}/do_DPR2GR_GeoMatch.${rundate}.log
export rundate
...
# listed in the do_DPR2GR_geo_matchup_catalog.yymmdd.txt file, in turn produced by
# do_DPR2GR_geo_matchup4date.sh by examining the do_DPR2GR_geo_matchup4date.yymmdd.log file. 
# Formats catalog entry for the geo_match_product table in the gpmgv database,
# and loads the entries to the database.

YYMMDD=$1
MATCHUP_LOG=${LOG_DIR}/do_DPR2GR_geo_matchup4date.${YYMMDD}.log  # NOT USED
DBCATALOGFILE=$2
...
for ncfile in `cat $DBCATALOGFILE`
  do
   ...
   # NEED TO EXTRACT THE SWATH TYPE OUT OF THE FILE NAMES AS MULTIPLE SWATHS
   # ARE RUN AUTOMATICALLY.  $SWATH MUST BE LOCAL TO THIS FUNCTION
    local SWATH=`echo ${ncfile} | cut -f7 -d '.'`
...
dateEnd='2016-03-01'
#dateEnd='2015-01-01'
# Get better performance using a query pair with the "left outer join geo_match_product"
...
from eventsatsubrad_vw c JOIN orbit_subset_product o \
  ON c.orbit = o.orbit AND c.subset = o.subset AND c.sat_id = o.sat_id \
   ...
   and c.subset = 'BrazilRadars' and c.nearest_distance<=${MAX_DIST} \
...
echo "SELECT c.* into temp tempevents \
from eventsatsubrad_vw c JOIN orbit_subset_product o \
  ON c.orbit = o.orbit AND c.subset = o.subset AND c.sat_id = o.sat_id \
   and o.product_type = '${ALGORITHM}' and o.version='${PPS_VERSION}' and o.sat_id='${SAT_ID}' \
   and c.subset = 'BrazilRadars' and c.nearest_distance<=${MAX_DIST} \
...
||to_char(d.filedate,'MM')||'/'||to_char(d.filedate,'DD')||'/'||d.filename as file2a \
     ...
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
        AND c.subset = d.subset AND c.sat_id='$SAT_ID' and c.subset = 'BrazilRadars' \
     ...
     select  c.orbit, count(*), c.datestamp, c.subset, c.version, c.instrument, c.swath, c.file2a\
       from possible_2adpr c left outer join geo_match_product b on (c.radar_id=b.radar_id \
        ...
        and b.parameter_set=${PARAMETER_SET} and b.sat_id='GPM') where b.pathname is null \
	...
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select a.event_num, a.orbit, \
	    ...
	    collate_satsubprod_1cuf c \
          ...
          where a.radar_id = b.instrument_id and a.radar_id = c.radar_id  \
            ...
            AND C.FILE1CUF NOT LIKE '%rhi%' and a.radar_id like '%1' \
          order by 3,9;
...
#exit  # if uncommented, creates the control file for first date, and exits
    ...
    if [ -s $outfileall ]
      then
       # Call the IDL wrapper script, do_DPR2GR_geo_matchup4date.sh, to run
       ...
       # has been done before.
        ...
        case $? in
          0 )
            echo ""
            echo "SUCCESS status returned from do_DPR2GR_geo_matchup4date.sh"\
             | tee -a $LOG_FILE
           ...
           # must be identically defined here and in do_DPR2GR_geo_matchup4date.sh
          ...
          1 )
            echo ""
            echo "FAILURE status returned from do_DPR2GR_geo_matchup4date.sh, quitting!"\
             | tee -a $LOG_FILE
            exit 1
          ;;
          2 )
            echo ""
            echo "REPEAT status returned from do_DPR2GR_geo_matchup4date.sh, do nothing!"\
             | tee -a $LOG_FILE
          ;;
        esac

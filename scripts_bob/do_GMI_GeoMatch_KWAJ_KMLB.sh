...
#                           do_DPR_GeoMatch.sh.
# 12/03/14   Morris        - Added input/internal parameter GEO_MATCH_VERSION,
...
PPS_VERSION=ITE062        # controls which GMI products we process
export PPS_VERSION
...
LOG_FILE=${LOG_DIR}/do_${SAT_ID}${INSTRUMENT_ID}_GeoMatch_KWAJ.${PPS_VERSION}.${rundate}.log
...
# Begin main script
echo "Starting GMI-GR matchups on $rundate." | tee $LOG_FILE
echo "========================================================"\
 | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE

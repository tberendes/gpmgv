...
PPS_VERSION="V01G"        # controls which PR products we process
export PPS_VERSION
...
for thisdate in '2014-06-02'  #`cat $datelist`
  do
   ...
   # 'outfileall', which is run-date-specific.
    filelist=${DATA_DIR}/tmp/DPR_filelist4geoMatch_temp.txt
    outfile=${DATA_DIR}/tmp/DPR_files_sites4geoMatch_temp.txt
    outfileall=${DATA_DIR}/tmp/DPR_files_sites4geoMatch.${yymmdd}.txt
    ...
    if [ -s $outfileall ]
      then
       ...
       # has been done before.
        ...
        ${BIN_DIR}/do_DPR_geo_matchup4date.sh $yymmdd

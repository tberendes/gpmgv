  ...
  then
    ...
    if [ ${pgproccount} -lt 3 ]
      then
        if [ ! -f $DBERRMSG_FILE ]
          then
            ...
            echo "${pgproccount} Postgres processes active, should be >= 3 !!" \
              >> $DBERRMSG_FILE
            echo "UNABLE TO RESTART POSTGRESQL ON ws1-gpmgv." >> $DBERRMSG_FILE
	    # \ pankaj.jaiswal@nasa.gov -c
            mailx -s 'postgresql down on ws1-gpmgv'  \
               kenneth.r.morris@nasa.gov < $DBERRMSG_FILE
        ...
        echo "${pgproccount} Postgres processes active, should be >= 3" \
          >> $DBERRMSG_FILE
        echo "NOTE: HAD TO RESTART POSTGRESQL ON ws1-gpmgv." >> $DBERRMSG_FILE
        mailx -s 'postgresql restart on ws1-gpmgv'  \
           kenneth.r.morris@nasa.gov < $DBERRMSG_FILE
        cat $DBERRMSG_FILE | tee -a $LOG_FILE
    fi
  else
	...
	if [ $? = 0 ]
	    ...
	    echo "FATAL: Could not load data from ${HELDTEMPFILE} to database!"\
	      >> $DBERRMSG_FILE
            ...
            mailx -s 'postgresql error on ws1-gpmgv' kenneth.r.morris@nasa.gov \
              < $DBERRMSG_FILE
            cat $DBERRMSG_FILE | tee -a $LOG_FILE
          else
...
/usr/bin/wget --tries=1 --mirror -P ${MOSAIC_BASE_DATA} \
   http://www.srh.noaa.gov/ridge/Conus/RadarImg/latest.gif

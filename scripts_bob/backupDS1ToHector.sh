...
echo ""
echo "Has this been done, or do you want to skip it? (Y or N):"
read -r bail
if [ "$bail" != 'Y' -a "$bail" != 'y' ]
  then
     if [ "$bail" != 'N' -a "$bail" != 'n' ]
...
target=${THESERVER}:\~/data/gpmgv/db_backup
ssh ${THESERVER} "ls ~/data/gpmgv/db_backup" | grep gpmgvDBdump.gz > /dev/null 2>&1
if [ $? = 0 ]
  then
echo "Found gpmgvDBdump.gz on ${THESERVER}:\~/data/gpmgv/db_backup" | tee -a $LOG_FILE
    ssh ${THESERVER} "mv -v ~/data/gpmgv/db_backup/gpmgvDBdump.gz \
                      ~/data/gpmgv/db_backup/gpmgvDBdump.old.gz" | tee -a $LOG_FILE
    pg_dump -f /data/gpmgv/tmp/gpmgvDBdump gpmgv | tee -a $LOG_FILE 2>&1
    gzip /data/gpmgv/tmp/gpmgvDBdump | tee -a $LOG_FILE 2>&1
    scp /data/gpmgv/tmp/gpmgvDBdump.gz $target | tee -a $LOG_FILE 2>&1
    rm -v /data/gpmgv/tmp/gpmgvDBdump.gz | tee -a $LOG_FILE 2>&1
  else
...
rsync -rtv --ignore-existing /data/gpmgv/coincidence_table/CT*archive.* \
     ${THESERVER}:\~/data/gpmgv/coincidence_table | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing /data/gpmgv/mosaicimages/archivedmosaic/ \
      ${THESERVER}:\~/data/gpmgv/mosaicimages/archivedmosaic | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing --exclude="*.R*" /data/gpmgv/netcdf/geo_match/ \
      ${THESERVER}:\~/data/gpmgv/netcdf/geo_match | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing --exclude="*HDF*" /data/gpmgv/prsubsets/ \
      ${THESERVER}:\~/data/gpmgv/prsubsets | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing /data/gpmgv/GRIB/NAMANL/ \
      ${THESERVER}:\~/data/gpmgv/GRIB/NAMANL | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing /data/gpmgv/netcdf/soundings/NAMANL/ \
      ${THESERVER}:\~/data/gpmgv/netcdf/soundings/NAMANL | tee -a $LOG_FILE 2>&1

# synch up gv_radar for year(s) listed in $years

echo "" | tee -a $LOG_FILE
echo "Back up /data/gpmgv/gv_radar" | tee -a $LOG_FILE
...
for site in `ls /data/gpmgv/gv_radar/defaultQC_in/`
  do
    for area in `ls /data/gpmgv/gv_radar/defaultQC_in/${site}`
      do
        ...
        for year in $years
          do
            rsync -rtv --ignore-existing \
                 /data/gpmgv/gv_radar/defaultQC_in/${site}/${area}/${year}/ \
                 ${THESERVER}:\~/data/gpmgv/gv_radar/defaultQC_in/${site}/${area}/${year} \
                 | tee -a $LOG_FILE 2>&1
        done
    done
done
...
for site in `ls /data/gpmgv/gv_radar/finalQC_in/`
  do
    ...
    for area in `ls /data/gpmgv/gv_radar/finalQC_in/${site}`
      do
        echo ${area} | grep "log_${site}.txt" > /dev/null 2>&1
	if [ $? = 0 ]
          then
        ...
        for year in $years
          do
            rsync -rtv --ignore-existing \
                 /data/gpmgv/gv_radar/finalQC_in/${site}/${area}/${year}/ \
                 ${THESERVER}:\~/data/gpmgv/gv_radar/finalQC_in/${site}/${area}/${year} \
                 | tee -a $LOG_FILE 2>&1
        done
    done
done

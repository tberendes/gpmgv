...
-- a V5 test version (ITE109) data file, with the GR data unadjusted to Ku (s2ku=0).
-- Of course, the double-dashes indicate SQL comment lines and would not be part of
-- the multi-line IDL command shown.

-- IDL> stats_by_dist_to_dbfile_dpr_pr_geo_match, pct=70, NAME_ADD='Block10_V04A', $
--      NCSITE='/data/gpmgv/netcdf/geo_match/GPM/2ADPR/NS/V04A/1_21/', out='/tmp', $
--      alt_BB='/data/tmp/GPM_rain_event_bb_km_Uniq.txt', VERSION2MATCH='ITE109', $
--      MAX_BLOCKAGE=0.1, s2ku=0

-- Table into which to load the output of stratified_by_dist_stats_to_dbfile, contained
-- in the file /tmp/StatsByDist_DPR_GR_Pct70_Block10_V04A_AltBB_DefaultS.unl
...
\copy zdiff_stats_dpr_v4 from '/tmp/StatsByDist_DPR_GR_Pct70_Block10_V04A_AltBB_DefaultS.unl' with delimiter '|' 
...
-- /tmp/StatsByDist_DPR_GR_Pct70_Block10_V04A_AltBB_S2Ku.unl
...
\copy zdiff_stats_dpr_v4_s2ku from '/tmp/StatsByDist_DPR_GR_Pct70_Block10_V04A_AltBB_S2Ku.unl' with delimiter '|'

--------------------------------------------------------------------------------------------

-- Here is an example of a run of the IDL procedure stats_by_dist_to_dbfile_dpr_pr_geo_match
-- that produces statistics for the V5 test version "ITE109", matched up case for case with
-- a V04A version data file, with the GR data unadjusted to Ku (s2ku=0):

-- IDL> stats_by_dist_to_dbfile_dpr_pr_geo_match, pct=70, NAME_ADD='Block10_ITE109', $
--      NCSITE='/data/gpmgv/netcdf/geo_match/GPM/2ADPR/NS/ITE109/1_21/', out='/tmp', $
--      alt_BB='/data/tmp/GPM_rain_event_bb_km_Uniq.txt', VERSION2MATCH='V04A', $
--      MAX_BLOCKAGE=0.1, s2ku=0

-- CREATE TABLES FOR THE DPR V5 TEST DATA
-- Table into which to load the output of stratified_by_dist_stats_to_dbfile, contained
-- in the file /tmp/StatsByDist_DPR_GR_Pct70_Block10_ITE109_AltBB_DefaultS.unl
...
\copy zdiff_stats_dpr_v5 from '/tmp/StatsByDist_DPR_GR_Pct70_Block10_ITE109_AltBB_DefaultS.unl' with delimiter '|' 
...
-- Table into which to load the output of stratified_by_dist_stats_to_dbfile
-- having the Ku adjustment to the GR data, contained in the file named
-- /tmp/StatsByDist_DPR_GR_Pct70_Block10_ITE109_AltBB_S2Ku.unl
...
\copy zdiff_stats_dpr_v5_s2ku from '/tmp/StatsByDist_DPR_GR_Pct70_Block10_ITE109_AltBB_S2Ku.unl' with delimiter '|'

...
###############################################################################
#
# doAnyGeoMatch4SomeDates.sh    Morris/SAIC/GPM GV    April 2014
#
# DESCRIPTION:
# Query gpmgv database for dates/times of any GR site overflight events and
# for a specified satellite/instrument and assemble a command file to drive
# the Instrument-GR geometry matching for events with data.  Ignores whether
# or not the overpass event was a "rainy" event.
#
# Completed geometry match files are cataloged in the 'gpmgv' database table
# 'geo_match_products'.
#
# 4/17/2014   Morris         Created from doGeoMatch4NewRainCases.sh.
#
###############################################################################
...
echo "TMI_VERSION: $TMI_VERSION"
echo "PARAMETER_SET: $PARAMETER_SET"
echo "ALGORITHM: $ALGORITHM"
echo "DATE2DO: $DATE2DO"

rundate=`date -u +%y%m%d`
LOG_FILE=${LOG_DIR}/do${SAT_ID}_${INSTRUMENT_ID}_GeoMatch.v${TMI_VERSION}.${rundate}.log
...
echo "select \
'${SAT_ID}/${INSTRUMENT_ID}/${ALGORITHM}/${TMI_VERSION}/'||d.subset||'/'||to_char(d.filedate,'YYYY')||'/'\
||to_char(d.filedate,'MM')||'/'||to_char(d.filedate,'DD')||'/'||d.filename\
     ...
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
        AND c.subset = d.subset AND c.sat_id='$SAT_ID' AND d.product_type = '${ALGORITHM}'\
    ...
    DBOUT2=`psql -a -A -t -o $filelist -e -d gpmgv -c "select \
'${SAT_ID}/${INSTRUMENT_ID}/${ALGORITHM}/${TMI_VERSION}/'||d.subset||'/'||to_char(d.filedate,'YYYY')||'/'\
||to_char(d.filedate,'MM')||'/'||to_char(d.filedate,'DD')||'/'||d.filename\
       as file2a12,\
       c.orbit, count(*), '${yymmdd}', c.subset, d.version \
   ...
   #  comes up more than once for an orbit.
	...
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select a.event_num, a.orbit, \
	    ...
	    and a.orbit = c.orbit  and c.sat_id='$SAT_ID' \
            ...
            and a.radar_id not in ('KMXX')  AND c.product_type = '${ALGORITHM}'\

  ...
  do
    for month in 01 02 03 04 05 06 07 08 09 10 11 12
      do
        ...
        #  in the format YYYY/MM-YYYY
        jdaydir=${year}/${month}-${year}
        echo "Get files for ${jdaydir} from hector." | tee -a $LOG_FILE
        wgetPRtypes4date $jdaydir $TMP_KWAJ_DATA
#        if [ $? -eq 4 ]
        if [ $? -eq 1 ]
          then
	    ...
	    echo "UPDATE appstatus SET status = '$SUCCESS' WHERE \
	          app_id = 'wgetKWAJ' AND datestamp = '${month}-${year}';" \
		  | tee -a $LOG_FILE 2>&1
            # Move the files into the baseline product directories
            # and catalog them in the database.
            echo "" | tee -a $LOG_FILE
            echo "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" | tee -a $LOG_FILE
            PRproductsToDB ${year}${month} $PR_BASE $LOG_DATA_FILE
            echo "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" | tee -a $LOG_FILE
        else
            echo "" | tee -a $LOG_FILE
            echo "Only got $? of the 4 types for ${month}-${year}" | tee -a $LOG_FILE
        fi
    done
done

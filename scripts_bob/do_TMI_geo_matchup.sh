...
################################################################################
#
# do_TMI_geo_matchup.sh    Morris/SAIC/GPM GV    May 2011
#
# DESCRIPTION
...
#
# FILES
#   TMI_files_sites4geoMatch.YYMMDD.txt (INPUT) - lists data files, orbits, and
...
#
#  LOGS
#    Output for script run is logged to file do_TMI_geo_matchup.log
...
#    do_TMI_geo_matchup.yymmdd.fatal.log in $LOG_DIR, where 'yymmdd' is for the
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/do_TMI_geo_matchup.${THISRUN}.fatal.log
...
LOG_FILE=${LOG_DIR}/do_TMI_geo_matchup.${THISRUN}.log
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'geo_match_tmi' AND status = '$UNTRIED' \
...
if [ ! -s ${IDL_PRO_DIR}/polar2tmi.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/polar2tmi.bat not found, exiting" \
       | tee -a $LOG_FILE
...
for yymmdd in `cat $DATES2GRID`
  do
    CONTROLFILE=${DATA_DIR}/tmp/TMI_files_sites4geoMatch.${yymmdd}.txt
    if [ -s $GETMYMETA ]
      then
        RUNDATE=$yymmdd
#	export RUNDATE              # IDL polar2tmi.bat needs this
	export CONTROLFILE          # IDL polar2tmi.bat needs this
	echo "" | tee -a $LOG_FILE
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'geo_match_tmi' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
...
echo "" | tee -a $LOG_FILE
echo "do_TMI_geo_matchup.sh complete, exiting." | tee -a $LOG_FILE
exit 0

...
LOG_DIR=/data/logs
ymd=`date -u +%Y%m%d`
LOG_FILE=$LOG_DIR/backupToDS1.${ymd}.log
THESERVER=ds1-gpmgv.gsfc.nasa.gov

echo "" | tee $LOG_FILE
echo "===================================================" | tee -a $LOG_FILE
echo "       Do full backup to ds1-gpmgv on $ymd." | tee -a $LOG_FILE
echo "---------------------------------------------------" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
rsync -rtv --ignore-existing /home/data/gpmgv/coincidence_table/CT*archive.* \
     ${THESERVER}:/data/gpmgv/coincidence_table | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing /home/data/gpmgv/mosaicimages/archivedmosaic/ \
      ${THESERVER}:/data/gpmgv/mosaicimages/archivedmosaic | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing --exclude="*.R*" /home/data/gpmgv/netcdf/geo_match/ \
      ${THESERVER}:/data/gpmgv/netcdf/geo_match | tee -a $LOG_FILE 2>&1

rsync -rtv --ignore-existing /home/data/gpmgv/prsubsets/ \
...
#            rsync -rtv --ignore-existing \
#                 /home/data/gpmgv/gv_radar/defaultQC_in/${site}/${area}/ \
#                 ${THESERVER}:/data/gpmgv/gv_radar/defaultQC_in/${site}/${area} \
#                 | tee -a $LOG_FILE 2>&1
    done
done
...
#            rsync -rtv --ignore-existing \
#                 /home/data/gpmgv/gv_radar/finalQC_in/${site}/${area}/ \
#                 ${THESERVER}:/data/gpmgv/gv_radar/finalQC_in/${site}/${area} \
#                 | tee -a $LOG_FILE 2>&1
    done
done

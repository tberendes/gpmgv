...
if [ ! -s ${IDL_PRO_DIR}/reorder1CUF_2km.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/reorder1CUF_2km.bat not found, exiting" \
       | tee -a $LOG_FILE
...
echo "" | tee -a $LOG_FILE
start1=`date -u`
echo "Calling IDL with reorder1CUF_2km.bat on $start1" | tee -a $LOG_FILE
$IDL < ${IDL_PRO_DIR}/reorder1CUF_2km.bat | tee -a $LOG_FILE 2>&1
echo "" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
end=`date -u`

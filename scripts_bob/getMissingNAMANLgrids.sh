...
LOG_FILE=${LOG_DIR}/getMissingNAMANLgrids.${rundate}.log
export LOG_FILE
...
DBTEMPFILE=${TMP_DIR}/getMisgNAMANLdata_dbtempfile
# list of cycle datetimes to retrieve, as URL/file specs
NAMTOGET=${TMP_DIR}/getMisgNAMANLdata_URLtempfile
if [ -f $NAMTOGET ]
  then
...
if [ $source = "namanl" ]
  then
   ...
   # will no longer be available from the nam archive
...
if [ -x $BIN_DIR/do2A5xMetadata.sh ]
  then
    echo $BIN_DIR/do2A5xMetadata.sh
  else
...
FIRSTORBIT=71451    # 2010-06-01 06Z cycle
...
# more likely to be met for a set of overpassed GR sites.  Excludes orbits whose matching 
# 2A-5x products have not been recieved yet.
...
echo "\t \a \o $DBTEMPFILE \\\select a.orbit, min(overpass_time at time zone 'UTC')\
 ...
 and a.orbit > $FIRSTORBIT and a.orbit<${DBOUT}\
 group by a.orbit\
 ...
 and a.orbit > $FIRSTORBIT and a.orbit<${DBOUT}\
 group by a.orbit order by 1;" \
  | psql gpmgv | tee -a $LOG_FILE 2>&1

# cat $DBTEMPFILE
# exit

# set the cutoff date for looking back to get missing nam grids
...
#    echo yyyymmpcp: $yyyymmpcp | tee -a $LOG_FILE
    ...
    file2check="namanl_218_${yyyymmdd}_${cycle}00_000"
    namfile="${yyyymm}/${yyyymmdd}/${file2check}"
    pcpfile2check="nam_218_${yyyymmddpcp}_${cyclepcp}00_006"
    namfilepcp="${yyyymmpcp}/${yyyymmddpcp}/${pcpfile2check}"
    ...
    echo ""
    # check whether we already have a version of ${}.grb
    ls ${GRIB_DATA}/${file2check}.grb* > /dev/null 2>&1
    if [ $? = 0  ]
      then
       # is it gzipped?
        ls ${GRIB_DATA}/${file2check}.grb* | grep '.gz' | grep -v grep > /dev/null 2>&1
        if [ $? = 0 ]
          then
            echo "Already have ${file2check}.grb.gz" | tee -a $LOG_FILE
          else
            echo "Already have ${file2check}.grb" | tee -a $LOG_FILE
        fi
      else
       # try to retrieve the grib file
    ...
    ls ${GRIB_DATA}/${pcpfile2check}.grb* > /dev/null 2>&1
    if [ $? = 0  ]
      then
       # is it gzipped?
        ls ${GRIB_DATA}/${pcpfile2check}.grb* | grep '.gz' | grep -v grep > /dev/null 2>&1
        if [ $? = 0 ]
          then
            echo "Already have ${pcpfile2check}.grb.gz" | tee -a $LOG_FILE
          else
            echo "Already have ${pcpfile2check}.grb" | tee -a $LOG_FILE
        fi
      else
       # check whether data date is more than 330 days ago; if so then the data
       # will no longer be available from the nam archive
        if [ $yyyymmddpcp -lt $lastyr_yyyymmdd ]
          then
            echo "Date too old, ${namfilepcp} not present, skipping." | tee -a $LOG_FILE
          else
           # try to retrieve the grib file
   ...
   # so we need to also get the 3-h projection with the 0-3h precip accumulation
    if [[ $cyclepcp = 06 || $cyclepcp = 18 ]]
      then
        ...
        pcpfile2check="nam_218_${yyyymmddpcp}_${cyclepcp}00_003"
        ls ${GRIB_DATA}/${pcpfile2check}.grb* > /dev/null 2>&1
        if [ $? = 0  ]
          then
           # is it gzipped?
...
# echo "See log file: $LOG_FILE"   # uncomment just for testing download set-up

...
###############################################################################
#
# doGeoMatchForL3test.sh    Morris/SAIC/GPM GV    Feb 2012
#
# Wrapper to do PR-GR NetCDF geometric matchups for 1C21/2A23/2A25/2B31/1CUF
...
#
# 2/28/2012   Morris         Created from doGeoMatch4NewRainCases.sh
#
###############################################################################
...
export PR_VERSION
PARAMETER_SET=1  # set of polar2pr parameters (polar2pr.bat file) in use
...
LOG_FILE=${LOG_DIR}/doGeoMatchForL3test.${rundate}.log
export rundate
...
for ncfile in `cat $DBCATALOGFILE`
  do
   ...
   # PR_VERSION=`echo ${ncfile} | cut -f5 -d '.'`
    ...
    rowpost="|${PR_VERSION}|${PARAMETER_SET}|${GEO_MATCH_VERSION}|1|${INSTRUMENT_ID}"
    gzfile=`ls ${ncfile}\.gz`
    if [ $? = 0 ]
      then
...
where pathname is null and c.radar_id not in ('RGSN','KMXX','RMOR','KWAJ','DARW') \
  and c.version=$PR_VERSION and a.orbit > $ncfilemaxorbit order by 1 limit 1;"`

# Step thru the dates, build an IDL control file for each date and run the matchups
   ...
   # for RGSN's mapping to two subsets. Morris, 12/2008
    ...
    DBOUT2=`psql -a -A -t -o $filelist  -d gpmgv -c "select file1c21, \
     ...
     where cast(nominal at time zone 'UTC' as date) = '${thisdate}' \
       ...
       and c.radar_id not in ('RGSN','KMXX','RMOR','KWAJ','DARW') \
   ...
   #  comes up more than once for a case.
	...
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select a.event_num, a.orbit, \
	    ...
	    and a.orbit = c.orbit and a.orbit = d.orbit and a.orbit = p.orbit \
            ...
            and a.radar_id not in ('RGSN','KMXX','RMOR','KWAJ','DARW') \

...
#LEFT OUTER JOIN geo_match_product g on (c.radar_id=g.radar_id and o.orbit=g.orbit and \
#   o.version=g.pps_version and g.instrument_id='${INSTRUMENT_ID}' and o.sat_id=g.sat_id) \
#JOIN rainy100inside100 r on (c.event_num=r.event_num) \
#WHERE pathname is null and o.version='${PPS_VERSION}' and o.sat_id='${SAT_ID}' order by 1;"`

DBOUT=`psql -a -A -t -o $datelist -d gpmgv -c "SELECT c.* into temp tempevents from eventsatsubrad_vw c \
JOIN orbit_subset_product o ON c.orbit = o.orbit AND c.subset = o.subset \
   AND c.sat_id = o.sat_id AND o.product_type = '${ALGORITHM}' and o.version='${PPS_VERSION}' and o.sat_id='${SAT_ID}' \
   and c.subset NOT IN ('KOREA','KORA') and c.nearest_distance<=${MAX_DIST} \
JOIN rainy100inside100 r on (c.event_num=r.event_num) order by c.overpass_time; \
select DISTINCT date(date_trunc('day', c.overpass_time at time zone 'UTC')) from tempevents c \
LEFT OUTER JOIN geo_match_product g on (c.radar_id=g.radar_id and c.orbit=g.orbit and \
   g.pps_version='${PPS_VERSION}' and g.instrument_id='${INSTRUMENT_ID}' and c.sat_id=g.sat_id) \
...
for thisdate in `cat $datelist`
  do
   ...
   # 'outfileall', which is run-date-specific.
    ...
    outfileall=${TMP_DIR}/DPR_files_sites4geoMatch_new.${yymmdd}.txt
...
||to_char(d.filedate,'MM')||'/'||to_char(d.filedate,'DD')||'/'||d.filename as file2a \
     ...
     select  c.orbit, count(*), c.datestamp, c.subset, c.version, c.instrument, c.swath, c.file2a\
       from possible_2adpr c left outer join geo_match_product b on (c.radar_id=b.radar_id \
        and c.orbit=b.orbit and c.version=b.pps_version and b.instrument_id = 'Ku' \
   ...
   #  comes up more than once for an orbit. NOTE THAT overpass_event AND
   #  collate_satsubprod_1cuf ARE REDUNDANT IN THE QUERY, ONLY NEED collate_satsubprod_1cuf
   #  NOW THAT WE INCLUDED nearest_distance AS A COLUMN IN THE VIEW

    for row in `cat $filelist | sed 's/ /_/'`
      do
        orbit=`echo $row | cut -f1 -d '|'`
        subset=`echo $row | cut -f4 -d '|'`
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select c.event_num, c.orbit, \
            c.radar_id, date_trunc('second', c.overpass_time at time zone 'UTC') as ovrptime, \
            extract(EPOCH from date_trunc('second', c.overpass_time)) as ovrpticks, \
            b.latitude, b.longitude, trunc(b.elevation/1000.,3) as elev, c.file1cuf, c.tdiff \
          into temp timediftmp
          from fixed_instrument_location b, rainy100inside100 r, collate_satsubprod_1cuf c \
          ...
          where c.radar_id = b.instrument_id   \
            and c.sat_id='$SAT_ID' and c.event_num=r.event_num\
            and c.orbit = ${orbit} and c.subset = '${subset}'
            and cast(c.overpass_time at time zone 'UTC' as date) = '${thisdate}'
            and c.product_type = '${ALGORITHM}' and c.nearest_distance <= ${MAX_DIST} \

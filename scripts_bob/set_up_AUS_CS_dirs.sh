...
# 08/05/16 - Modified to just do 2A DPR directories for new Aussie subsets
...
for sat in GPM/DPR/2ADPR GPM/Ku/2AKu #GPM/Ka/2AKa
  do
    cd $ORB
    #mkdir -p $sat
    cd $sat
    for version in V04A
      do
        #mkdir -p $version
        cd $version
        for subset in  AUS-East AUS-West Tasmania
          do
            mkdir -p ${subset}/${YYYY}

...
export PPS_VERSION
PARAMETER_SET=99  # set of polar2pr parameters (polar2pr.bat file) in use
...
if [ -s $loadfile ]
  then
   # load the rows to the database  DISABLED !!!!
    ...
    if [ -s $outfileall ]
      then
       ...
       # has been done before.
        ...
        echo "" | tee -a $LOG_FILE
        ${BIN_DIR}/do_DPR_geo_matchup4dateAllPts.sh -f $FORCE_MATCH $yymmdd $outfileall

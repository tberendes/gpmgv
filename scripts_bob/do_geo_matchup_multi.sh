...
################################################################################
#
# do_geo_matchup_multi.sh    Morris/SAIC/GPM GV    September 2010
#
# DESCRIPTION
#   Called by doGeoMatch_KMLB_multiVols.sh, which generates the needed inputs.
...
#   polar2pr_multi_vol.bat (INTERNAL) - Batch file of IDL commands to be run.
#
#  DATABASE
#    Status of this script's runs are maintained in the 'appstatus' table,
#    in rows tagged with the app_id attribute value 'geo_match_multi'.
#
#  LOGS
...
export IDL_PRO_DIR                 # IDL polar2pr_multi_vol.bat needs this
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'geo_match_multi' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('geo_match_multi', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'geo_match_multi' AND status = '$UNTRIED' \
...
if [ ! -s ${IDL_PRO_DIR}/polar2pr_multi_vol.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/polar2pr_multi_vol.bat not found, exiting" \
       | tee -a $LOG_FILE
...
#	export RUNDATE              # IDL polar2pr.bat needs this
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'geo_match_multi' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
	...
	$IDL < ${IDL_PRO_DIR}/polar2pr_multi_vol.bat | tee -a $LOG_FILE 2>&1

...
################################################################################
#
# get2A55Meta.sh    Morris/SAIC/GPM GV    October 2006
#
# DESCRIPTION
#   Runs IDL in batch mode to extract metadata from 2A55 data files
#   for NEXRAD sites overpassed in each day.  Files and overpassed
...
#
# FILES
#   file2a55sites.YYMMDD.txt (INPUT) - text file listing 2A55 files and
#                                      NEXRAD sites overpassed in each. 
...
#   get2A55MetaIDL.bat (INTERNAL)    - Batch file of IDL commands to be run.
#   2A55_VOLUMES.YYMMDD.unl (OUTPUT) - ASCII-delimited metadata file generated
#                                      by IDL routines, ready to load to
#                                      database.
#   Meta2A55_dbtempfile (INTERNAL)   - re-used file to temporarily hold output
#                                      from database queries
#   Meta2A55_todo.txt (INTERNAL)     - re-used file to temporarily hold list of
#                                      all YYMMDDs to process in this run
#
#  DATABASE
#    Catalogs 2A55 file NEXRAD volume scan times in 'gvradarvolume' table in
...
#
#  LOGS
#    Output for script run logged to daily log file get2A55Meta.YYMMDD.log
...
#    - User must have write privileges in $TMP_DIR, $META_LOG_DIR directories
#
################################################################################
...
IDL_PRO_DIR=${GV_BASE_DIR}/idl/valnet/getmetadata  # change for operational
export IDL_PRO_DIR                 # IDL get2A55MetaIDL.bat needs this
...
DBTEMPFILE=${TMP_DIR}/Meta2A55_dbtempfile
# re-used file listing all yymmdd to be processed this run
META2A55=${TMP_DIR}/Meta2A55_todo.txt
rm -f $META2A55 $DBTEMPFILE
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${META_LOG_DIR}/get2A55Meta.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2A55Meta.${THISRUN}.log
echo "Processing 2A55 metadata for rundate ${THISRUN}" | tee  $LOG_FILE
echo "+++++++++++++++++++++++++++++++++++++++++++" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'get2A55Meta' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('get2A55Meta', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'get2A55Meta' AND status = '$UNTRIED' \
...
if [ ! -s ${IDL_PRO_DIR}/get2A55MetaIDL.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/get2A55MetaIDL.bat not found, exiting" \
       | tee -a $LOG_FILE
     echo "with status 'UNTRIED', no metadata processed for ${THISRUN}." \
       | tee -a $LOG_FILE
     exit 1
...
for yymmdd in `cat $META2A55`
  do
    GETMYMETA=${DATA_DIR}/tmp/file2a55sites.${yymmdd}.txt
    if [ -s $GETMYMETA ]
      then
        RUNDATE=$yymmdd
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'get2A55Meta' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
	...
	$IDL < ${IDL_PRO_DIR}/get2A55MetaIDL.bat | tee -a $LOG_FILE 2>&1
	
	echo "=============================================" | tee -a $LOG_FILE
	
	# check the IDL output file before declaring success
	idl_out=${TMP_DIR}/2A55_VOLUMES.${yymmdd}.unl
	if [ -s $idl_out ]
	  then
	    # load metadata to database and check status of load script
	    ${BIN_DIR}/load2A55Metadata.sh $idl_out | tee -a $LOG_FILE 2>&1
	    if [ $? ]
	      then
...
echo "" | tee -a $LOG_FILE
echo "get2A55Meta.sh complete, exiting." | tee -a $LOG_FILE
exit 0

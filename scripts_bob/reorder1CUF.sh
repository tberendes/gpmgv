...
###############################################################################
#
# reorder1CUF.sh    Morris/SAIC/GPM GV    March 2007
#
...
echo "\t \a \f '|' \o $REO_1CUF_LIST \\\ set datestyle to postgres, DMY;
 ...
 and a.orbit > 58525 and a.radar_id not in ('DARW','RMOR');" \
...
echo "" | tee -a $LOG_FILE
start1=`date -u`
echo "Calling IDL with reorder1CUF.bat on $start1" | tee -a $LOG_FILE
$IDL < ${IDL_PRO_DIR}/reorder1CUF.bat | tee -a $LOG_FILE 2>&1
echo "" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
end=`date -u`

...
#
# Wrapper to do NetCDF grid creation for 1C21/2A25/2A55/2A54/2A53 files already
...
echo "\t \a \f '|' \o $datelist \
  ...
  (select distinct orbit from ovlp25_w_rain25 where orbit > 50100) \
  order by filedate;"\
...
echo "\t \a \f '|' \o $filelist \
     ...
     from collatedZproducts a, ovlp25_w_rain25 b\

...
echo "\t \a \f '|' \o $datelist \
  ...
  (select distinct orbit from rainy100inside100 where radar_id = '${RADARID}' and orbit < 2000) \
...
echo "\t \a \f '|' \o $filelist \
     ...
     a.orbit, count(*), '${yymmdd}', subset, version \
     ...
     and a.radar_id = '${RADARID}' and a.subset='${SUBSETID}' and a.version=${VERSIONID} \
     group by file1c21, file2a25, file2b31, a.orbit, subset, version \
...
#                             the same radar_id comes up more than once for a case.
	...
	echo "\t \a \f '|' \o $outfile \\\ select a.event_num, a.orbit, \
            ...
            trunc(b.elevation/1000.,3), COALESCE(c.file1c51, 'no_1CUF_file') \
	  ...
	  collatedGVproducts c \
	    ...
	    and a.orbit = c.orbit and a.orbit = ${orbit} and c.subset = '${subset}'
          order by 3;" \

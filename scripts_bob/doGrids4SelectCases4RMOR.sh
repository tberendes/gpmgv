...
###############################################################################
#
# doGrids4SelectCases_w_2A53-54_2B31.sh    Morris/SAIC/GPM GV    August 2007
#
...
echo "\t \a \f '|' \o $datelist \
  ...
  and b.product_type = '2A25' and a.subset = b.subset and a.subset='sub-GPMGV1'\
  and a.orbit in \
  (select distinct orbit from ovlp25_w_rain25 where radar_id='RMOR' and orbit>59624) \
  order by filedate;"\
...
echo "\t \a \f '|' \o $filelist \
     ...
     a.orbit, count(*) \
     from collatedPRproductswsub a, ovlp25_w_rain25 b\
     where cast(nominal at time zone 'UTC' as date) = '${thisdate}'\
     and a.orbit = b.orbit and a.radar_id = b.radar_id and a.radar_id='RMOR'\
...
# - Prepare the control file for IDL to do PR and GV grid file creation.
	...
	echo "\t \a \f '|' \o $outfile \
	  ...
	  collatedGVproducts c, ovlp25_w_rain25 d \
          where a.radar_id = b.instrument_id and a.radar_id = c.radar_id \
	  and a.radar_id = d.radar_id and a.radar_id='RMOR' \
	  and a.orbit = c.orbit  and a.orbit = d.orbit and a.orbit = ${orbit} \
          LIMIT 1;" | psql gpmgv | tee -a $LOG_FILE 2>&1

...
################################################################################
#
# get2A23-25Meta_wSub.sh    Morris/SAIC/GPM GV    October 2006
#
# DESCRIPTION
...
#
# 2/11/08 - Morris - Modified the query which produces the control file info to
...
#   NONZERO-FILE-EXISTS TEST BELOW WOULD NORMALLY SUCCEED!
    ...
    for mirdate in `grep 2A23 $MIR_LOG_FILE | cut -f2 -d '.' | sort -u`
      do
        ...
        echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT * FROM appstatus \
        WHERE app_id = 'wgetCTdaily' AND datestamp = '$mirdate';" \
...
#	fi
   ...
   #
    ...
    echo "Generating orbit/file/site overpass control file for ${yymmdd}."\
     | tee -a $LOG_FILE
   #
   # Modified this query in Feb '08 to take PR product subset into account
   # when determining overpassed radars for the product/orbit.
    for file2a23 in `grep 2A23 $MIR_LOG_FILE | cut -f2 -d '/' | cut -f1 -d ' '`
      do
        echo ""  | tee -a $LOG_FILE
        echo "\t \a \f '|' \o $outfile \
         \\\ SELECT file2a23, file2a25, orbit, count(*) \
          FROM collatedprproductswsub where file2a23 =  '${file2a23}'\
          GROUP BY file2a23, file2a25, orbit;\
        SELECT a.event_num, a.radar_id, b.latitude, b.longitude\
          FROM overpass_event a, fixed_instrument_location b,\
               siteproductsubset c, orbit_subset_product d\
         WHERE a.radar_id = b.instrument_id AND a.radar_id = c.radar_id\
           AND a.orbit = d.orbit AND c.subset = d.subset \
           AND d.filename='${file2a23}';" | psql gpmgv  | tee -a $LOG_FILE 2>&1
       #

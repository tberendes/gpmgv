...
# Passes along a single argument, a 'yymmdd' date string, to the child
# script.  (Note that success in processing metadata for a set of DPR files
...
INSTRUMENT=GMI #Ku   # 2A data type to process for metadata.  Other option is 'DPR'
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT * FROM appstatus \
      WHERE app_id = 'get2ADPRMeta' AND status = '$MISSING' \
...
#   which is run-date-specific:
    outfile=${DATA_DIR}/tmp/file2aDPRsites_temp.txt
    outfileall=${DATA_DIR}/tmp/file2aDPRsites.${yymmdd}.txt
    rm -f $outfileall
...
# file to hold the 2ADPR pathnames extracted from the get_PPS_CS_data log file
    filepaths2ADPR=${DATA_DIR}/tmp/filepaths2ADPR_temp.txt
...
# find the pathnames of the 2ADPR or 2AKu files listed as downloaded in this log file
    ...
    grep 2A $MIR_LOG_FILE | grep $INSTRUMENT | grep orbit_subset | grep -v INSERT \
           ...
           echo "\t \a \f '|' \o $outfile \
            \\\ SELECT '${thisPPSfilepath}', b.orbit, count(*) \
             FROM overpass_event a, orbit_subset_product b, \
                  siteproductsubset d \
            WHERE a.orbit = b.orbit AND b.filename = '${file2aDPR}' \
              AND b.subset = d.subset \
              AND a.radar_id = d.radar_id AND d.sat_id='GPM' \
         GROUP BY b.filename, b.orbit; \
           SELECT a.event_num, a.radar_id, b.latitude, b.longitude \
            ...
            WHERE a.radar_id = b.instrument_id and a.radar_id = d.radar_id \
              AND a.orbit = c.orbit and c.subset = d.subset and d.sat_id='GPM' \
    ...
    if [ -s $outfileall -a $CTMISSING != "t" ]
      then
        ...
        # CHECK THE STATUS OF THE ABOVE SCRIPT BEFORE BLINDLY DECLARING SUCCESS
	     if [ $? ]
	       then
	         ...
	         echo "See log file ${LOG_DIR}/get2ADPRMeta4date${yymmdd}.log" \
              | tee -a $LOG_FILE
            exit 1
          else
            echo "" | tee -a $LOG_FILE
        ...
        echo "Mark $yymmdd as incomplete -- no coincidence data for orbit(s)," \
	          | tee -a $LOG_FILE
        echo "or no 2A${INSTRUMENT} subset data files listed in ${MIR_LOG_FILE}, so" \
	          | tee -a $LOG_FILE

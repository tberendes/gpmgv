...
#
# FILES
#   file2a23sites.YYMMDD.txt (INPUT) - text file listing 2A23 files, orbits, and
...
THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2A23Meta.${THISRUN}.log
echo "Processing 2A23 metadata for rundate ${THISRUN}" | tee -a $LOG_FILE
echo "+++++++++++++++++++++++++++++++++++++++++++" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'get2A23Meta' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('get2A23Meta', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'get2A23Meta' AND status = '$UNTRIED' \
...
for yymmdd in `cat $META2A23`
  do
    GETMYMETA=${DATA_DIR}/tmp/file2a23sites.${yymmdd}.txt
    if [ -s $GETMYMETA ]
      then
        RUNDATE=$yymmdd
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'get2A23Meta' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
	echo "Calling IDL for yymmdd = ${yymmdd}, file = $GETMYMETA" \
	 | tee -a $LOG_FILE
	echo "" | tee -a $LOG_FILE
        cat ${IDL_PRO_DIR}/get2A23MetaIDL.bat
...
echo "" | tee -a $LOG_FILE
echo "get2A23Meta.sh complete, exiting." | tee -a $LOG_FILE
exit 0

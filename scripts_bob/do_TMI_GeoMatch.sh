...
#                           do_DPR_GeoMatch.sh.
# 10/31/2016  Morris        Cleanup, and removed optional arguments for SAT_ID
...
LOG_FILE=${LOG_DIR}/doTMIGeoMatch4NewRainCases.V${PPS_VERSION}.${rundate}.log
...
from eventsatsubrad_vw c JOIN orbit_subset_product o \
  ON c.orbit = o.orbit AND c.subset = o.subset AND c.sat_id = 'PR' \
 AND o.product_type = '${ALGORITHM}' and o.version='${PPS_VERSION}' and o.sat_id='TRMM' \
...
into temp possible_2agprof \
     ...
     JOIN orbit_subset_product d ON c.sat_id='PR' and c.orbit = d.orbit\
        AND c.subset = d.subset AND d.sat_id='TRMM' and c.subset NOT IN ('KOREA','KORA') \
...
#        echo "${orbit}, $subset, ${INSTRUMENT_ID}, $PPS_VERSION, ${thisdate}"
	...
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select a.event_num, a.orbit, \
	    ...
	    collate_satsubprod_1cuf_TMIGPROF c \
            left outer join geo_match_product e on \
              (c.radar_id=e.radar_id and c.orbit=e.orbit and \
               ...
               and e.parameter_set=${PARAMETER_SET} and e.sat_id='PR') \
          where a.radar_id = b.instrument_id and a.radar_id = c.radar_id  \
            and a.orbit = c.orbit and a.event_num=r.event_num \

...
#
# Computes missing rain event metadata from 2AKu or 2ADPR data for the DARW
...
PPS_VERSION="V03B"        # controls which DPR products we process
export PPS_VERSION
INSTRUMENT_ID="Ku"
...
||to_char(d.filedate,'DD')||'/'||d.filename, d.orbit, c.radar_id, count(*)\
     ...
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
        AND c.subset = d.subset AND c.sat_id='$SAT_ID' and c.subset IN ('DARW') \
...
# - Prepare the control file for IDL to do 2AKu file metadata extraction
#echo "Filelist:"
#head $filelist
#echo ""
#tail $filelist

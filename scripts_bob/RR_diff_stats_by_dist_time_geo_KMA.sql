...
create table rrdiff_stats_by_dist_time_geo_kma (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
...
create table rrdiff_stats_by_dist_time_geo_kma_s2ku (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
delete from rrdiff_stats_by_dist_time_geo_kma;
\copy rrdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/tmp/RR_StatsByDistToDBGeo_Pct90_ZR3_all4rad_DefaultS.unl' with delimiter '|'
\copy rrdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/tmp/RR_StatsByDistToDBGeo_Pct90_ZR3_Zadj_RGSN_DefaultS.unl' with delimiter '|'
\copy rrdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/tmp/RR_StatsByDistToDBGeo_Pct90_ZR3_Zadj_RJNI_DefaultS.unl' with delimiter '|'
\copy rrdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/tmp/RR_StatsByDistToDBGeo_Pct90_ZR3_Zadj_RPSN_DefaultS.unl' with delimiter '|'
\copy rrdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/tmp/RR_StatsByDistToDBGeo_Pct90_ZR3_Zadj_RSSP_DefaultS.unl' with delimiter '|'
\copy rrdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/tmp/RR_StatsByDistToDBGeo_Pct90_ZR3_GRZadjByPR_DefaultS.unl' with delimiter '|'

delete from rrdiff_stats_by_dist_time_geo_kma_s2ku;
--\copy rrdiff_stats_by_dist_time_geo_kma_s2ku from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_S2Ku.unl' with delimiter '|' 




-- differences and profiles by height/percent:
select height, percent_of_bins, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanprv, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangr, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'C_%' and numpts>5 and radar_id='KMLB' group by 1,2 order by 1,2;
select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totalany into temp anytotal from rrdiff_stats_by_dist_time_geo_kma where regime='Total' group by 1,2,3,4,5,6;


select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totaltypes into temp totalbytypes from rrdiff_stats_by_dist_time_geo_kma where regime!='Total' group by 1,2,3,4,5,6;
...
select radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 and percent_of_bins=100 group by 1,2 order by 1,2;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinmax/10)*10+5 as grmaxz, regime, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'S%' and numpts>0 group by 3,2 order by 1;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinstddev/3)*3 as stddev, regime, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'S%' and numpts>5 group by 3,2 order by 3,2;
...
select radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 group by 1 order by 1;

-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, rangecat*50+25 as mean_range_km, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 and percent_of_bins=100 group by 1,2 order by 1,2;

-- Bias by site and gv source, for given regime(s), at 1.5km GR CAPPI level

select radar_id, gvtype||'-Conv' as zr_type,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as rr_diff, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'C_%' and numpts>5 and height=1.5 group by 1,2 order by 1,2;
...
select radar_id, gvtype||'-Strat' as zr_type,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as rr_diff, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'S_%' and numpts>5 and height=1.5 group by 1,2 order by 1,2;
...
select radar_id, gvtype||'-Strat', round((timediff+30)/60), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'S_%' and numpts>5 and height=1.5 group by 1,2,3 order by 1,2,3;
...
(86 rows)
select radar_id, gvtype||'-Conv', round((timediff+30)/60), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime like 'C_%' and numpts>5 and height=1.5 group by 1,2,3 order by 1,2,3;
 radar_id | ?column?  | round | meandiffavg | total 
----------+-----------+-------+-------------+-------
...
select gvtype, radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanpr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangv, sum(numpts) as total from rrdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
 ...
 into temp dbzsums from rrdiff_stats_by_dist_time_geo_kma where meandiff > -99.9
...
select percent_of_bins, radar_id, orbit, sum(numpts) as n_conv into temp npts_conv from rrdiff_stats_by_dist_time_geo_kma where regime like 'C_%' group by 1,2,3 order by 1,2,3;
select percent_of_bins, radar_id, orbit, sum(numpts) as n_strat into temp npts_strat from rrdiff_stats_by_dist_time_geo_kma where regime like 'S_%' group by 1,2,3 order by 1,2,3;
...
select percent_of_bins, radar_id, orbit, ((sum(numpts)/250)+1)*250 as n_conv into temp area_conv from rrdiff_stats_by_dist_time_geo_kma where regime like 'C_%' and height=3 group by 1,2,3 order by 1,2,3;
select percent_of_bins, radar_id, orbit, ((sum(numpts)/250)+1)*250 as n_strat into temp area_strat from rrdiff_stats_by_dist_time_geo_kma where regime like 'S_%' and height=3 group by 1,2,3 order by 1,2,3;
-- compute mean bias, stdDev as function of area of precip for KMLB
select e.n_strat, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from rrdiff_stats_by_dist_time_geo_kma c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%' and c.radar_id='KMLB' group by 1 order by 1;
...
select e.n_strat, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from rrdiff_stats_by_dist_time_geo_kma c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%'  group by 1 order by 1;
 n_strat | biasgeo | stdev |  ngeo 
---------+---------+-------+--------
...
-- as above, site by site
 select e.n_strat, e.radar_id, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from rrdiff_stats_by_dist_time_geo_kma c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%' group by 1,2 order by 2,1;
...
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsgeo from rrdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 and gvtype='GeoM' and rangecat<2 and percent_of_bins=100 group by 1,2 order by 1,2;

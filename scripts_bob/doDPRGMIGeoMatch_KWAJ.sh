...
###############################################################################
#
# doDPRGMIGeoMatch4RainDates.sh    Morris/SAIC/GPM GV    June 2014
#
# DESCRIPTION:
# Query gpmgv database for dates/times of rainy DPR events and assemble
# command string to do the DPRGMI-GR geometry matching for events with data.
...
while getopts s:i:v:p:a:d:m: option
  do
    case "${option}"
      in
        s) SAT_ID=${OPTARG};;
...
select DISTINCT date(date_trunc('day', c.overpass_time at time zone 'UTC')) \
  from tempevents c LEFT OUTER JOIN geo_match_product g \
   ...
   and g.PARAMETER_SET=${PARAMETER_SET} and g.geo_match_version=${GEO_MATCH_VERSION} order by 1 ;"`
...
while read thisdate
  do
   ...
   # 'outfileall', which is run-date-specific.
    ...
    outfileall=${TMP_DIR}/COMB_files_sites4geoMatch.${yymmdd}.txt
...
AND C.RADAR_ID IN ('KWAJ') \
     ...
     select  c.orbit, count(*), c.datestamp, c.subset, c.version, c.instrument, c.file2a \
       from possible_2agprof c left outer join geo_match_product b on (c.radar_id=b.radar_id \
        ...
        and b.parameter_set=0 and b.sat_id='GPM')  \
   group by 1,3,4,5,6,7 order by c.orbit;"`  | tee -a $LOG_FILE 2>&1

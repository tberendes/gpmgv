...
LOG_FILE=${META_LOG_DIR}/doAllMetadataKHTX.${rundate}.log
export rundate
...
echo "\t \a \f '|' \o $datelist \
     ...
     orbit_subset_product b where a.orbit=b.orbit and a.product_type='2A23'\
     and b.product_type = '2A25' and a.filedate < '2006-12-21' order by filedate;" | psql gpmgv \
      | tee -a $LOG_FILE 2>&1
...
outfileall=${DATA_DIR}/tmp/file2a23_KHTX.${yymmdd}.txt
...
echo "\t \a \f '|' \o $filelist \
     ...
     where a.orbit = b.orbit and a.product_type = '2A23'\
       ...
       and a.filedate = '${thisdate}' and c.radar_id = 'KHTX'\
...
# - Prepare the control file for IDL to do 2A23/2A25 file metadata extraction.
	...
	echo "\t \a \f '|' \o $outfile \
          ...
          where a.radar_id = b.instrument_id and a.radar_id = 'KHTX'\
...
if [ -s $outfileall ]
  then
    ...
    ${BIN_DIR}/get2A23MetaKHTX.sh $yymmdd
    if [ $? ]
      then
    ...
    ${BIN_DIR}/get2A25MetaKHTX.sh $yymmdd
    if [ $? ]
      then

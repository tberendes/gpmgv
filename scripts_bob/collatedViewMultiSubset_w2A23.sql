...
select a.*, g.filename as file2A53, 
       b.filename as file2A54, c.filename as file2A55,
       d.filename as file1C21, e.filename as file2A23, 
       f.filename as file2A25, h.filename as file2B31
  from collatecolswsub a 
    left outer join gvradar g on (a.nominal=g.nominal
      and a.radar_id=g.radar_id)
      and g.product = '2A53'
    ...
    left outer join orbit_subset_product h on (a.orbit=h.orbit
      and a.subset = h.subset)
      and h.product_type = '2B31';
...
grant select on collatedPRproductswsub to public;
...
select a.*, a.radar_id||'/'||b.filepath||'/'||b.filename as file2A55,
  ...
  from collatecolswsub a 
    ...
    left outer join gvradar c
      ...
      and c.product = '2A54'
    left outer join gvradar f
      on a.nominal=f.nominal and a.radar_id=f.radar_id
      and f.product = '2A53'

...
RADARID=KWAJ
SUBSETID="sub-GPMGV1"
VERSIONID=225   # make sure to edit NC_NAME_ADD in polar2pr.bat accordingly
...
echo "\t \a \f '|' \o $datelist \
  ...
  from overpass_event where  \
    date( date_trunc('day', overpass_time at time zone 'UTC') ) between \
   '2009-01-01' and '2009-12-31' order by 1;" | psql gpmgv | tee -a $LOG_FILE 2>&1
...
echo "\t \a \f '|' \o $filelist \
     ...
     and a.orbit = b.orbit and a.radar_id = b.radar_id and a.subset = '${SUBSETID}' \
     and file1c21 is not null and file1c21 like '%ITE_225%' \
...
if [ -s $outfileall ]
  then
    # Call the IDL wrapper script, do_geo_matchup_w2A23.sh, to run

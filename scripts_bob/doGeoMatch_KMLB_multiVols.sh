...
RADARID=KMLB
SUBSETID=sub-GPMGV1
VERSIONID=167   # make sure to edit NC_NAME_ADD in polar2pr.bat accordingly
...
echo "\t \a \f '|' \o $datelist \
  ...
  (select distinct orbit from rainy100inside100 where radar_id = '${RADARID}' \
   and date( date_trunc('year', overpass_time at time zone 'UTC') ) in \
   ('2007-01-01','2008-01-01') ) order by 1;" | psql gpmgv | tee -a $LOG_FILE 2>&1
...
if [ -s $outfileall ]
  then
    ...
    echo "" | tee -a $LOG_FILE
    start1=`date -u`
    echo "Calling do_geo_matchup_multi.sh $yymmdd on $start1" | tee -a $LOG_FILE
    ${BIN_DIR}/do_geo_matchup_multi.sh $yymmdd

    if [ $? = 0 ]
      then
        echo ""
        echo "SUCCESS status returned from do_geo_matchup_multi.sh"\
	 | tee -a $LOG_FILE
      else
        echo ""
        echo "FAILURE status returned from do_geo_matchup_multi.sh, quitting!"\
	 | tee -a $LOG_FILE
#	exit 1
    fi

...
###############################################################################
#
# doGrids4SelectCases4RGSN.sh    Morris/SAIC/GPM GV    July 2008
#
...
echo "\t \a \f '|' \o $datelist \
  ...
  (select distinct orbit from ovlp25_w_rain25 where radar_id='RGSN') \
  order by 1;"\
...
echo "\t \a \f '|' \o $filelist \
     ...
     and a.orbit = b.orbit and a.radar_id = b.radar_id and a.radar_id='RGSN'\
...
# - Prepare the control file for IDL to do PR and GV grid file creation.
	...
	echo "\t \a \f '|' \o $outfile \
	  ...
	  and a.radar_id = d.radar_id and a.radar_id='RGSN' \
...
if [ -s $outfileall ]
  then
    ...
    echo "${BIN_DIR}/grid_1C21_2A25_2A55.sh $yymmdd"

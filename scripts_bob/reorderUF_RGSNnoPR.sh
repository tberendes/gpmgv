...
###############################################################################
#
# reorderUF_RGSNnoPR.sh    Morris/SAIC/GPM GV    June 2008
#
# Wrapper to do grid generation from Gosan, Korea (RGSN) 1CUF files.
#
###############################################################################
...
export IDL_PRO_DIR                 # IDL reorderUF_multi.bat needs this
...
# ONE TIME QUERY WITH TABLE rgsn2do TO RE-RUN FOR NETCDFs WITH WRONG DATE STAMPS

echo "\t \a \f '|' \o $REO_1CUF_LIST \\\ set datestyle to postgres, DMY;
 select c.event_num, c.orbit, a.radar_id, a.nominal at time zone 'UTC', b.latitude, b.longitude, trunc(b.elevation/1000.,3),a.radar_id||'/'||filepath||'/'||filename  from gvradar a,  fixed_instrument_location b, overpass_event c, rgsn2do d where a.radar_id = b.instrument_id and a.radar_id = 'RGSN' and a.radar_id=c.radar_id and a.nominal = date_trunc('hour',c.overpass_time) and c.orbit=d.orbit;" \

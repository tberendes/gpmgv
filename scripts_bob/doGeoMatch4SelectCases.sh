...
# set "Limit 1" for Level 3 Testing
...
echo "\t \a \f '|' \o $datelist \
  ...
  (select distinct orbit from rainy100inside100 where orbit > $ncfilemaxorbit \
   and radar_id not in ('DARW','KMXX','RGSN','RMOR','KWAJ')) \
  order by 1 limit 1;" | psql gpmgv | tee -a $LOG_FILE 2>&1
...
echo "\t \a \f '|' \o $filelist \
     ...
     and a.radar_id not in ('DARW','KMXX','RGSN','RMOR','KWAJ') \

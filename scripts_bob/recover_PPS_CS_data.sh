...
################################################################################
#
#  recover_PPS_CS_data.sh     Morris/SAIC/GPM GV     February 2014
#
#  DESCRIPTION
...
#
#    as listed in PREVIOUSLY RETRIEVED daily ftp_url files posted there by the
#    PPS.  Filters out CS file types not of interest to GPM Validation.  Takes
#    the yymmdd datestamp of a previous run of get_PPS_CS_data.sh and
#    getftpListings.sh as the single input parameter, and retrieves PPS products
#    from the ftp_url_YYYYMMDD.txt files already downloaded by getftpListings.sh.
#
#  ROUTINES CALLED
#
#  FILES
...
#    replace_PPS_CS_data.YYYYMMDD.log in data/logs subdirectory.
#
#  CONSTRAINTS
...
LOG_FILE=${LOG_DIR}/replace_PPS_CS_data.${rundate}.log       # script log file
export LOG_FILE
...
rm -f $FILES2DO
URLFILES2DO=${TMP_CS_DATA}/URLFILESToDo
rm -f $URLFILES2DO
                           ...
                           then
                             ...
                             if [ ! -s $targetDir1 ]
                               then
                                 ...
                                 mkdir -p -v $targetDir
                                # see if we already have file, and move or delete accordingly
...
echo "===================================================" | tee $LOG_FILE
echo " Attempting download of PPS CS files from $today." | tee -a $LOG_FILE
echo "---------------------------------------------------" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
if [ ${pgproccount} -lt 3 ]
  then
    thistime=`date -u`
    echo "Message from replace_PPS_CS_data.sh cron job on ${thistime}:" \
...
if [ ${status} != $DUPLICATE ]
  ...
  echo "We've tried to get this day's files before"\
   | tee -a $LOG_FILE
  echo "UPDATE appstatus SET status='$DUPLICATE' WHERE app_id = 'get_PPS_CS' \
    AND datestamp = '$rundate';" \
...
if [ -s $FILES2DO ]
  then
    echo "Relocating PPS ftp_url_YYYYMMDDhhmm.txt files:" | tee -a $LOG_FILE
...
#            URLFILES=`grep Got ${LOG_DIR}/PPSftpListings.${rundate}.log | cut -f2 -d ' '`
            grep Got ${LOG_DIR}/PPSftpListings.${rundate}.log | cut -f2 -d ' ' > URLFILES2DO
        fi
      else
...
cat $URLFILES2DO
    ...
    for fdate in `cat URLFILES2DO`   #`echo $URLFILES`
      do

...
###############################################################################
#
# do_AnySite_AnyGPROF_Metadata.sh    Morris/SAIC/GPM GV    May 2016
#
# Computes missing rain event metadata from 2AGPROF data for a user-selected
...
while getopts s:i:v:a:g: option
  do
    case "${option}"
      in
        s) SAT_ID=${OPTARG};;
...
||to_char(d.filedate,'DD')||'/'||d.filename, d.orbit, c.radar_id, count(*)\
     ...
     JOIN orbit_subset_product d ON c.sat_id=d.sat_id and c.orbit = d.orbit\
       ...
       and 11 > (select count(*) from event_meta_numeric_copy \
    ...
    if [ -s $outfileall ]
      then
       ...
       #   so we need to remove all values with this tag from appstatus table.
        echo "delete from appstatus where app_id = 'getGPROFMeta4dy' and datestamp = '${yymmdd}';" \
          | psql -a -d gpmgv | tee -a $LOG_FILE 2>&1

        echo "Calling get2AGPROFMeta4date.sh $yymmdd on $start1" | tee -a $LOG_FILE
        ${BIN_DIR}/get2AGPROFMeta4date.sh $yymmdd

        # CHECK THE STATUS OF THE ABOVE SCRIPT BEFORE BLINDLY DECLARING SUCCESS
	     if [ $? = 1 ]
	       then
	         echo "UPDATE appstatus SET status = '$MISSING' \
	           WHERE app_id = 'getGPROFMeta' AND datestamp = '$yymmdd';" \
	           | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
            ...
            echo "UPDATE appstatus SET status = '$SUCCESS' WHERE \
             app_id = 'getGPROFMeta' AND datestamp = '$yymmdd';"\
              | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	     fi
      else
        echo "" | tee -a $LOG_FILE

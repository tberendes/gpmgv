...
\copy zdiff_stats_by_dist_time_geov7 from '/data/gpmgv/tmp/StatsByDistTimeToDBGeo_Pct100_v7_App_DefaultS.unl' with delimiter '|' 
\copy zdiff_stats_by_dist_time_geo_s2kuv7 from '/data/gpmgv/tmp/StatsByDistTimeToDBGeo_Pct100_v7_App_S2Ku.unl' with delimiter '|'

-- CREATE TABLES FOR THE DATA WITH GR MIN THRESHOLD OF 18 DBZ

create table zdiff_stats_by_dist_time_geo18 (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
create table zdiff_stats_by_dist_time_geo_s2ku18 (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
delete from zdiff_stats_by_dist_time_geo18;
\copy zdiff_stats_by_dist_time_geo18 from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_dbz18_DefaultS.unl' with delimiter '|' 
delete from zdiff_stats_by_dist_time_geo_s2ku18;
\copy zdiff_stats_by_dist_time_geo_s2ku18 from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_dbz18_S2Ku.unl' with delimiter '|' 
...
select radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geov7 where regime='S_above' and numpts>5 group by 1 order by 1;
...
-- "Best" bias regime (stratiform above BB), broken out by site only, for pre/post-dual-pol period:
select a.radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiff_pre, sum(numpts) as total_pre into temp diffspre from zdiff_stats_by_dist_time_geov7 a, dualpol_active b, overpass_event c where regime='S_above' and numpts>5 and a.radar_id=b.radar_id and a.radar_id=c.radar_id and a.orbit=c.orbit and date(c.overpass_time at time zone 'UTC') < b.activation group by 1 order by 1;

select a.radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiff_post, sum(numpts) as total_post into temp diffspost from zdiff_stats_by_dist_time_geov7 a, dualpol_active b, overpass_event c where regime='S_above' and numpts>5 and a.radar_id=b.radar_id and a.radar_id=c.radar_id and a.orbit=c.orbit and date(c.overpass_time at time zone 'UTC') >= b.activation group by 1 order by 1;

\H \o /data/gpmgv/tmp/GR_PR_Bias_unadj.html \\select a.radar_id||' (unadj.)' as radar_id, a.meandiff_pre*(-1) as gr_pr_diff_pre, a.total_pre, b.meandiff_post*(-1) as gr_pr_diff_post, b.total_post from diffspre a, diffspost b where a.radar_id=b.radar_id;

-- as above, but S-to-KU adjustements
select a.radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiff_pre, sum(numpts) as total_pre into temp diffspres2ku from zdiff_stats_by_dist_time_geo_s2kuv7 a, dualpol_active b, overpass_event c where regime='S_above' and numpts>5 and a.radar_id=b.radar_id and a.radar_id=c.radar_id and a.orbit=c.orbit and date(c.overpass_time at time zone 'UTC') < b.activation group by 1 order by 1;

select a.radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiff_post, sum(numpts) as total_post into temp diffsposts2ku from zdiff_stats_by_dist_time_geo_s2kuv7 a, dualpol_active b, overpass_event c where regime='S_above' and numpts>5 and a.radar_id=b.radar_id and a.radar_id=c.radar_id and a.orbit=c.orbit and date(c.overpass_time at time zone 'UTC') >= b.activation group by 1 order by 1;
\o /data/gpmgv/tmp/GR_PR_Bias_KUadj.html \\select a.radar_id||' (Ku-adj.)' as radar_id, a.meandiff_pre*(-1) as gr_pr_diff_pre, a.total_pre, b.meandiff_post*(-1) as gr_pr_diff_post, b.total_post from diffspres2ku a, diffsposts2ku b where a.radar_id=b.radar_id;
...
select regime,  round((gvmean+2.5)/5)*5 as GV_dbz, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_z_diff, sum(numpts) as total, count(orbit) as cases from zdiff_stats_by_dist_time_geo where numpts>25 group by 1,2 order by 1,2;
 regime  | gv_dbz | mean_z_diff | total  | cases 
---------+--------+-------------+--------+-------
 C_above |     20 |        3.59 |     35 |     1
 C_above |     25 |        0.91 |   4538 |    76
 C_above |     30 |       -0.21 |  26502 |   464
 C_above |     35 |       -1.13 |  34474 |   577
 C_above |     40 |       -2.01 |  12675 |   216
 C_above |     45 |       -2.95 |   2576 |    44
 C_above |     50 |       -4.14 |    194 |     4
 C_below |     35 |        1.58 |    788 |    16
 C_below |     40 |        0.83 |   9752 |   183
 C_below |     45 |        -0.9 |  29215 |   457
 C_below |     50 |       -1.86 |   4176 |    59
 C_in    |     30 |        0.97 |    776 |    16
 C_in    |     35 |        0.22 |  10469 |   204
 C_in    |     40 |       -0.57 |  71660 |  1068
 C_in    |     45 |       -1.43 |  67294 |   866
 C_in    |     50 |       -2.39 |   6814 |    91
 S_above |     20 |        2.06 |     62 |     2
 S_above |     25 |       -0.29 | 118715 |  1469
 S_above |     30 |       -1.88 | 130668 |  1301
 S_above |     35 |       -3.93 |   1121 |    15
 S_below |     25 |       -0.11 |   2086 |    38
 S_below |     30 |       -0.06 |  87713 |   785
 S_below |     35 |        -0.3 | 138690 |  1067
 S_below |     40 |        -0.7 |  16848 |    96
 S_in    |     25 |         0.2 |   8264 |   128
 S_in    |     30 |       -0.98 | 220025 |  2060
 S_in    |     35 |       -1.78 | 543031 |  3178
 S_in    |     40 |       -2.35 | 133285 |   504
 S_in    |     45 |       -4.34 |   4154 |     9
 Total   |     20 |        2.52 |     71 |     2
 Total   |     25 |       -0.11 | 111785 |  1226
 Total   |     30 |       -1.07 | 520882 |  3818
 Total   |     35 |       -1.36 | 800509 |  4269
 Total   |     40 |       -1.48 | 362209 |  1838
 Total   |     45 |       -2.26 |  45930 |   528
 Total   |     50 |       -2.18 |   4665 |    59
(36 rows)
...
\t \a \f '|' \o /tmp/event_best_diffs95pts25.txt \\select a.radar_id, date_trunc('day',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo a, overpass_event b where regime='S_above' and numpts>25 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id not in ('RGSN','DARW','KWAJ') and percent_of_bins=100 group by 1,2 order by 1,2;

\t \a \f '|' \o /data/gpmgv/tmp/event_best_diffs100pts3v7s2ku.txt \\select a.radar_id, date_trunc('day',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_s2kuv7 a, overpass_event b where regime='S_above' and numpts>2 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id not in ('RGSN','DARW','KWAJ') and percent_of_bins=100 group by 1,2 order by 1,2;

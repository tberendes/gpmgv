...
rsync -rtv --modify-window=3602  /home/data/gpmgv/coincidence_table/ \
...
# see if this USB disk has the NPP directory as a backup target.  If so, back up ~/NPP

if [ -d /media/usbdisk/NPP ]
  then
     rsync -rtv --modify-window=3602  /home/morris/NPP/  /media/usbdisk/NPP \
     | tee -a $LOG_FILE 2>&1
  else
     echo "" | tee -a $LOG_FILE 2>&1
     echo "No /NPP directory on USB disk, skipping backup of NPP files." | tee -a $LOG_FILE 2>&1
fi

...
echo "\t \a \f '|' \o $datelist \
  ...
  and a.orbit in \
  (select distinct orbit from ovlp25_w_rain25 where radar_id='DARW') \
  order by filedate;"\
...
echo "\t \a \f '|' \o $filelist \
     ...
     and a.orbit = b.orbit and a.radar_id = b.radar_id and a.radar_id='DARW'\
...
# - Prepare the control file for IDL to do PR and GV grid file creation.
	...
	echo "\t \a \f '|' \o $outfile \
	  ...
	  and a.radar_id = d.radar_id and a.radar_id='DARW' \

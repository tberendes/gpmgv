...
# 6/12/2012   Morris         - Added automatic configuration of GV_BASE_DIR and
#                            DATA_DIR based on user and machine IDs.
# 1/23/2015   Morris         - Changed TMP_DIR and LOG_DIR definitions to match
#                            new /data directory tree configurations.
#
###############################################################################
...
#    rowpost="|${PR_VERSION}|${PARAMETER_SET}|2.1|1|${INSTRUMENT_ID}"
    rowpost="|${PR_VERSION}|${PARAMETER_SET}|${GEO_MATCH_VERSION}|1|${INSTRUMENT_ID}|${SAT_ID}|NS"
    gzfile=`ls ${ncfile}\.gz`
    if [ $? = 0 ]
      then
...
ymdstart=`offset_date $ymd -150`
datestart=`echo $ymdstart | awk \
  '{print substr($1,1,4)"-"substr($1,5,2)"-"substr($1,7,2)" 00:00:00+00"}'`
datestart="2013-01-01 00:00:00+00"
dateEnd="2014-01-01 00:00:00+00"
#echo $datestart
echo "Running PRtoGR matchups for dates from $datestart to ${dateEnd}" | tee -a $LOG_FILE
...
from collatedprproductswsub c left outer join geo_match_product b \
  on (c.radar_id=b.radar_id and c.orbit=b.orbit and c.version=b.pps_version \
      and c.sat_id=b.sat_id and b.instrument_id='${INSTRUMENT_ID}') \
      and b.geo_match_version=${GEO_MATCH_VERSION} \
  join rainy100inside100 a on (a.orbit=c.orbit AND a.radar_id=c.radar_id AND a.sat_id=c.sat_id) \
where pathname is null and c.radar_id not in ('RGSN','KWAJ','KMXX','RMOR') and \
  c.overpass_time at time zone 'UTC' > '${datestart}' and \
  c.overpass_time at time zone 'UTC' < '${dateEnd}' and c.version='$PR_VERSION' order by 1;"`
	...
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select a.event_num, a.orbit, \
	    ...
	    collatedGVproducts c, rainy100inside100 d, collatedprproductswsub p \
            left outer join geo_match_product e on \
              (p.radar_id=e.radar_id and p.orbit=e.orbit and p.sat_id=e.sat_id and \
               p.version=e.pps_version and e.instrument_id = '${INSTRUMENT_ID}' \
               and e.geo_match_version=${GEO_MATCH_VERSION} and e.sat_id='${SAT_ID}')
          where a.radar_id = b.instrument_id and a.radar_id = c.radar_id \
	    and a.radar_id = d.radar_id and a.radar_id = p.radar_id \
	    and a.sat_id = d.sat_id and a.sat_id = p.sat_id \

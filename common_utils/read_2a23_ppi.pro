...
;       November 2012 - Bob Morris, GPM GV (SAIC)
;       - Added RainFlag and Bright Band Height as variables able to be read, to
;         support metadata extraction along the orbit swath.
;       - Added VERBOSE option to print file SD variables.
;       April 2014 - Bob Morris, GPM GV (SAIC)
;       - Added FILE_TEST validity check.
;
...
function read_2a23_ppi, filename, GEOL=geolocation, RAINTYPE=rainType, $
...
common sample, start_sample,sample_range
    ...
    IF FILE_TEST(filename, /REGULAR) NE 1 THEN BEGIN
       message, "Cannot file file: "+filename, /INFO
       flag = "File not found!"
       return, flag
    ENDIF
...
IF N_ELEMENTS(geolocation) GT 0 THEN BEGIN
   IF VERBOSE THEN print, 'PR 2A23 version: ', trmmversion
   CASE trmmversion OF
     6: begin
...
IF keyword_set(BBheight) THEN BEGIN
...
stride = [1,1]
   ...
   Catch, err_sts
   IF err_sts EQ 0 THEN BEGIN
      sds_id=HDF_SD_SELECT(sd_id,hdf_sd_nametoindex(sd_id,'HBB'))
      HDF_SD_GETDATA, sds_id, BBheight, START=start, COUNT=count, $
                STRIDE=stride, /noreverse
   ENDIF ELSE BEGIN
      help,!error_state,/st
      Catch, /Cancel

...
; All Rights Reserved.
;
; get_gr_geo_match_rain_type.pro    Morris/SAIC/GPM_GV    June 2011
;
; DESCRIPTION
; -----------
; Takes arrays of GR reflectivity data geometry-matched to PR or TMI, top and
; bottom heights of the volumes, and the set of array indices pointing to the
; set of TRMM instrument footprints under consideration for one level of the
; geometry match arrays; extracts vertical profiles of GR reflectivity for these
; footprints; and examines the vertical profiles to determine a GR rain type.
;
...
; - Created.
; 11/29/11 Morris, GPM GV, SAIC
; - Fixed error where meanBB_Z and stdev_BB_Z variables may not be instantiated
;   in certain cases, and use of their values is then attempted.
; 08/13/14 Morris, GPM GV, SAIC
; - Put in a check to avoid situation where a zero value for avgdepth results,
;   leading to a divide by zero situation and function crash.  This is more
;   common now that sample top/bottom heights are set to zero in cross section
;   program to indicate below-percent-threshold samples.
; 05/05/16 Morris, GPM GV, SAIC
; - Put error message for bad avgdepth under VERBOSE flag check.
;
...
FUNCTION get_gr_geo_match_rain_type, idx_in, gvz, top, botm, MEANBB=meanBB, $
...
ENDIF ELSE BEGIN
   ; Build the corresponding PR/TMI scan and ray number buffers (not displayed).  May
   ; need to cut one layer out of idx_in, which may have been replicated over each
   ; sweep level, e.g., by fprep_geo_match_profiles():
  ...
  ; analyze idx_in, decomposed into PR-product-relative scan and ray number
  ...
  ;    original pr_master_index or tmi_master_index values
  ; the following algorithm is documented at http://www.dfanning.com/code_tips/mode.html
   nfooties = n_elements(pridxscanray)
   array= (ABS(pridxscanray[0:nfooties-2]-pridxscanray[1:nfooties-1]))
  ...
  ; expand this subset of PR master indices into its scan,ray coordinates.
   raypr = pridxscanray/sample_range
   scanpr = pridxscanray MOD sample_range
ENDELSE
...
for ifoot = 0, nfooties-1 DO BEGIN
  ...
  ; these samples for evaluation
   ...
   IF verbose GT 0 THEN message, "Only "+STRING(countactual, FORMAT='(I0)')+ $
                                 " non-zero gvz values in ray, skipping.", /INFO
   ...
   ENDIF ELSE BEGIN
     ...
     ; center heights to best assure we are looking at adjacent samples
      ...
      IF avgdepth LE 0.0 THEN BEGIN
         ; this can happen when running x-sections where sample top/bottom are
         ; set to zero to flag below-threshold data
         IF verbose GT 0 THEN message, "Bad avgdepth value for ray, skipping.", /INFO
         CONTINUE
      ENDIF
   ENDELSE
...
;   maxvolhgt = MAX(hgtprofile)
   IF profilehgt LT 5.5 THEN BEGIN
;   IF avgdepth LT 0.2 THEN BEGIN
...
IF meanBB GT 0.0 THEN BEGIN
  ...
  ; 1st and last scan for multiscans, or define multiscan-like parameters for one
  ; scan case
   IF KEYWORD_SET( singlescan ) THEN BEGIN
      minscan=0 & maxscan = 0                  ; define one scan, index value 0
      raypr = idxscan                          ; initialize raypr to idx_in (=idxscan)
      scanpr = INTARR( N_ELEMENTS(idxscan) )   ; initialize all scan #'s to 0
   ENDIF ELSE BEGIN

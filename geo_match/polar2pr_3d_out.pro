...
; 8/23/13  Morris/GPM GV/SAIC
; - Renamed from polar2pr.pro, "includes" modified polar2pr_resampling.pro
;    file named polar2pr_resampling_3d_out.pro, modified to optionally
...
FUNCTION plot_bins_bailout, newAskAgain
   PRINT, ""
   PRINT, "PLOT_BINS is activated, this is EXTREMELY slow."
   prompt2do = "Disable PLOT_BINS? (enter Y, N, or W to write, or S for new step): "
   choicenum = 1
   tryagain:
   reply = 'x'
   WHILE (reply NE 'Y' AND reply NE 'N' AND reply NE 'W') DO BEGIN
      READ, reply, PROMPT=prompt2do
      IF (reply EQ 'Y' OR reply EQ 'y') THEN reply='Y'
      IF (reply EQ 'N' OR reply EQ 'n') THEN reply='N'
      IF (reply EQ 'W' OR reply EQ 'w') THEN reply='W'
      IF (reply EQ 'S' OR reply EQ 's') THEN BEGIN
         reply='N'
         print, "Current prompt step = ", STRING(newAskAgain, FORMAT='(I0)'), " bin plots."
         newVal = 'z'
         READ, newVal, PROMPT="Enter new value: "
         IF valid_num(newVal, newValNum, /INTEGER) THEN BEGIN
            IF newValNum LT 1 OR newValNum GT 300 THEN $
               print, "Value must be between 1 and 300, leaving value as-is." $
            ELSE newAskAgain = newValNum
         ENDIF ELSE print, "Value must be integer between 1 and 100, leaving value as-is."
      ENDIF
   ENDWHILE
...
PRO polar2pr_3d_out, control_file, range_threshold_km, PR_ROOT=prroot, DIR1C=dir1c, $
...
; set to a constant, until database supports PR product version override values
PR_version = 6
...
WHILE NOT (EOF(lun0)) DO BEGIN 
  ...
  ; get filenames as listed in/on the database/disk
   ...
   IF N_ELEMENTS(parsed) EQ 9 THEN BEGIN
      ...
      PR_version = FIX( parsed[8] )  ;control file includes PR_version
   ENDIF
...
; store the file basenames in a string array to be passed to gen_geo_match_netcdf()
   ...
   infileNameArr[0] = origFile21Name
   infileNameArr[1] = origFile23Name
   infileNameArr[2] = origFile25Name
   infileNameArr[3] = origFile31Name
...
; store the file basename in the string array to be passed to gen_geo_match_netcdf()
   ...
   PRINT, igv+1, ": ", pr_dtime, "  ", siteID, siteLat, siteLon
...
;   idx_uniq_elevs = UNIQ(elev_angle)
  ...
  ;   parallax-adjusted PR point X,Y and lon/lat coordinates, and PR corner X,Ys
   ...
   pr_x_corners_sfc = FLTARR(4, max_pr_fp)
   pr_y_corners_sfc = pr_x_corners_sfc
...
;-------------------------------------------------------------------------------
        ...
        ; is to-sfc projection of any point along PR ray is within range of GV volume?
         IF ( precise_range[ray_num,subset_scan_num] LE max_ranges[0] ) THEN BEGIN
           ...
           ; - MAKE THE INDEX IN TERMS OF THE (SCAN,RAY) COORDINATE ARRAYS
            ...
            x_sfc[numPRrays] = pr_x0[ray_num, subset_scan_num]
            y_sfc[numPRrays] = pr_y0[ray_num, subset_scan_num]
        ...
        ; If/As flag directs, add PR point(s) to the subarrays for each elevation
         IF ( action2do NE 0 ) THEN BEGIN
           ; compute the at-surface x,y values for the 4 corners of the current PR footprint
            ...
            pr_x_corners_sfc[*,numPRrays] = xy[0,*]
            pr_y_corners_sfc[*,numPRrays] = xy[1,*]
         ...
         IF ( action2do EQ 2 ) THEN BEGIN
           ; add another PR footprint to the analyzed set, to delimit the PR scan edge
            ...
            x_sfc[numPRrays] = pr_x0[ray_num, subset_scan_num] + Xoff  ; ditto, extrap. x,y
            y_sfc[numPRrays] = pr_y0[ray_num, subset_scan_num] + Yoff
  ...
  ; NOTE WE DON'T SAVE THE SURFACE X,Y FOOTPRINT CENTERS AND CORNERS TO NETCDF
   ...
   IF ( numPRinrange GT 0 ) THEN BEGIN
     ...
     ;   BBheight, rainFlag, rainType, BBstatus, status
  ...
  ; in code in a separate file, included below:
   ...
   @polar2pr_resampling_3d_out.pro

...
; 2)  compute_averages:      Computes averages of array data, with options
...
; - Got rid of PATH_OUT and logic, now expect to get the full pathname of any
;   output netCDF file as the value for NCFILE_OUT.
;
; EMAIL QUESTIONS OR COMMENTS TO:
;       <Bob Morris> kenneth.r.morris@nasa.gov
;       <Matt Schwaller> mathew.r.schwaller@nasa.gov
...
FUNCTION pr2tmi_nn, tmi2a12file, RADIUS=radius, NCFILE_OUT=ncfile_out, $
                    PATH_OUT=path_out, TRY_NC=try_nc, PRINT_TIMES=print_times

COMMON sample, start_sample, sample_range, num_range, NPIXEL_TMI_PR, NSPECIES_GMI
...
; set up for output of netCDF if we are writing
IF N_ELEMENTS( ncfile_out ) NE 0 THEN do_nc = 1 ELSE do_nc = 0

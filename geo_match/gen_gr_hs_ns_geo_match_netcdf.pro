...
;-------------------------------------------------------------------------------
;
; gen_gr_hsmsns_geo_match_netcdf.pro    Bob Morris, GPM GV (SAIC)    Feb 2016
;
; DESCRIPTION:
...
;
; HISTORY:
; 02/25/2016 by Bob Morris, GPM GV (SAIC)
...
FUNCTION gen_gr_hsmsns_geo_match_netcdf, geo_match_nc_file, numpts_HS, numpts_MS, $
...
GEO_MATCH_FILE_VERSION=1.0   ; hard code inside function now, not from "Include"
...
; define the 3 swaths in the DPR product, we need separate variables
...
fpdimid_HS = ncdf_dimdef(cdfid, 'fpdim_MS', numpts_MS>1)  ; # of MS footprints in range
fpdimid_NS = ncdf_dimdef(cdfid, 'fpdim_NS', numpts_NS)    ; # of NS footprints in range
fpdimid = [ fpdimid_HS, fpdim_MS, fpdimid_NS ]            ; match to "swath" array, above

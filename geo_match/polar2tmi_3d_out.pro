...
; 8/20/13  Morris/GPM GV/SAIC
; - Renamed from polar2tmi.pro, "includes" modified polar2tmi_resampling.pro
;    file named polar2tmi_resampling_3d_out.pro, modified to optionally
...
;   PRINT, "PLOT_BINS is activated, this is EXTREMELY slow."
   prompt2do = "Disable PLOT_BINS? (enter Y or N, or W to write this footprint): "
   choicenum = 1
   tryagain:
   reply = 'x'
...
PRO polar2tmi_3d_out, control_file, range_threshold_km, PR_ROOT=prroot, $
...
;   PRINT, igv+1, ": ", file_1CUF
  ...
  ;   parallax-adjusted TMI point X,Y and lon/lat coordinates, and TMI corner X,Ys
   ...
   tmi_x_corners_sfc = FLTARR(4, max_tmi_fp)
   tmi_y_corners_sfc = tmi_x_corners_sfc
...
; tmi_echoes[numTMIrays] = 1B  ; TEST PERFORMANCE OF DOING ALL FOOTIES
        ...
        ; If/As flag directs, add TMI point(s) to the subarrays for each elevation
         IF ( action2do NE 0 ) THEN BEGIN
           ; compute the at-surface x,y values for the 4 corners of the current TMI footprint
            ...
            tmi_x_corners_sfc[*,numTMIrays] = xy[0,*]
            tmi_y_corners_sfc[*,numTMIrays] = xy[1,*]
  ...
  ; in code in a separate file, included below:
   ...
   @polar2tmi_resampling_3d_out.pro

...
################################################################################
#
# get2A23Meta.sh    Morris/SAIC/GPM GV    October 2006
#
# DESCRIPTION
#   Runs IDL in batch mode to extract metadata from 2A23 orbit subset data files
...
#
# FILES
#   file2a23_KHTX.YYMMDD.txt (INPUT) - text file listing 2A23 files, orbits, and
...
#   get2A23MetaIDL.bat (INTERNAL)    - Batch file of IDL commands to be run.
#   2A23_HISTOGRAM.YYMMDD.unl (OUTPUT) - ASCII-delimited metadata file generated
#                                      by IDL routines, ready to load to
#                                      database.
#   Meta2A23_dbtempfile (INTERNAL)   - re-used file to temporarily hold output
#                                      from database queries
#   Meta2A23_todo.txt (INTERNAL)     - re-used file to temporarily hold list of
#                                      all YYMMDDs to process in this run
#
#  DATABASE
#    Catalogs PR 2A23 file RainType metadata in 'metadata_temp' table in
...
#
#  LOGS
#    Output for script run logged to daily log file get2A23Meta.YYMMDD.log
...
export IDL_PRO_DIR                 # IDL get2A23MetaIDL.bat needs this
...
DBTEMPFILE=${TMP_DIR}/Meta2A23_dbtempfile
# re-used file listing all yymmdd to be processed this run
META2A23=${TMP_DIR}/Meta2A23_todo.txt
rm -f $META2A23 $DBTEMPFILE
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${META_LOG_DIR}/get2A23Meta.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2A23MetaKHTX.${THISRUN}.log
echo "Processing KHTX 2A23 metadata for rundate ${THISRUN}" | tee -a $LOG_FILE
echo "+++++++++++++++++++++++++++++++++++++++++++" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'get2A23MetaHTX' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('get2A23MetaHTX', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'get2A23MetaHTX' AND status = '$UNTRIED' \
...
if [ ! -s ${IDL_PRO_DIR}/get2A23MetaIDL.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/get2A23MetaIDL.bat not found, exiting" \
       | tee -a $LOG_FILE
     echo "with status 'UNTRIED', no metadata processed for ${THISRUN}." \
       | tee -a $LOG_FILE
     exit 1
...
for yymmdd in `cat $META2A23`
  do
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'get2A23MetaHTX' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
	...
	$IDL < ${IDL_PRO_DIR}/get2A23MetaIDL.bat | tee -a $LOG_FILE 2>&1
	
	echo "=============================================" | tee -a $LOG_FILE
	
	# check the IDL output file before declaring success
	idl_out=${TMP_DIR}/2A23_HISTOGRAM.${yymmdd}.unl
	if [ -s $idl_out ]
	  then
...
echo "" | tee -a $LOG_FILE
echo "get2A23MetaKHTX.sh complete, exiting." | tee -a $LOG_FILE
exit 0

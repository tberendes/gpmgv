...
outfile=${TMP_DIR}/doSiteSpecificComparisons.txt
if [ -f $outfile ]
  then
    rm -v $outfile
fi
...
for site in `ls PRgrids.KMLB.07* | cut -f2 -d '.' | sort -u`
  do
    count=`ls ${DATA_DIR}/netcdf/PR/PRgrids.${site}.07* | wc -l`
    echo "$site|$count" | tee -a $outfile
    for prfile in `ls ${DATA_DIR}/netcdf/PR/PRgrids.${site}.07*`
      do

...
################################################################################
#
# update_CS_dirs_v4test_NSSL.sh      Morris/GPM GV/SAIC     March 2016
#
...
if [ ! -s $ORB ]
  then
    echo "update_CS_dirs_v4test_NSSL.sh:  Directory $ORB non-existent, please correct."
    exit 1
fi
...
for satInstrAlg in $1
  do
    if [ ! -s $ORB/$1 ]
      then
        echo "update_CS_dirs_v4test_NSSL.sh:  Directory $ORB/$1 non-existent, please correct."
        exit 1
    fi
    cd ${ORB}/$1

#!/bin/sh
#
# getPPSftpListings_NSSL.sh    Morris/SAIC/GPM GV    April 2016
#
# DESCRIPTION:
#
...
#                           augmented and/or overridden by package file.  The
#                           'mail_to' parameter in this file controls whether
#                           mirror will send a download status email, and the
#                           address to which the email will be sent.
#
...
#                           It is renamed mirror.YYMMDD.log and moved from
#                           $MIR_DATA_DIR to $LOG_DIR at end of script run.
#                           Its name is defined in the 'package' file.
#
#  DATABASE
...
GV_BASE_DIR=/oldeos/NASA_NMQ/satellite    # CHANGE THIS AS NEEDED, SEE BIN_DIR, MIR_BIN_DIR below

TMP_DIR=/oldeos/NASA_NMQ/satellite/data/tmp
if [ ! -s $TMP_DIR ]
  then
    echo "getPPSftpListings_NSSL.sh:  Directory $TMP_DIR non-existent, please correct."
    exit 1
fi
...
if [ ! -s $LOG_DIR ]
  then
    echo "getPPSftpListings_NSSL.sh:  Directory $LOG_DIR non-existent, please correct."
    exit 1
fi
...
if [ ! -s $BIN_DIR ]
  then
    echo "getPPSftpListings_NSSL.sh:  Directory $BIN_DIR non-existent, please correct."
    exit 1
fi
...
### MIR_DATA_DIR MUST BE SAME AS TMP_CS_DATA IN get_PPS_CS_data_NSSL.sh ###

# Following two variables must be the same as specified for the variables
# 'local_dir' and 'update_log' in the package file:
#      $MIR_BIN_DIR/$MIR_PACKAGES_DIR/arthurhou.pps.eosdis.nasa.gov

MIR_DATA_DIR=${TMP_DIR}/PPS_CS                    # "local_dir"
MIR_LOG_FILE=${MIR_DATA_DIR}/pps_ftp_lists.log    # "update_log"
if [ ! -s $MIR_DATA_DIR ]
  then
    echo "getPPSftpListings_NSSL.sh:  Directory $MIR_DATA_DIR non-existent, please correct."
    exit 1
fi
...
if [ -s $MIR_LOG_FILE ]
  then
    ...
    echo ""  | tee -a $LOG_FILE
    echo "Renaming mirror log file to a unique, datestamped name:"  | tee -a $LOG_FILE

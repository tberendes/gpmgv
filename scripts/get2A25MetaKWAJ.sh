...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${META_LOG_DIR}/get2A25MetaKWAJ.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2A25MetaKWAJ.${THISRUN}.log
echo "Processing KWAJ 2A25 metadata for rundate ${THISRUN}" | tee -a $LOG_FILE
echo "+++++++++++++++++++++++++++++++++++++++++++" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'get2A25MetaWAJ' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('get2A25MetaWAJ', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'get2A25MetaWAJ' AND status = '$UNTRIED' \
...
for yymmdd in `cat $META2A25`
  do
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'get2A25MetaWAJ' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
...
echo "" | tee -a $LOG_FILE
echo "get2A25MetaKWAJ.sh complete, exiting." | tee -a $LOG_FILE
exit 0

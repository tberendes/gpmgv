...
################################################################################
#
# doGVS-REOSiteSpecificComparisons.sh
#
# Prepare a site-by-site control file listing the matching GV netcdf grid files
# available in /data/netcdf[NEXRAD|NEXRAD_REO/allYMD], and invoke IDL to
# generate reflectivity comparison products for each site.
#
################################################################################

GV_BASE_DIR=/home/morris/swdev
IDL_PRO_DIR=${GV_BASE_DIR}/idl/valnet/comparez
...
outfile=${TMP_DIR}/doGVS-REOSiteSpecificComparisons.txt
if [ -f $outfile ]
  then
    rm -v $outfile
fi
...
cd ${DATA_DIR}/netcdf/NEXRAD
for site in `ls GVgrids.* | cut -f2 -d '.' | sort -u`
  do
    count=`ls ${DATA_DIR}/netcdf/NEXRAD/GVgrids.${site}.* | wc -l`
    echo "$site|$count" | tee -a $outfile
    for prfile in `ls ${DATA_DIR}/netcdf/NEXRAD/GVgrids.${site}.*`
      do
        gvfile=`echo $prfile | sed 's=NEXRAD/GVgrids=NEXRAD_REO/allYMD/GVgridsREO='`
        if [ -s $gvfile ]
          then
...
# $IDL < ${IDL_PRO_DIR}/doGVS-REOSiteSpecificComparisons.bat | tee -a $LOG_FILE 2>&1

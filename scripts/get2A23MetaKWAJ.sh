...
#
# FILES
#   file2a23_KWAJ.YYMMDD.txt (INPUT) - text file listing 2A23 files, orbits, and
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${META_LOG_DIR}/get2A23MetaKWAJ.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2A23MetaKWAJ.${THISRUN}.log
echo "Processing KWAJ 2A23 metadata for rundate ${THISRUN}" | tee -a $LOG_FILE
echo "+++++++++++++++++++++++++++++++++++++++++++" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'get2A23MetaWAJ' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('get2A23MetaWAJ', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'get2A23MetaWAJ' AND status = '$UNTRIED' \
...
for yymmdd in `cat $META2A23`
  do
    GETMYMETA=${DATA_DIR}/tmp/file2a23_KWAJ.${yymmdd}.txt
    if [ -s $GETMYMETA ]
      then
        RUNDATE=$yymmdd
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'get2A23MetaWAJ' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
...
echo "" | tee -a $LOG_FILE
echo "get2A23MetaKWAJ.sh complete, exiting." | tee -a $LOG_FILE
exit 0

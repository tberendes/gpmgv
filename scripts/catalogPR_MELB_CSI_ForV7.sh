...
LOG_FILE=${LOG_DIR}/catalogPR_MELB_CSI_FromDAAC.${rundate}.log
...
subsetout="sub-GPMGV1"  # override for output to DB
subset="sub-SE"         # hard-wire subset
version=166
#versionout=6            # override for output to DB
cd ${TMP_DIR}/MELB_Feb98/v7_${version}  # ftp download common directory for PR product files

    # catalog the files in the database
    for type in 1C21 2A23 2A25 2B31
      do
        cd $type
        for file in `ls ${type}.*.${subset}.*`
          do
            mv -v $file ${DATA_DIR}/prsubsets/${type} | tee -a $LOG_FILE       # uncomment if running
#	    echo mv -v $file ${DATA_DIR}/prsubsets/${type} | tee -a $LOG_FILE  # uncomment if testing
            dateString=`echo $file | cut -f2 -d '.' | awk \
              '{print "19"substr($1,1,2)"-"substr($1,3,2)"-"substr($1,5,2)}'`
            orbit=`echo $file | cut -f3 -d '.'`
#	    version=`echo $file | cut -f5 -d '.'`
	    #echo "subset ID = $subset" | tee -a $LOG_FILE
            #echo "file = ${file}, dbdate = ${dateString}, orbit = $orbit"
	    echo ${satid}'|'${orbit}'|'${type}'|'${dateString}'|'${file}'|'${subsetout}'|'${version} \
	      | tee -a $loadfile | tee -a $LOG_FILE
        done
	cd ..
    done

#exit  # uncomment if running

...
################################################################################
#
# get2AGPROFMeta4date.sh    Morris/SAIC/GPM GV    May 2016
#
# DESCRIPTION
#   Runs IDL in batch mode to extract metadata from 2AGPROF orbit subset data files
...
#   get2AGPROFMetaIDL.bat (INTERNAL)    - Batch file of IDL commands to be run.
#   2AGPROF_METADATA.YYMMDD.unl (OUTPUT) - ASCII-delimited metadata file generated
...
#
#  DATABASE
#    Catalogs PR 2AGPROF file rain event metadata in 'metadata_temp' table in
...
#
#  LOGS
#    Output for script run logged to daily log file get2AGPROFMeta4date.YYMMDD.log
...
export IDL_PRO_DIR                 # IDL get2AGPROFMetaIDL.bat needs this
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${META_LOG_DIR}/get2AGPROFMeta4date.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
LOG_FILE=${META_LOG_DIR}/get2AGPROFMeta4date.${THISRUN}.log
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'getGPROFMeta4dy' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('getGPROFMeta4dy', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'getGPROFMeta4dy' AND status = '$UNTRIED' \
...
if [ ! -s ${IDL_PRO_DIR}/get2AGPROFMetaIDL.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/get2AGPROFMetaIDL.bat not found, exiting" \
       | tee -a $LOG_FILE
     echo "with status 'UNTRIED', no metadata processed for ${THISRUN}." \
       | tee -a $LOG_FILE
     exit 1
      ...
      then
        RUNDATE=$yymmdd
       	...
       	echo "UPDATE appstatus SET ntries = ntries + 1 \
       	      WHERE app_id = 'getGPROFMeta4dy' AND datestamp = '$RUNDATE';" \
            	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
       	echo "" | tee -a $LOG_FILE
       	echo "=============================================" | tee -a $LOG_FILE
       	...
       	$IDL < ${IDL_PRO_DIR}/get2AGPROFMetaIDL.bat | tee -a $LOG_FILE 2>&1
	
       	echo "=============================================" | tee -a $LOG_FILE
	
       	# check the IDL output file before declaring success
	       idl_out=${TMP_DIR}/2AGPROF_METADATA.${yymmdd}.unl
ls -al $idl_out
	       if [ -s $idl_out ]
	         then
	           # load metadata to database and check status of load script
	      echo ${BIN_DIR}/loadMetadataGPROF.sh $idl_out | tee -a $LOG_FILE 2>&1
	           if [ $? ]
	             then
...
echo "" | tee -a $LOG_FILE
echo "get2AGPROFMeta4date.sh complete, exiting normally." | tee -a $LOG_FILE
exit 0

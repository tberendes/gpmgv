...
create table dbzdiff_stats_by_dist_geo (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
...
create table dbzdiff_stats_by_dist_geo_s2ku (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totalany into temp anytotal from dbzdiff_stats_by_dist_geo where regime='Total' group by 1,2,3,4,5,6;

select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totaltypes into temp totalbytypes from dbzdiff_stats_by_dist_geo where regime!='Total' group by 1,2,3,4,5,6;
...
delete from dbzdiff_stats_by_dist_geo;
\copy dbzdiff_stats_by_dist_geo from '/data/gpmgv/tmp/StatsByDistToDBGeo_Pct100_DefaultS.unl' with delimiter '|' 
delete from dbzdiff_stats_by_dist_geo_s2ku;
\copy dbzdiff_stats_by_dist_geo_s2ku from '/data/gpmgv/tmp/StatsByDistToDBGeo_Pct100_S2Ku.unl' with delimiter '|' 

-- "Best" bias regime (stratiform above BB), broken out by site and range:
select radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo where regime='S_above' and numpts>5 and percent_of_bins=95 group by 1,2 order by 1,2;

-- "Best" bias regime (stratiform above BB), broken out by site only:
select radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo where regime='S_above' and numpts>5 group by 1 order by 1;

-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, rangecat*50+25 as mean_range_km, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from dbzdiff_stats_by_dist_geo where regime='S_above' and numpts>5 and percent_of_bins=95 group by 1,2 order by 1,2;

-- Bias by site, height, and regime(s), for given gv source
select radar_id, height, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from dbzdiff_stats_by_dist_geo where regime like 'S_%' and numpts>0 and gvtype='GeoM' group by 1,2 order by 1,2;


-- "Best" bias regime (stratiform above BB), broken out by site, GV type and range:
select gvtype, radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanpr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangv, sum(numpts) as total from dbzdiff_stats_by_dist_geo where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
 ...
 into temp dbzsums from dbzdiff_stats_by_dist_geo where meandiff > -99.9
...
select a.radar_id, date_trunc('month',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo a, overpass_event b where regime='S_above' and numpts>5 and percent_of_bins=95 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id ='KAMX' group by 1,2 order by 1,2;

-- Vertical profiles of PR and GV mean Z:
select radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo where rangecat<2 and gvtype='REOR' group by 1,2 order by 1,2;

select gvtype, radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo where rangecat<2 group by 1,2,3 order by 2,1,3;
...
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp stats2a55 from dbzdiff_stats_by_dist_geo where regime='S_above' and numpts>5 and percent_of_bins=95 and rangecat<2 group by 1,2 order by 1,2;

-- Case-by-case differences for REOR
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsreo from dbzdiff_stats_by_dist_geo where regime='S_above' and numpts>5 and gvtype='REOR' and rangecat<2 group by 1,2 order by 1,2;

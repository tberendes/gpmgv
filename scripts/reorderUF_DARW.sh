...
###############################################################################
#
# reorderUF_DARW.sh    Morris/SAIC/GPM GV    Feb 2008
#
# Wrapper to do grid generation from UF files for DARW cases meeting criteria
...
export IDL_PRO_DIR                 # IDL reorderUF_DARW.bat needs this
...
if [ ! -s ${IDL_PRO_DIR}/reorder1CUF.bat ]
  then
     echo "Script ${IDL_PRO_DIR}/reorder1CUF.bat not found, exiting" \
       | tee -a $LOG_FILE
...
echo "" | tee -a $LOG_FILE
start1=`date -u`
echo "Calling IDL with reorderUF_DARW.bat on $start1" | tee -a $LOG_FILE
$IDL < ${IDL_PRO_DIR}/reorderUF_DARW.bat | tee -a $LOG_FILE 2>&1
echo "" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
end=`date -u`
echo "reorderUF_DARW.sh for $yymmdd completed on $end"\
 | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
echo "===================================================================="\
 | tee -a $LOG_FILE

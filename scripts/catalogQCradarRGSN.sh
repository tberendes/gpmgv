...
SQL_BIN=${BIN_DIR}/catalogQCradarDarw.sql
loadfile=${DATA_DIR}/tmp/finalQC_DarwMeta.unl

rundate=`date -u +%y%m%d`
LOG_FILE=${LOG_DIR}/catalogQCradarDarw.${rundate}.log
DB_LOG_FILE=${LOG_DIR}/catalogQCradarDarwSQL.log
runtime=`date -u`
...
# for 88D files from TRMM GV, otherwise from 0-23 for UF files from all
# other sources.  RGSN is Korea Std Time Zone, others are UTC.
# -- convert to 00:00 to 23:00 UTC for loading in database

case $type in
                1CUF )  case $2 in
                           RMOR ) dtime=`echo $pathless | cut -f2 -d'_' | cut -c9-10` ;;
			   RGSN ) dtime=`echo $pathless | cut -f3 -d'_' | cut -c9-10` ;;
			      * ) dtime=`echo $pathless | cut -f2 -d'.'`
		                  dtime=`expr $dtime - 1` ;;
			esac
                        dtime=${dtime}":00"  ;;
...
# append correct time zone ID to nominal datetime
case $2 in
    RGSN ) dtimestr=${datestr}' '${dtime}"+09" ;;
...
for Kxxx in RGSN              # for testing, just do one site
  do
    echo "" | tee -a $LOG_FILE

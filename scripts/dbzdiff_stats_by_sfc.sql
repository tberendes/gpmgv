...
create table dbzdiff_stats_by_sfc_geo (
   percent_of_bins int,
   sfctype int,
   gvtype char(4),
   ...
   gvmean float,
   numpts int,
   primary key (percent_of_bins, sfctype, gvtype, regime, radar_id, orbit, height)
);

delete from dbzdiff_stats_by_sfc_geo;
\copy dbzdiff_stats_by_sfc_geo from '/data/tmp/StatsBySfcToDBbyGeo.unl' with delimiter '|' 

-- "Best" bias regime (stratiform above BB), broken out by site and surface, for Geo:
select radar_id, sfctype, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_sfc_geo where regime='S_above' and numpts>5 and gvtype='GeoM' group by 1,2 order by 1,2;

-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, sfctype, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from dbzdiff_stats_by_sfc_geo where regime='S_above' and numpts>5 and gvtype='GeoM' group by 1,2 order by 1,2;

-- As above, but by site and surface and stratiform regimes
select radar_id, sfctext, regime, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_sfc_geo natural join surface_type where regime like 'S_%' and numpts>5 and gvtype='GeoM' group by 1,2,3 order by 1,2,3;

-- Bias by site, height, and regime(s), for given gv source
select radar_id, height, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from dbzdiff_stats_by_sfc_geo where regime like 'S_%' and numpts>0 and gvtype='GeoM' group by 1,2 order by 1,2;


-- "Best" bias regime (stratiform above BB), broken out by site, GV type and range:
select gvtype, radar_id, sfctype, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_sfc_geo where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
       sum(numpts) as n, gvtype, height, sfctype
 into temp dbzsums from dbzdiff_stats_by_sfc_geo where meandiff > -99.9
 group by 4,5,6 order by 4,5,6; 
select round(100.*w/n)/100. as bias, gvtype, height, sfctype from dbzsums; 
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
       ...
       sum(numpts) as n, gvtype, regime, radar_id, height, sfctype
  into temp sitedbzsums from dbzdiff_stats_by_sfc_geo
 where meandiff > -99.9 and diffstddev > -99.9
 group by 8,9,10,11,12 order by 8,9,10,11,12;

select a.radar_id, a.regime, a.height, a.sfctype, round(100*(a.w/a.n))/100 as bias_vs_2A55, round(100*(a.s/a.n))/100 as stddev2A55, a.n as num_2A55,  round(100*(b.w/b.n))/100 as bias_vs_REORD, round(100*(b.s/b.n))/100 as stddevREORD, b.n as num_REORD from sitedbzsums a, sitedbzsums b where a.gvtype = '2A55' and b.gvtype = 'REOR' and a.radar_id = b.radar_id and a.regime = b.regime and a.height = b.height and a.sfctype = b.sfctype order by 1,2,4,3;

select a.radar_id, a.regime, a.height, a.sfctype, round(100*(a.w/a.n))/100 as bias_vs_2A55, round(100*(b.w/b.n))/100 as bias_vs_GEOM, round(100*(a.s/a.n))/100 as stddev2A55, round(100*(b.s/b.n))/100 as stddevGEOM, a.n as num_2A55, b.n as num_GEOM from sitedbzsums a, sitedbzsums b where a.gvtype = '2A55' and b.gvtype = 'GeoM' and a.radar_id = b.radar_id and a.regime = b.regime and a.height = b.height and a.sfctype = b.sfctype and a.regime = 'S_above' order by 1,2,4,3; 

select a.radar_id, a.regime, a.height, a.sfctype, round(100*(a.w/a.n))/100 as bias_vs_REOR, round(100*(b.w/b.n))/100 as bias_vs_GEOM, round(100*(a.s/a.n))/100 as stddevREOR, round(100*(b.s/b.n))/100 as stddevGEOM, a.n as num_REOR, b.n as num_GEOM from sitedbzsums a, sitedbzsums b where a.gvtype = 'REOR' and b.gvtype = 'GeoM' and a.radar_id = b.radar_id and a.regime = b.regime and a.height = b.height and a.sfctype = b.sfctype and a.regime = 'S_above' order by 1,2,4,3; 

select a.radar_id, a.regime, round(100*SUM(a.w)/SUM(a.n))/100 as bias_vs_GRID, round(100*SUM(b.w)/SUM(b.n))/100 as bias_vs_GEOM, round(100*SUM(a.s)/SUM(a.n))/100 as stddevGRID, round(100*SUM(b.s)/SUM(b.n))/100 as stddevGEOM, SUM(a.n) as num_GRID, SUM(b.n) as num_GEOM from sitedbzsums a, sitedbzsums b where a.gvtype = 'REOR' and b.gvtype = 'GeoM' and a.radar_id = b.radar_id and a.regime = b.regime and a.height = b.height and a.sfctype = b.sfctype and a.regime = 'S_above' group by 1,2 order by 1,2;

-- "Best" bias regime (stratiform above BB), broken out by site and month, for 2A55:
select a.radar_id, date_trunc('month',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_sfc_geo a, overpass_event b where regime='S_above' and numpts>5 and gvtype='2A55' and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id ='KAMX' group by 1,2 order by 1,2;

-- Vertical profiles of PR and GV mean Z:
select radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_sfc_geo where sfctype<2 and gvtype='REOR' group by 1,2 order by 1,2;

select gvtype, radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_sfc_geo where sfctype<2 group by 1,2,3 order by 2,1,3;


-- Case-by-case differences for GeoM
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsgeo from dbzdiff_stats_by_sfc_geo where regime='S_above' and numpts>5 and gvtype='GeoM' and sfctype<2 group by 1,2 order by 1,2;
          
-- Case-by-case differences for 2A55
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp stats2a55 from dbzdiff_stats_by_sfc_geo where regime='S_above' and numpts>5 and gvtype='2A55' and sfctype<2 group by 1,2 order by 1,2;

-- Case-by-case differences for REOR
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsreo from dbzdiff_stats_by_sfc_geo where regime='S_above' and numpts>5 and gvtype='REOR' and sfctype<2 group by 1,2 order by 1,2;

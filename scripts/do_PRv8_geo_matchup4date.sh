...
################################################################################
#
#  do_PRv8_geo_matchup4date.sh    Morris/SAIC/GPM GV    Oct. 2017
#
#
#  DESCRIPTION
#
#   Called by do_PRv8_GeoMatch.sh, which generates the needed inputs.
...
#  SYNOPSIS
#
#   do_PRv8_geo_matchup4date.sh [OPTIONS]  YYMMDD  CONTROLFILE
#
#
#  OPTIONS/ARGUMENTS:
#
...
#  LOGS
#
#    Output for script run is logged to file do_PRv8_geo_matchup4date.yymmdd.log
...
#    logged to the current-date-tagged file do_PRv8_geo_matchup.yymmdd.fatal.log
...
# 10/18/2017   Morris     Created from do_DPR_geo_matchup4date.sh.
#
################################################################################
...
DBTEMPFILE=${TMP_DIR}/do_PRv8_geo_matchup_dbtempfile
...
if [ $# != 2 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/do_PRv8_geo_matchup4date.${THISRUN}.fatal.log
     echo "FATAL: Exactly two non-option arguments required, $# given." | tee -a $LOG_FILE
     echo "Usage: do_PRv8_geo_matchup4date.sh [-f[0|1]] YYMMDD CONTROLFILE"
     exit 1
fi
...
LOG_FILE=${LOG_DIR}/do_PRv8_geo_matchup4date.${THISRUN}.log
...
case "${FORCE_MATCH}"
  in
    ...
    *) echo "Invalid value ${FORCE_MATCH} for -f option in do_PRv8_geo_matchup4date.sh."\
       | tee -a $LOG_FILE
       exit 1;;
esac
echo "" | tee -a $LOG_FILE
...
case "${DO_RHI}"
  in
    0) BATFILE=polar2dpr_prV8_${PARAMETER_SET}.bat ;;
    1) BATFILE=rhi2dpr_prV8_${PARAMETER_SET}.bat ;;
    *) echo "Invalid value ${DO_RHI} for -r option in do_PRv8_geo_matchup4date.sh."\
       | tee -a $LOG_FILE
       exit 1;;
esac
...
if [ -s $CONTROLFILE ]
  then
    ...
    echo "ERROR in do_PRv8_geo_matchup4date.sh, control file $CONTROLFILE not found."\
     | tee -a $LOG_FILE
    exit 1
fi
...
# must be identically defined here and in do_PRv8_GeoMatch.sh

DBCATALOGFILE=${TMP_DIR}/do_PRv8_geo_matchup_catalog.${THISRUN}.txt
...
echo "" | tee -a $LOG_FILE
echo "do_PRv8_geo_matchup4date.sh complete, exiting." | tee -a $LOG_FILE

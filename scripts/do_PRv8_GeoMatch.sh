...
###############################################################################
#
# do_PRv8_GeoMatch.sh    Morris/SAIC/GPM GV    October 2018
#
# Wrapper to do PR-GR NetCDF geometric matchups for TRMM 2A-PR Version 8 files
# already received and cataloged, for cases meeting predefined criteria.  This
# script drives volume matches between TRMM PR and ground radar (GR) data for
# the single PR normal scan (NS) type to produce the baseline "GRtoDPR"
...
# each date, calls the child script do_PRv8_geo_matchup4date.sh, which in turn
...
# dates to be processed, listing PR and ground radar data files to be processed
# for rainy site overpass events for that calendar date, as well as metadata
# parameters related to the PR and GR data and the site overpass events.
#
# Volume matches are done for only the TRMM 2APR file type, for the NS scan
# type in that 2A data type.
#
...
# script as a default option.  Event definition includes cases where the PR
# indicates "rain certain" at 100 or more gridpoints within 100 km of the radar
# within the 4km gridded 2A-PR product.  See the SQL command file
# ${BIN_DIR}/'rainCases100kmAddNewEvents.sql'.
#
# SYNOPSIS
# --------
#
#    do_PRv8_GeoMatch.sh [OPTION]...
#
#
#    OPTIONS/ARGUMENTS:
#    -----------------
...
#                           See do_PRv8_geo_matchup4date.sh for details.  INTEGER type.
#
...
#
#
# NOTE:  When running dates that might have already had PR-GR matchup sets
...
#        from this table where app_id='geo_match_PRv8', either for the date(s)
#        to be run, or for all dates.  EXCEPTION:  If script is called with the
#        -f option (e.g., "do_PRv8_GeoMatch.sh -f"), then the status of prior
...
# 10/18/2017   Morris      - Created from do_DPR_GeoMatch.sh.
#
###############################################################################
...
PPS_VERSION=TRMM016d  #"V05A"        # specifies which PPS version of products to process
export PPS_VERSION
PARAMETER_SET=0  # set of polar2dpr parameters (polar2dpr.bat file) in use
export PARAMETER_SET
INSTRUMENT_ID="PR"        # type of TRMM 2A products to process: 2APR
export INSTRUMENT_ID
SAT_ID="TRMM"
export SAT_ID
SWATH="NS"      # type of PR scan to process: only NS exists for 2APR
export SWATH
...
# force (re)run of matchups by child script do_PRv8_geo_matchup4date.sh:
  ...
  do
    case "${option}"
      in
        ...
        f) FORCE_MATCH=1;;
        *) echo "Usage: "
           echo "do_PRv8_GeoMatch.sh -i INSTRUMENT -v PPS_Version -p ParmSet -w SWATH " \
...
LOG_FILE=${LOG_DIR}/do_PRv8_GeoMatch.${rundate}.log
export rundate
...
# listed in the 'do_PRv8_geo_matchup_catalog.yymmdd.txt' file, which was produced
# by do_PRv8_geo_matchup4date.sh by examining its own log file for date yymmdd, 
# 'do_PRv8_geo_matchup4date.yymmdd.log'. Parses the output netCDF file names to
...
# Specify the start and end dates here in the code for an ad-hoc run.
dateStart='2009-04-10'
dateEnd='2009-04-15'
...
"SELECT b.sat_id, a.orbit, a.radar_id, a.overpass_time, date_trunc('hour'::text, a.overpass_time) AS nominal, b.subset, a.event_num, a.nearest_distance into temp tempevents \
from overpass_event a, siteproductsubset b, orbit_subset_product o, rainy100inside100 r \
 ...
 AND o.product_type = '${ALGORITHM}' and o.version='${PPS_VERSION}' and o.sat_id=b.sat_id \
   and b.subset in ('CONUS','KWAJ') and b.radar_id not in ('RGSN','KMXX','RMOR') \
   and a.overpass_time at time zone 'UTC' > '${dateStart}' \
   and a.overpass_time at time zone 'UTC' < '${dateEnd}' \
...
for thisdate in `cat $datelist`
  do
   ...
   # append manner to 'outfileall', which is run-date-specific.
    filelist=${TMP_DIR}/PRv8_filelist4geoMatch_temp.txt
    outfile=${TMP_DIR}/PRv8_files_sites4geoMatch_temp.txt
   ...
   # define the pathname of the final control file for the date and type
    outfileall=${TMP_DIR}/PRv8_files_sites4geoMatch.${tag}.txt
   ...
   # -- 2BPRTMI file is ignored for now
...
||to_char(d.filedate,'MM')||'/'||to_char(d.filedate,'DD')||'/'||d.filename\
     ...
     JOIN orbit_subset_product d ON c.sat_id='PR' and c.orbit = d.orbit\
        AND c.subset = d.subset AND d.sat_id='$SAT_ID' \
...
echo ''
   ...
   #   for RHI matchups.
	...
	DBOUT3=`psql -a -A -t -o $outfile -d gpmgv -c "select a.event_num, a.orbit, \
	    ...
	    collatedGVproducts c, rainy100inside100 d, collatedPRv8products p \
...
#exit  # if uncommented, creates the control file for first date, and exits
    ...
    if [ -s $outfileall ]
      then
       # Call the IDL wrapper script, do_PRv8_geo_matchup4date.sh, to run
       # the IDL .bat files.  Let each of these deal with whether the yymmdd
       # has been done before.

        echo "" | tee -a $LOG_FILE
        start1=`date -u`
        echo "Calling do_PRv8_geo_matchup4date.sh $yymmdd on $start1" | tee -a $LOG_FILE
        ${BIN_DIR}/do_PRv8_geo_matchup4date.sh -f $FORCE_MATCH $yymmdd $outfileall

        case $? in
          0 )
            echo ""
            echo "SUCCESS status returned from do_PRv8_geo_matchup4date.sh"\
             | tee -a $LOG_FILE
           ...
           # must be identically defined here and in do_PRv8_geo_matchup4date.sh
            DBCATALOGFILE=${TMP_DIR}/do_PRv8_geo_matchup_catalog.${yymmdd}.txt
            if [ -s $DBCATALOGFILE ] 
              then
          ...
          1 )
            echo ""
            echo "FAILURE status returned from do_PRv8_geo_matchup4date.sh, quitting!"\
             | tee -a $LOG_FILE
            exit 1
          ;;
          2 )
            echo ""
            echo "REPEAT status returned from do_PRv8_geo_matchup4date.sh, do nothing!"\
             | tee -a $LOG_FILE
          ;;
        esac

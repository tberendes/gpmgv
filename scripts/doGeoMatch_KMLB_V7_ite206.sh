...
###############################################################################
#
# doGeoMatch_KMLB_multiVols.sh    Morris/SAIC/GPM GV    September 2010
#
...
# 8/31/2010   Morris         Created from doGeoMatch_KMLB_v7.sh.  Modified SQL
#                            to match all volume scans within +/- 30 minutes of
#                            the PR overpass.  Need to query new 'radarcatalog'
#                            table which catalogs all KMLB UF files for '07-'08
#
###############################################################################
...
RADARID=KWAJ
SUBSETID=KWAJ
VERSIONID=206   # make sure to edit NC_NAME_ADD in polar2pr.bat accordingly
...
echo "\t \a \f '|' \o $datelist \
  ...
  from overpass_event where radar_id = '${RADARID}' \
   and date( date_trunc('day', overpass_time at time zone 'UTC') ) between \
   '2001-01-01' and '2001-06-30' order by 1;" | psql gpmgv | tee -a $LOG_FILE 2>&1
...
echo "\t \a \f '|' \o $filelist \
     ...
     from collatedPRproductswsub a, overpass_event b, radarcatalog c \
     ...
     and file1c21 is not null and a.radar_id = '${RADARID}' and a.subset='${SUBSETID}' and a.version='${VERSIONID}' \
...
#                             the same radar_id comes up more than once for a case.
	...
	echo "\t \a \f '|' \o $outfile \\\ select a.event_num, a.orbit, \
	    ...
	    and a.radar_id = '${RADARID}' \
	    and a.orbit = ${orbit} \
            and c.valid between a.overpass_time-interval '30 minutes' and a.overpass_time+interval '30 minutes'
          order by 3;" \
...
#	exit 1
    ...
    echo "Matchup scripts for $yymmdd completed on $end"\
    | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
    echo "=================================================================="\
    | tee -a $LOG_FILE

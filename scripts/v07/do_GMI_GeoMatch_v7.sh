...
###############################################################################
#
# do_GMI_GeoMatch_v7.sh    Morris/SAIC/GPM GV    October 2014
#
# DESCRIPTION
# -----------
...
#
# For each date, calls the child script do_GMI_geo_matchup4date_v7.sh, which
...
# do_GMI_geo_matchup4date_v7.sh by examining the do_GMI_geo_matchup4date_v7.yymmdd.log 
...
while read thisdate
  do
   ...
   # 'outfileall', which is run-date-specific.
    ...
    outfileall=${TMP_DIR}/GMI_files_sites4geoMatch_v7.${yymmdd}.txt
...
#    exit  # if uncommented, creates the control file for first date, and exits
    ...
    if [ -s $outfileall ]
      then
       # Call the IDL wrapper scripts, do_GMI_geo_matchup4date_v7.sh, to run
       # the IDL .bat files.  Let each of these deal with whether the yymmdd
       # has been done before.

        echo "" | tee -a $LOG_FILE
        start1=`date -u`
        echo "Calling do_GMI_geo_matchup4date_v7.sh $yymmdd on $start1" | tee -a $LOG_FILE
        ${BIN_DIR}/do_GMI_geo_matchup4date_v7.sh $yymmdd
...
#		continue
        ...
        case $? in
          0 )
            echo ""
            echo "SUCCESS status returned from do_GMI_geo_matchup4date_v7.sh"\
        	 | tee -a $LOG_FILE
           ...
           # must be identically defined here and in do_geo_matchup4date_v7.sh
...
#                exit 1
          ...
          1 )
            echo ""
            echo "FAILURE status returned from do_GMI_geo_matchup4date_v7.sh, quitting!"\
	     | tee -a $LOG_FILE
	    exit 1
          ;;
          2 )
            echo ""
            echo "REPEAT status returned from do_GMI_geo_matchup4date_v7.sh, do nothing!"\
	     | tee -a $LOG_FILE
          ;;
        esac

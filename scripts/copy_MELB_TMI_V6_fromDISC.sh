...
################################################################################
#
#  copy_MELB_TMI_V6_fromDISC.sh     Morris/SAIC/GPM GV     November 2011
#
#  DESCRIPTION
#    Retrieves MELB CSI subset PR files from DAAC site: disc2.nascom.nasa.gov.
...
#    into a log file 'MELB_PR_CSI_newfiles.yymmdd.log' to be appended to the
#    mirror.yymmdd.log file so that get2A23-25Meta.sh will extract the PR
#    metadata for the MELB PR CSI files as well as from those retrieved via
...
#       wgetMELB_PR_CSI.YYMMDD.log in data/logs subdirectory, where YYMMDD is
#       the date when the script is run.
#    2) List of retrieved files is written to the additional log file
#       MELB_PR_CSI_newfiles.YYMMDD.log in data/logs subdirectory.
#
#  CONSTRAINTS
...
TMP_MELB_DATA=${DATA_DIR}/tmp/PR_KWAJ_DAAC
BIN_DIR=${GV_BASE_DIR}/scripts
LOG_DIR=${DATA_DIR}/logs
rundate=`date -u +%y%m%d`
LOG_FILE=${LOG_DIR}/copyMELB_PR_CSI.${rundate}.log
LOG_DATA_FILE=${LOG_DIR}/MELB_PR_CSI_newfiles.${rundate}.log
PATH=${PATH}:${BIN_DIR}
ZZZ=1800
...
DBTEMPFILE=${TMP_MELB_DATA}/dbtempfile
# file listing status of each date's download attempts
THE_SCOOP=${TMP_MELB_DATA}/DateStatus
rm -f $THE_SCOOP
...
today=`date -u +%Y%m%d`
echo "===================================================" | tee $LOG_FILE
echo " Attempting download of MELB CSI PR files on $today." | tee -a $LOG_FILE
echo "---------------------------------------------------" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
...
if [ ${pgproccount} -lt 3 ]
  then
    thistime=`date -u`
    echo "Message from copy_MELB_TMI_V6_fromDISC.sh job on ${thistime}:" \
...
function wgetPRtypes4date() {
     ...
     # Use wget to download PR MELB subsets for each type from DAAC ftp site. 
   ...
   do
     ...
     while [ "$runagain" != 'n' ]
       do
          ...
          wget -P $2 -N \
            ftp://disc2.nascom.nasa.gov/data/s4pa/${LDIR}/TRMM_${type}_CSI_MELB/$1/${type}_CSI.$3.*.MELB.6.HDF.Z
          filelist4type=`ls $2/${type}_CSI.$3.*.MELB.6.HDF.Z`
          if [ $? = 1 ]
            then
               if [ $tries -eq 2 ]
                 then
...
echo "Getting MELB CSI files." | tee -a $LOG_FILE
      ...
      do
        ...
        for day in `echo $days`
          do
            ...
            wgetPRtypes4date $jdaydir $TMP_MELB_DATA $yymmdd
#        if [ $? -eq 4 ]
            if [ $? -eq 1 ]
              then
	        ...
	        echo "UPDATE appstatus SET status = '$SUCCESS' WHERE \
	          app_id = 'wgetMELB' AND datestamp = '$yymmdd';" \
		  | tee -a $LOG_FILE 2>&1

...
create table zdiff_stats_by_dist_time_geo_kma (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
...
create table zdiff_stats_by_dist_time_geo_kma_s2ku (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
\copy zdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_DefaultS.unl' with delimiter '|' 
delete from zdiff_stats_by_dist_time_geo_kma_s2ku;
\copy zdiff_stats_by_dist_time_geo_kma_s2ku from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_S2Ku.unl' with delimiter '|' 
\copy zdiff_stats_by_dist_time_geo_kma from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_KMLB_DefaultS.unl' with delimiter '|' 
...
select height, percent_of_bins, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanprv, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangr, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime like 'C_%' and numpts>5 and radar_id='KMLB' group by 1,2 order by 1,2;
select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totalany into temp anytotal from zdiff_stats_by_dist_time_geo_kma where regime='Total' group by 1,2,3,4,5,6;


select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totaltypes into temp totalbytypes from zdiff_stats_by_dist_time_geo_kma where regime!='Total' group by 1,2,3,4,5,6;
...
select radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 and percent_of_bins=100 group by 1,2 order by 1,2;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinmax/10)*10+5 as grmaxz, regime, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime like 'S%' and numpts>0 group by 3,2 order by 1;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinstddev/3)*3 as stddev, regime, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime like 'S%' and numpts>5 group by 3,2 order by 3,2;
...
select radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 group by 1 order by 1;

-- As above, but by max PR Z categories
select radar_id, round(prmax/5)*5 as przcat, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 group by 1,2 order by 1,2;

-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, rangecat*50+25 as mean_range_km, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 and percent_of_bins=100 group by 1,2 order by 1,2;

-- Bias by site, height, and regime(s), for given gv source
select radar_id, height, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime like 'S_%' and numpts>0 and gvtype='GeoM' group by 1,2 order by 1,2;

-- Bias by site and regime
select radar_id, regime, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_z_diff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where numpts>25 group by 1,2 order by 1,2;
...
select gvtype, radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanpr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangv, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;

-- breakout by regime and 5 dBZ GR Z ranges
 select regime,  round((gvmean+2.5)/5)*5 as GV_dbz, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_z_diff, sum(numpts) as total, count(orbit) as cases from zdiff_stats_by_dist_time_geo_kma where numpts>25 group by 1,2 order by 1,2;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
 ...
 into temp dbzsums from zdiff_stats_by_dist_time_geo_kma where meandiff > -99.9
...
select a.radar_id, date_trunc('month',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_kma a, overpass_event b where regime='S_above' and numpts>5 and percent_of_bins=100 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id ='KAMX' group by 1,2 order by 1,2;

-- Vertical profiles of PR and GV mean Z:
select radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo_kma where rangecat<2 and regime like 'S_%' and numpts > 0 group by 1,2 order by 1,2;

select gvtype, radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo_kma where rangecat<2 group by 1,2,3 order by 2,1,3;

--Strat profile forPrlx, non-BB
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo_kma where rangecat<2 and regime in ('S_above','S_below') and numpts > 0 and radar_id < 'KWAJ' group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo_kma where rangecat<2 and regime in ('C_above','C_below') and numpts > 0 and radar_id < 'KWAJ' group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo_kma where rangecat<2 and regime in ('S_above','S_below') and numpts > 0 and radar_id < 'KWAJ' and orbit between 57821 and 63364 group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo_kma where rangecat<2 and regime in  ('C_above','C_below') and numpts > 0 and radar_id < 'KWAJ' and orbit between 57821 and 63364 group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select a.height, round((sum(a.prmean*a.numpts)/sum(a.numpts))*100)/100 as prc, round((sum(a.gvmean*a.numpts)/sum(a.numpts))*100)/100 as gvc, sum(a.numpts) as nc, round((sum(b.prmean*b.numpts)/sum(b.numpts))*100)/100 as prs, round((sum(b.gvmean*b.numpts)/sum(b.numpts))*100)/100 as gvs, sum(b.numpts) as ns, round((sum(c.prmean*c.numpts)/sum(c.numpts))*100)/100 as pr, round((sum(c.gvmean*c.numpts)/sum(c.numpts))*100)/100 as gv, sum(c.numpts) as n from zdiff_stats_by_dist_time_geo_kma a, zdiff_stats_by_dist_time_geo_kma b, zdiff_stats_by_dist_time_geo_kma c where a.rangecat<2 and a.regime in ('C_above','C_below') and a.numpts > 4 and b.numpts>4 and c.numpts > 4 and a.radar_id < 'KWAJ' and b.regime in ('S_above','S_below') and a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.height=c.height and a.orbit=c.orbit and a.radar_id=c.radar_id and a.rangecat = c.rangecat group by 1 order by 1;
...
select percent_of_bins, radar_id, orbit, sum(numpts) as n_conv into temp npts_conv from zdiff_stats_by_dist_time_geo_kma where regime like 'C_%' group by 1,2,3 order by 1,2,3;
select percent_of_bins, radar_id, orbit, sum(numpts) as n_strat into temp npts_strat from zdiff_stats_by_dist_time_geo_kma where regime like 'S_%' group by 1,2,3 order by 1,2,3;
...
select percent_of_bins, radar_id, orbit, ((sum(numpts)/250)+1)*250 as n_conv into temp area_conv from zdiff_stats_by_dist_time_geo_kma where regime like 'C_%' and height=3 group by 1,2,3 order by 1,2,3;
select percent_of_bins, radar_id, orbit, ((sum(numpts)/250)+1)*250 as n_strat into temp area_strat from zdiff_stats_by_dist_time_geo_kma where regime like 'S_%' and height=3 group by 1,2,3 order by 1,2,3;
-- compute mean bias, stdDev as function of area of precip for KMLB
select e.n_strat, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from zdiff_stats_by_dist_time_geo_kma c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%' and c.radar_id='KMLB' group by 1 order by 1;
...
select e.n_strat, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from zdiff_stats_by_dist_time_geo_kma c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%'  group by 1 order by 1;
 n_strat | biasgeo | stdev |  ngeo 
---------+---------+-------+--------
...
-- as above, site by site
 select e.n_strat, e.radar_id, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from zdiff_stats_by_dist_time_geo_kma c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%' group by 1,2 order by 2,1;
...
select a.percent_of_bins, a.rangecat, a.regime, a.radar_id, a.orbit, a.height into temp commontemp from zdiff_stats_by_dist_time_geo_kma a, zdiff_stats_by_dist_time_geo_kma18 b where a.rangecat<2 and a.radar_id < 'KWAJ' and a.numpts>4 and b.numpts>4 and a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.regime = b.regime and a.regime not in ('C_in', 'S_in', 'Total') and a.percent_of_bins=b.percent_of_bins;
drop table convtemp;
drop table strattemp;
drop table alltemp;

select a.height, round((sum(a.prmean*a.numpts)/sum(a.numpts))*100)/100 as prc, round((sum(a.gvmean*a.numpts)/sum(a.numpts))*100)/100 as gvc, sum(a.numpts) as nc into temp convtemp from zdiff_stats_by_dist_time_geo_kma a, commontemp d where a.regime in ('C_above','C_below')  and a.numpts > 4 and a.height=d.height and a.orbit=d.orbit and a.radar_id=d.radar_id and a.rangecat = d.rangecat and a.regime = d.regime group by 1 order by 1;

select b.height, round((sum(b.prmean*b.numpts)/sum(b.numpts))*100)/100 as prs, round((sum(b.gvmean*b.numpts)/sum(b.numpts))*100)/100 as gvs, sum(b.numpts) as ns into temp strattemp from zdiff_stats_by_dist_time_geo_kma b, commontemp d where b.regime in ('S_above','S_below') and b.numpts > 4 and d.height=b.height and d.orbit=b.orbit and d.radar_id=b.radar_id and d.rangecat = b.rangecat and d.regime = b.regime group by 1 order by 1;

select c.height, round((sum(c.prmean*c.numpts)/sum(c.numpts))*100)/100 as pr, round((sum(c.gvmean*c.numpts)/sum(c.numpts))*100)/100 as gv, sum(c.numpts) as n into temp alltemp from zdiff_stats_by_dist_time_geo_kma c, commontemp d where c.height=d.height and c.orbit=d.orbit and c.radar_id=d.radar_id and c.rangecat = d.rangecat and c.regime = d.regime and c.numpts > 4 group by 1 order by 1;
...
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsgeo from zdiff_stats_by_dist_time_geo_kma where regime='S_above' and numpts>5 and gvtype='GeoM' and rangecat<2 and percent_of_bins=100 group by 1,2 order by 1,2;

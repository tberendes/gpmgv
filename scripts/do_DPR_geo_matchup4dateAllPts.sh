...
################################################################################
#
#  do_DPR_geo_matchup4dateAllPts.sh    Morris/SAIC/GPM GV    April 2017
#
#
#  DESCRIPTION
#
#   Called by do_DPR_GeoMatchAllPts.sh, which generates the needed inputs.
...
#  SYNOPSIS
#
#   do_DPR_geo_matchup4dateAllPts.sh [OPTIONS]  YYMMDD  CONTROLFILE
#
#
#  OPTIONS/ARGUMENTS:
#
...
if [ $# != 2 ] 
  then
     ...
     echo "Usage: do_DPR_geo_matchup4dateAllPts.sh [-f[0|1]] YYMMDD CONTROLFILE"
     exit 1
fi
...
case "${FORCE_MATCH}"
  in
    ...
    *) echo "Invalid value ${FORCE_MATCH} for -f option in do_DPR_geo_matchup4dateAllPts.sh."\
       | tee -a $LOG_FILE
       exit 1;;
esac
echo "" | tee -a $LOG_FILE
...
case "${DO_RHI}"
  in
    0) BATFILE=polar2dprAllPts_${PARAMETER_SET}.bat ;;
    1) BATFILE=rhi2dprAllPts_${PARAMETER_SET}.bat ;;
    *) echo "Invalid value ${DO_RHI} for -r option in do_DPR_geo_matchup4dateAllPts.sh."\
       | tee -a $LOG_FILE
       exit 1;;
esac
...
if [ -s $CONTROLFILE ]
  then
    export CONTROLFILE          # IDL polar2dprAllPts_V.bat needs this
  else
    echo "ERROR in do_DPR_geo_matchup4dateAllPts.sh, control file $CONTROLFILE not found."\
     | tee -a $LOG_FILE
    exit 1
fi
...
echo "" | tee -a $LOG_FILE
echo "do_DPR_geo_matchup4dateAllPts.sh complete, exiting." | tee -a $LOG_FILE

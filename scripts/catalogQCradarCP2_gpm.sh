...
LOG_FILE=${LOG_DIR}/catalogQCradarRJNI.${rundate}.log
DB_LOG_FILE=${LOG_DIR}/catalogQCradarRJNISQL.log
runtime=`date -u`
                ...
                1CUF )  echo $pathless | grep '_' > /dev/null
                        if [ $? = 0 ]
                          then
                            ...
                            # check the file pattern, only take those with "_PPI"
                            echo $pathless | egrep 'CP2+[_a-z]*[_0-9]*_PPI.UF.*' > /dev/null
                            if [ $? = 1 ]
                              then
                                echo "Ignore 1CUF filename: $pathless"
                                return
                              else

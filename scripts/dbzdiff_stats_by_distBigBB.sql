...
create table dbzdiff_stats_by_dist_geo_bb (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
...
delete from dbzdiff_stats_by_dist_geo_bb;
\copy dbzdiff_stats_by_dist_geo_bb from '/data/tmp/StatsByDistToDB.unl' with delimiter '|' 

-- "Best" bias regime (stratiform above BB), broken out by site and range:
select radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo_bb where regime='S_above' and numpts>5 group by 1,2 order by 1,2;
-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, rangecat*50+25 as mean_range_km, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from dbzdiff_stats_by_dist_geo_bb where regime='S_above' and numpts>5 group by 1,2 order by 1,2;

-- Bias by site, height, and regime(s), for given gv source, for stratiform rain
select radar_id, height, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from dbzdiff_stats_by_dist_geo_bb where regime like 'S_above' and numpts>0 and gvtype='GeoM' group by 1,2 order by 1,2;


-- "Best" bias regime (stratiform above BB), broken out by site, GV type and range:
select gvtype, radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo_bb where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
 ...
 into temp dbzsums from dbzdiff_stats_by_dist_geo_bb where meandiff > -99.9
...
select a.radar_id, to_char(date_trunc('month',b.overpass_time at time zone 'UTC'),'YYYY-MM') as month, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiff, sum(numpts) as n_samples, count(distinct a.orbit) as rain_events from dbzdiff_stats_by_dist_geo_bb a, overpass_event b where regime='S_above' and numpts>5 and a.orbit=b.orbit and a.radar_id=b.radar_id group by 1,2 order by 1,2;

-- Vertical profiles of PR and GV mean Z:

select radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo_bb where rangecat<2 and numpts>5 group by 1,2 order by 1,2;

select gvtype, radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo_bb where rangecat<2 group by 1,2,3 order by 2,1,3;
...
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp stats2a55 from dbzdiff_stats_by_dist_geo_bb where regime='S_above' and numpts>5 and gvtype='2A55' and rangecat<2 group by 1,2 order by 1,2;

-- Case-by-case differences for REOR
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsreo from dbzdiff_stats_by_dist_geo_bb where regime='S_above' and numpts>5 and gvtype='REOR' and rangecat<2 group by 1,2 order by 1,2;

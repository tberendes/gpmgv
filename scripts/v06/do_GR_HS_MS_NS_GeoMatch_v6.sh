...
###############################################################################
#
# do_GR_HS_MS_NS_GeoMatch_v6.sh    Morris/SAIC/GPM GV    March 2016
#
...
#        the child script do_GR_HS_MS_NS_geo_matchup4date_v6.sh will skip
...
PPS_VERSION="V06A"   # specifies which PPS version of products to process
export PPS_VERSION
PARAMETER_SET=1  # set of polar2dpr_hs_ms_ns parameters (polar2dpr_hs_ms_ns.bat file) in use
...
export SWATH
# - Note that appstatus table entries for the child script do_GR_HS_MS_NS_geo_matchup4date_v6.sh
#   use the fixed app_id value 'geo_match_GRx3', not something based on $SWATH.

GEO_MATCH_VERSION=2.0     # current GRtoDPR_HS_MS_NS netCDF matchup file definition version, 
# must match version in gen_gr_hsmsms_geo_match_netcdf_v6.pro
...
# matchups by child script do_GR_HS_MS_NS_geo_matchup4date_v6.sh:
FORCE_MATCH=0
NPOL_SITE=""
DO_NPOL=0
...
LOG_FILE=${LOG_DIR}/do_GR_HS_MS_NS_GeoMatch_v6.${rundate}.log
export rundate
...
# produced by do_GR_HS_MS_NS_geo_matchup4date_v6.sh by examining its own log file,
# do_GR_HS_MS_NS_geo_matchup4date_v6.yymmdd.log, for the date yymmdd.  Parses the
...
#exit  # if uncommented, creates the control file for first date, and exits
    ...
    if [ -s $outfileall ]
      then
       # Call the IDL wrapper script, do_GR_HS_MS_NS_geo_matchup4date_v6.sh, to run
       # the IDL .bat files.  Let each of these deal with whether the yymmdd
       # has been done before.

        echo "" | tee -a $LOG_FILE
        start1=`date -u`
        echo "Calling do_GR_HS_MS_NS_geo_matchup4date_v6.sh $yymmdd on $start1" | tee -a $LOG_FILE
# default do_GR_HS_MS_NS_geo_matchup4date_v6.sh to set $FORCE_MATCH to 1 so every date is processed by default
#        ${BIN_DIR}/do_GR_HS_MS_NS_geo_matchup4date_v6.sh -f $FORCE_MATCH $yymmdd $outfileall
        ${BIN_DIR}/do_GR_HS_MS_NS_geo_matchup4date_v6.sh -f 1 $yymmdd $outfileall

        case $? in
          0 )
            echo ""
            echo "SUCCESS status returned from do_GR_HS_MS_NS_geo_matchup4date_v6.sh"\
             | tee -a $LOG_FILE
           ...
           # must be identically defined here and in do_GR_HS_MS_NS_geo_matchup4date_v6.sh
          ...
          1 )
            echo ""
            echo "FAILURE status returned from do_GR_HS_MS_NS_geo_matchup4date_v6.sh, quitting!"\
             | tee -a $LOG_FILE
            exit 1
          ;;
          2 )
            echo ""
            echo "REPEAT status returned from do_GR_HS_MS_NS_geo_matchup4date_v6.sh, do nothing!"\
             | tee -a $LOG_FILE
          ;;
        esac

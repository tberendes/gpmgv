...
################################################################################
#
#  do_GMI_geo_matchup4date_v5.sh    Morris/SAIC/GPM GV    May 2011
#
#
#  DESCRIPTION
#
...
#  SYNOPSIS
#
#   do_GMI_geo_matchup4date_v5.sh  CONTROLFILE
#
#
#  ARGUMENTS
#
#   YYMMDD - Mandatory argument, datestamp of the fully qualified pathname of
#            the IDL control file GMI_files_sites4geoMatch_v5.YYMMDD.txt, where
...
#  FILES
#
#   GMI_files_sites4geoMatch_v5.YYMMDD.txt (INPUT)
...
#  LOGS
#
#    Output for script run is logged to file do_GMI_geo_matchup4date_v5.yymmdd.log
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/do_GMI_geo_matchup4date_v5.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
THISRUNFILE=${TMP_DIR}/GMI_files_sites4geoMatch_v5.${THISRUN}.txt
LOG_FILE=${LOG_DIR}/do_GMI_geo_matchup4date_v5.${THISRUN}.log
...
CONTROLFILE=${TMP_DIR}/GMI_files_sites4geoMatch_v5.${THISRUN}.txt
if [ -s $CONTROLFILE ]
  then
...
#        exit 1
    ...
    echo "ERROR in do_GMI_geo_matchup4date_v5.sh, control file $CONTROLFILE not found."
    exit 1
fi

echo "" | tee -a $LOG_FILE
echo "do_GMI_geo_matchup4date_v5.sh complete, exiting." | tee -a $LOG_FILE
exit 0

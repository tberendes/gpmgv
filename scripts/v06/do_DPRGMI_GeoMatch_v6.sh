...
export PPS_VERSION
PARAMETER_SET=1  # default set of polar2dprgmi_v6 parameters (polar2dprgmi_v6 .bat file) in use
...
# must match version in gen_dprgmi_geo_match_netcdf_v6.pro
...
LOG_FILE=${LOG_DIR}/doDPRGMIGeoMatch4NewRainCases_v6.${PPS_VERSION}.${rundate}.log
...
# function finds matchup file names produced by IDL polar2dprgmi_v6 procedure, as
# listed in the do_DPRGMI_geo_matchup_catalog.yymmdd.txt file, in turn produced
# by do_DPRGMI_geo_matchup4date_v6.sh by examining the
...
while read thisdate
  do
   ...
   # append manner to 'outfileall', which is run-date-specific.
    ...
    outfileall=${CTL_DIR}/COMB_files_sites4geoMatch_v6.${yymmdd}.txt
...
#	            AND C.FILE1CUF NOT LIKE '%rhi%' \
    ...
    if [ -s $outfileall -a "$SKIP_MATCHUPS" = "0" ]
      then
       # Call the IDL wrapper script, do_DPRGMI_geo_matchup_v6.sh, to run
       # the IDL .bat file.  Let each of these deal with whether the yymmdd
       # has been done before.

        echo "" | tee -a $LOG_FILE
        start1=`date -u`
        echo "Calling do_DPRGMI_geo_matchup4date_v6.sh $yymmdd on $start1" | tee -a $LOG_FILE
        ${BIN_DIR}/do_DPRGMI_geo_matchup4date_v6.sh $yymmdd

        case $? in
          0 )
            echo ""
            echo "SUCCESS status returned from do_DPRGMI_geo_matchup4date_v6.sh"\
        	 | tee -a $LOG_FILE
           ...
           # must be identically defined here and in do_DPRGMI_matchup4date.sh
            DBCATALOGFILE=${TMP_DIR}/do_DPRGMI_geo_matchup_catalog_v6.${yymmdd}.txt
            if [ -s $DBCATALOGFILE ] 
              then
...
#                exit 1
          ...
          1 )
            echo ""
            echo "FAILURE status returned from do_DPRGMI_geo_matchup4date_v6.sh, quitting!"\
	     | tee -a $LOG_FILE
	    exit 1
          ;;
          2 )
            echo ""
            echo "REPEAT status returned from do_DPRGMI_geo_matchup4date_v6.sh, do nothing!"\
	     | tee -a $LOG_FILE
          ;;
        esac

...
################################################################################
#
#  do_GR_HS_MS_NS_geo_matchup4date_v6.sh    Morris/SAIC/GPM GV    April 2014
#
#
#  DESCRIPTION
#
#   Called by do_GR_HS_MS_NS_GeoMatch.sh, which generates the needed inputs.
...
#
#   polar2dpr_hs_ms_ns_V.bat or rhi2dpr_hs_ms_ns_V.bat (EXTERNAL)
...
#
#           do_GR_HS_MS_NS_geo_matchup4date_v6.yymmdd.log
#
...
#
#           do_GR_HS_MS_NS_geo_matchup.yymmdd.fatal.log
#
...
DBTEMPFILE=${TMP_DIR}/do_GR_HS_MS_NS_geo_matchup_dbtempfile
...
if [ $# != 2 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/do_GR_HS_MS_NS_geo_matchup4date_v6.${THISRUN}.fatal.log
     echo "FATAL: Exactly two non-option arguments required, $# given." | tee -a $LOG_FILE
     echo "Usage: do_GR_HS_MS_NS_geo_matchup4date_v6.sh [-f[0|1]] YYMMDD CONTROLFILE"
     exit 1
fi
...
LOG_FILE=${LOG_DIR}/do_GR_HS_MS_NS_geo_matchup4date_v6.${THISRUN}.log
...
case "${FORCE_MATCH}"
  in
    ...
    *) echo "Invalid value ${FORCE_MATCH} for -f option in do_GR_HS_MS_NS_geo_matchup4date_v6.sh."\
       | tee -a $LOG_FILE
       exit 1;;
esac
echo "" | tee -a $LOG_FILE
...
case "${DO_RHI}"
  in
    0) BATFILE=polar2dpr_hs_ms_ns_v6_${PARAMETER_SET}.bat ;;
    1) BATFILE=rhi2dpr_hs_ms_ns_${PARAMETER_SET}.bat ;;
    *) echo "Invalid value ${DO_RHI} for -r option in do_GR_HS_MS_NS_geo_matchup4date_v6.sh."\
       | tee -a $LOG_FILE
       exit 1;;
esac
...
if [ -s $CONTROLFILE ]
  then
    ...
    echo "ERROR in do_GR_HS_MS_NS_geo_matchup4date_v6.sh, control file $CONTROLFILE not found."\
     | tee -a $LOG_FILE
    exit 1
fi
...
DBCATALOGFILE=${TMP_DIR}/do_GR_HS_MS_NS_geo_matchup_catalog.${THISRUN}.txt
...
echo "" | tee -a $LOG_FILE
echo "do_GR_HS_MS_NS_geo_matchup4date_v6.sh complete, exiting." | tee -a $LOG_FILE

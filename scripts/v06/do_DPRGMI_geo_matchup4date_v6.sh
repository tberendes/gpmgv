...
################################################################################
#
#  do_DPRGMI_geo_matchup4date_v6.sh    Morris/SAIC/GPM GV    April 2014
#
#
#  DESCRIPTION
#
...
#
#   polar2dprgmi_v6_V.bat (EXTERNAL) - Batch file of IDL commands to be run, where
#                                   V is defined by the value of $PARAMETER_SET,
#                                   e.g., polar2dprgmi_v6_1.bat.
#
#
#  DATABASE
#
...
#                         value to define the version of the polar2dprgmi_v6 ".bat"
...
export IDL_PRO_DIR                # IDL polar2dprgmi_v6_V.bat needs this
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/do_DPRGMI_geo_matchup4date_v6.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUN=$1
CONTROLFILE=${CTL_DIR}/COMB_files_sites4geoMatch_v6.${THISRUN}.txt
THISRUN=`echo $CONTROLFILE | cut -f2 -d'.'`
LOG_FILE=${LOG_DIR}/do_DPRGMI_geo_matchup4date_v6.${THISRUN}.log
...
if [ -s $CONTROLFILE ]
  then
    #export THISRUN              # IDL polar2dprgmi_v6.bat needs this
    export CONTROLFILE          # IDL polar2dprgmi_v6.bat needs this
  else
    echo "ERROR in do_DPRGMI_geo_matchup4date_v6.sh, control file $CONTROLFILE not found."\
     | tee -a $LOG_FILE
    exit 1
fi
...
# polar2dprgmi_v6 batch file to use
if [ ! -s ${IDL_PRO_DIR}/polar2dprgmi_v6_${PARAMETER_SET}.bat ]
  then
     echo "" | tee -a $LOG_FILE
     echo "FATAL: Script ${IDL_PRO_DIR}/polar2dprgmi_v6_${PARAMETER_SET}.bat not found, " \
       | tee -a $LOG_FILE
...
if [ -s $CONTROLFILE ]   # legacy if check, moved error logic to top of script
  then
    ...
    echo "Calling IDL for yymmdd = ${THISRUN}, file = $CONTROLFILE" \
     | tee -a $LOG_FILE
    echo "using batch file ${IDL_PRO_DIR}/polar2dprgmi_v6_${PARAMETER_SET}.bat" \
     | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
       
    $IDL < ${IDL_PRO_DIR}/polar2dprgmi_v6_${PARAMETER_SET}.bat | tee -a $LOG_FILE 2>&1
   ...
   # do_DPRGMI_GeoMatch_from_ControlFiles.sh
    ...
    DBCATALOGFILE=${TMP_DIR}/do_DPRGMI_geo_matchup_catalog_v6.${THISRUN}.txt
...
echo "" | tee -a $LOG_FILE
echo "do_DPRGMI_geo_matchup4date_v6.sh complete, exiting." | tee -a $LOG_FILE
exit 0

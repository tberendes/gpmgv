...
create table dbzdiff_stats_by_dist_geo_225 (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
...
create table dbzdiff_stats_by_dist_geo_s2ku_225 (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totalany into temp anytotal from dbzdiff_stats_by_dist_geo_225 where regime='Total' group by 1,2,3,4,5,6;

select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totaltypes into temp totalbytypes from dbzdiff_stats_by_dist_geo_225 where regime!='Total' group by 1,2,3,4,5,6;
...
delete from dbzdiff_stats_by_dist_geo_225;
\copy dbzdiff_stats_by_dist_geo_225 from '/data/gpmgv/tmp/StatsByDistToDBGeo_Pct100_v7_225_DefaultS.unl' with delimiter '|' 
delete from dbzdiff_stats_by_dist_geo_s2ku_225;
\copy dbzdiff_stats_by_dist_geo_s2ku_225 from '/data/gpmgv/tmp/StatsByDistToDBGeo_Pct100_v7_225_S2Ku.unl' with delimiter '|' 

-- "Best" bias regime (stratiform above BB), broken out by site and range:
select radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 and percent_of_bins=95 group by 1,2 order by 1,2;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinmax/10)*10+5 as grmaxz, regime, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime like 'S%' and numpts>0 group by 3,2 order by 1;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinstddev/3)*3 as stddev, regime, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime like 'S%' and numpts>5 group by 3,2 order by 3,2;
...
select radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 group by 1 order by 1;

-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, rangecat*50+25 as mean_range_km, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 and percent_of_bins=95 group by 1,2 order by 1,2;

-- Bias by site, height, and regime(s), for given gv source
select radar_id, height, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime like 'S_%' and numpts>0 and gvtype='GeoM' group by 1,2 order by 1,2;


-- "Best" bias regime (stratiform above BB), broken out by site, GV type and range:
select gvtype, radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanpr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangv, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
 ...
 into temp dbzsums from dbzdiff_stats_by_dist_geo_225 where meandiff > -99.9
...
select a.radar_id, date_trunc('month',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from dbzdiff_stats_by_dist_geo_225 a, overpass_event b where regime='S_above' and numpts>5 and percent_of_bins=95 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id ='KAMX' group by 1,2 order by 1,2;

-- Vertical profiles of PR and GV mean Z:
select radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo_225 where rangecat<2 and regime like 'S_%' and numpts > 0 group by 1,2 order by 1,2;

select gvtype, radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo_225 where rangecat<2 group by 1,2,3 order by 2,1,3;

--Strat profile for V7, non-BB
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo_225 where rangecat<2 and regime in ('S_above','S_below') and numpts > 0 and radar_id < 'KWAJ' group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from dbzdiff_stats_by_dist_geo_225 where rangecat<2 and regime in ('C_above','C_below') and numpts > 0 and radar_id < 'KWAJ' group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select a.height, round((sum(a.prmean*a.numpts)/sum(a.numpts))*100)/100 as prc, round((sum(a.gvmean*a.numpts)/sum(a.numpts))*100)/100 as gvc, sum(a.numpts) as nc, round((sum(b.prmean*b.numpts)/sum(b.numpts))*100)/100 as prs, round((sum(b.gvmean*b.numpts)/sum(b.numpts))*100)/100 as gvs, sum(b.numpts) as ns, round((sum(c.prmean*c.numpts)/sum(c.numpts))*100)/100 as pr, round((sum(c.gvmean*c.numpts)/sum(c.numpts))*100)/100 as gv, sum(c.numpts) as n from dbzdiff_stats_by_dist_geo_225 a, dbzdiff_stats_by_dist_geo_225 b, dbzdiff_stats_by_dist_geo_225 c where a.rangecat<2 and a.regime in ('C_above','C_below') and a.numpts > 4 and b.numpts>4 and c.numpts > 4 and a.radar_id < 'KWAJ' and b.regime in ('S_above','S_below') and a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.height=c.height and a.orbit=c.orbit and a.radar_id=c.radar_id and a.rangecat = c.rangecat group by 1 order by 1;
...
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsgeo from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 and gvtype='GeoM' and rangecat<2 group by 1,2 order by 1,2;
          
-- Case-by-case differences for 2A55
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp stats2a55 from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 and percent_of_bins=95 and rangecat<2 group by 1,2 order by 1,2;

-- Case-by-case differences for REOR
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsreo from dbzdiff_stats_by_dist_geo_225 where regime='S_above' and numpts>5 and gvtype='REOR' and rangecat<2 group by 1,2 order by 1,2;

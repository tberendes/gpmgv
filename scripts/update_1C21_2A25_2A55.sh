...
#   NEXRAD sites are listed in the well-known filename whose YYMMDD date part
...
#
#  LOGS
#    Output for script run is logged to file grid_1C21_2A25_2A55.log
...
#    grid_1C21_2A25_2A55.yymmdd.fatal.log in $LOG_DIR, where 'yymmdd' is for the
...
if [ $# != 1 ] 
  then
     ...
     LOG_FILE=${LOG_DIR}/grid_1C21_2A25_2A55.${THISRUN}.fatal.log
     echo "FATAL: Exactly one argument required, $# given." | tee -a $LOG_FILE
     exit 1
fi

THISRUNFILE=$1
LOG_FILE=${LOG_DIR}/grid_1C21_2A25_2A55.log
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT status FROM appstatus \
 WHERE app_id = 'ncgridPRGV' AND datestamp = '${THISRUN}';" \
...
if [ -s ${DBTEMPFILE} ]
  then
     ...
     echo "INSERT INTO appstatus (app_id, datestamp, status) VALUES \
       ('ncgridPRGV', '${THISRUN}', '$UNTRIED');" | psql -a -d gpmgv \
       | tee -a $LOG_FILE 2>&1
fi
...
echo "\pset format u \t \o ${DBTEMPFILE} \\\ SELECT datestamp FROM appstatus \
      WHERE app_id = 'ncgridPRGV' AND status = '$UNTRIED' \
...
#if [ ! -s ${IDL_PRO_DIR}/get_pr_and_gv_ncgrids.bat ]
if [ ! -s ${IDL_PRO_DIR}/update_pr_ncgrids.bat ]
  then
#     echo "Script ${IDL_PRO_DIR}/get_pr_and_gv_ncgrids.bat not found, exiting" \
#       | tee -a $LOG_FILE
     echo "Script ${IDL_PRO_DIR}/update_pr_ncgrids.bat not found, exiting" \
       | tee -a $LOG_FILE
...
for yymmdd in `cat $DATES2GRID`
  do
	...
	echo "UPDATE appstatus SET ntries = ntries + 1 \
	      WHERE app_id = 'ncgridPRGV' AND datestamp = '$RUNDATE';" \
	 | psql -a -d gpmgv  | tee -a $LOG_FILE 2>&1
	echo "" | tee -a $LOG_FILE
	echo "=============================================" | tee -a $LOG_FILE
...
#	$IDL < ${IDL_PRO_DIR}/get_pr_and_gv_ncgrids.bat | tee -a $LOG_FILE 2>&1
	$IDL < ${IDL_PRO_DIR}/update_pr_ncgrids.bat | tee -a $LOG_FILE 2>&1
...
echo "" | tee -a $LOG_FILE
echo "grid_1C21_2A25_2A55.sh complete, exiting." | tee -a $LOG_FILE
exit 0

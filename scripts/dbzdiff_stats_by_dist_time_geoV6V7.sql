...
create table zdiff_stats_by_dist_time_geo (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
...
create table zdiff_stats_by_dist_time_geo_s2ku (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
delete from zdiff_stats_by_dist_time_geo;
\copy zdiff_stats_by_dist_time_geo from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_DefaultS.unl' with delimiter '|' 
delete from zdiff_stats_by_dist_time_geo_s2ku;
\copy zdiff_stats_by_dist_time_geo_s2ku from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_S2Ku.unl' with delimiter '|' 
\copy zdiff_stats_by_dist_time_geo from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_KMLB_DefaultS.unl' with delimiter '|' 


-- CREATE TABLES FOR THE V7 DATA

create table zdiff_stats_by_dist_time_geoV7 (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
create table zdiff_stats_by_dist_time_geo_s2kuV7 (
   percent_of_bins int,
   rangecat int,
   gvtype char(4),
   regime character varying(10),
...
-- if loading all new data, do the following 2-4 steps:

delete from zdiff_stats_by_dist_time_geoV7;
\copy zdiff_stats_by_dist_time_geoV7 from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_V7_DefaultS.unl' with delimiter '|' 
delete from zdiff_stats_by_dist_time_geo_s2kuV7;
\copy zdiff_stats_by_dist_time_geo_s2kuV7 from '/data/gpmgv/bulkStats/StatsByDistToDBGeo_Pct100_V7_S2Ku.unl' with delimiter '|' 

-- if just adding new data to the tables, do these instead:

\copy zdiff_stats_by_dist_time_geoV7 from '/data/gpmgv/tmp/StatsByDistTimeToDBGeo_Pct100_v7_fromOrb90397_DefaultS.unl' with delimiter '|'

\copy zdiff_stats_by_dist_time_geo_s2kuV7 from '/data/gpmgv/tmp/StatsByDistTimeToDBGeo_Pct100_v7_fromOrb90397_S2Ku.unl' with delimiter '|'

--------------------------------------------------------------------------------


-- differences and profiles by height/percent:
select height, percent_of_bins, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanprv, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangr, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime like 'C_%' and numpts>5 and radar_id='KMLB' group by 1,2 order by 1,2;
select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totalany into temp anytotal from zdiff_stats_by_dist_time_geo where regime='Total' group by 1,2,3,4,5,6;


select percent_of_bins, rangecat, gvtype, radar_id, orbit, height, sum(numpts) as totaltypes into temp totalbytypes from zdiff_stats_by_dist_time_geo where regime!='Total' group by 1,2,3,4,5,6;
...
select radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime='S_above' and numpts>5 and percent_of_bins=100 group by 1,2 order by 1,2;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinmax/10)*10+5 as grmaxz, regime, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime like 'S%' and numpts>0 group by 3,2 order by 1;

select round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round(gvbinstddev/3)*3 as stddev, regime, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime like 'S%' and numpts>5 group by 3,2 order by 3,2;
...
select radar_id, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime='S_above' and numpts>5 group by 1 order by 1;

-- As above, but output to HTML table
\o /data/tmp/BiasByDistance.html \\select radar_id, rangecat*50+25 as mean_range_km, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as mean_diff_dbz, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime='S_above' and numpts>5 and percent_of_bins=100 group by 1,2 order by 1,2;

-- Bias by site, height, and regime(s), for given gv source
select radar_id, height, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiffavg, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime like 'S_%' and numpts>0 and gvtype='GeoM' group by 1,2 order by 1,2;


-- "Best" bias regime (stratiform above BB), broken out by site, GV type and range:
select gvtype, radar_id, rangecat*50+25 as meanrangekm, round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, round((sum(prmean*numpts)/sum(numpts))*100)/100 as meanpr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as meangv, sum(numpts) as total from zdiff_stats_by_dist_time_geo where regime='S_above' and numpts>5 group by 1,2,3 order by 1,2,3;
...
select sum(meandiff*numpts) as w, sum(diffstddev*numpts) as s,
 ...
 into temp dbzsums from zdiff_stats_by_dist_time_geo where meandiff > -99.9
...
select a.radar_id, date_trunc('month',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo a, overpass_event b where regime='S_above' and numpts>5 and percent_of_bins=100 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id ='KAMX' group by 1,2 order by 1,2;

-- Vertical profiles of PR and GV mean Z:
select radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo where rangecat<2 and regime like 'S_%' and numpts > 0 group by 1,2 order by 1,2;

select gvtype, radar_id, height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo where rangecat<2 group by 1,2,3 order by 2,1,3;

--Strat profile forPrlx, non-BB
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo where rangecat<2 and regime in ('S_above','S_below') and numpts > 0 and radar_id < 'KWAJ' group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo where rangecat<2 and regime in ('C_above','C_below') and numpts > 0 and radar_id < 'KWAJ' group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo where rangecat<2 and regime in ('S_above','S_below') and numpts > 0 and radar_id < 'KWAJ' and orbit between 57821 and 63364 group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select height, round((sum(prmean*numpts)/sum(numpts))*100)/100 as pr, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as gv, sum(numpts) as n from zdiff_stats_by_dist_time_geo where rangecat<2 and regime in  ('C_above','C_below') and numpts > 0 and radar_id < 'KWAJ' and orbit between 57821 and 63364 group by 1 order by 1;
 height |  pr   |  gv   |   n   
--------+-------+-------+-------
...
select a.height, round((sum(a.prmean*a.numpts)/sum(a.numpts))*100)/100 as prc, round((sum(a.gvmean*a.numpts)/sum(a.numpts))*100)/100 as gvc, sum(a.numpts) as nc, round((sum(b.prmean*b.numpts)/sum(b.numpts))*100)/100 as prs, round((sum(b.gvmean*b.numpts)/sum(b.numpts))*100)/100 as gvs, sum(b.numpts) as ns, round((sum(c.prmean*c.numpts)/sum(c.numpts))*100)/100 as pr, round((sum(c.gvmean*c.numpts)/sum(c.numpts))*100)/100 as gv, sum(c.numpts) as n from zdiff_stats_by_dist_time_geo a, zdiff_stats_by_dist_time_geo b, zdiff_stats_by_dist_time_geo c where a.rangecat<2 and a.regime in ('C_above','C_below') and a.numpts > 4 and b.numpts>4 and c.numpts > 4 and a.radar_id < 'KWAJ' and b.regime in ('S_above','S_below') and a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.height=c.height and a.orbit=c.orbit and a.radar_id=c.radar_id and a.rangecat = c.rangecat group by 1 order by 1;
...
select percent_of_bins, radar_id, orbit, ((sum(numpts)/250)+1)*250 as n_conv into temp area_conv from zdiff_stats_by_dist_time_geo where regime like 'C_%' and height=3 group by 1,2,3 order by 1,2,3;
select percent_of_bins, radar_id, orbit, ((sum(numpts)/250)+1)*250 as n_strat into temp area_strat from zdiff_stats_by_dist_time_geo where regime like 'S_%' and height=3 group by 1,2,3 order by 1,2,3;
-- compute mean bias, stdDev as function of area of precip for KMLB
select e.n_strat, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from zdiff_stats_by_dist_time_geo c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%' and c.radar_id='KMLB' group by 1 order by 1;
...
select e.n_strat, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from zdiff_stats_by_dist_time_geo c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%'  group by 1 order by 1;
 n_strat | biasgeo | stdev |  ngeo 
---------+---------+-------+--------
...
-- as above, site by site
 select e.n_strat, e.radar_id, round((sum(c.meandiff*c.numpts)/sum(c.numpts))*100)/100 as biasgeo,  round((sum(c.diffstddev*c.numpts)/sum(c.numpts))*100)/100 as stdev, sum(c.numpts) as ngeo from zdiff_stats_by_dist_time_geo c, area_strat e where c.percent_of_bins=e.percent_of_bins and c.percent_of_bins=100 and c.orbit=e.orbit and c.radar_id=e.radar_id and c.regime like 'S_%' group by 1,2 order by 2,1;
...
-- DIFFERENCES FOR V6 VS. V7

-- get a common set of samples between V6 and V7
drop table commontemp;
select a.percent_of_bins, a.rangecat, a.regime, a.radar_id, a.orbit, a.height into temp commontemp from zdiff_stats_by_dist_time_geo a, zdiff_stats_by_dist_time_geoV7 b where a.rangecat<2 and a.radar_id < 'KWAJ' and a.numpts>4 and b.numpts>4 and a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.regime = b.regime and a.regime not in ('C_in', 'S_in', 'Total') and a.percent_of_bins=b.percent_of_bins;
drop table convtemp;
drop table strattemp;
drop table alltemp;

select a.height, round((sum(a.prmean*a.numpts)/sum(a.numpts))*100)/100 as prc, round((sum(a.gvmean*a.numpts)/sum(a.numpts))*100)/100 as gvc, sum(a.numpts) as nc into temp convtemp from zdiff_stats_by_dist_time_geo a, commontemp d where a.regime in ('C_above','C_below')  and a.numpts > 4 and a.height=d.height and a.orbit=d.orbit and a.radar_id=d.radar_id and a.rangecat = d.rangecat and a.regime = d.regime group by 1 order by 1;

select b.height, round((sum(b.prmean*b.numpts)/sum(b.numpts))*100)/100 as prs, round((sum(b.gvmean*b.numpts)/sum(b.numpts))*100)/100 as gvs, sum(b.numpts) as ns into temp strattemp from zdiff_stats_by_dist_time_geo b, commontemp d where b.regime in ('S_above','S_below') and b.numpts > 4 and d.height=b.height and d.orbit=b.orbit and d.radar_id=b.radar_id and d.rangecat = b.rangecat and d.regime = b.regime group by 1 order by 1;

select c.height, round((sum(c.prmean*c.numpts)/sum(c.numpts))*100)/100 as pr, round((sum(c.gvmean*c.numpts)/sum(c.numpts))*100)/100 as gv, sum(c.numpts) as n into temp alltemp from zdiff_stats_by_dist_time_geo c, commontemp d where c.height=d.height and c.orbit=d.orbit and c.radar_id=d.radar_id and c.rangecat = d.rangecat and c.regime = d.regime and c.numpts > 4 group by 1 order by 1;
...
select a.radar_id, round((sum(a.meandiff*a.numpts)/sum(a.numpts))*100)/100 as meandiff_V6, sum(a.numpts) as n_V6, round((sum(b.meandiff*b.numpts)/sum(b.numpts))*100)/100 as meandiff_V7, sum(b.numpts) as n_V7 from zdiff_stats_by_dist_time_geo a, zdiff_stats_by_dist_time_geoV7 b, commontemp c where  a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.regime = b.regime and a.height=c.height and a.orbit=c.orbit and a.radar_id=c.radar_id and a.rangecat = c.rangecat and a.regime = c.regime and a.percent_of_bins=b.percent_of_bins and a.percent_of_bins=c.percent_of_bins and a.percent_of_bins=100 and a.regime='S_above' group by 1 order by 1;
 radar_id | meandiff_v6 | n_v6  | meandiff_v7 | n_v7  
----------+-------------+-------+-------------+-------
 KAMX     |       -0.78 |  5160 |       -0.79 |  5227
 KBMX     |        -1.6 | 32555 |        -1.6 | 32642
 KBRO     |       -1.81 |  4634 |        -1.8 |  4736
 KBYX     |       -1.03 |  2842 |       -1.03 |  2835
 KCLX     |       -1.91 | 18356 |       -1.89 | 18367
 KCRP     |       -0.39 |  5771 |       -0.37 |  5897
 KDGX     |       -1.24 | 15960 |       -1.26 | 15875
 KEVX     |       -1.17 | 14925 |       -1.17 | 15000
 KFWS     |       -0.64 | 20976 |       -0.65 | 21320
 KGRK     |        0.57 | 12921 |         0.6 | 12972
 KHGX     |       -1.09 |  8637 |        -1.1 |  8832
 KHTX     |        -0.9 | 26922 |       -0.87 | 26922
 KJAX     |       -2.38 |  8995 |       -2.38 |  9008
 KJGX     |       -1.18 | 15620 |        -1.2 | 15586
 KLCH     |       -1.48 | 12149 |       -1.47 | 12114
 KLIX     |        -1.4 | 15978 |        -1.4 | 15941
 KMLB     |        -0.3 |  8014 |       -0.29 |  8110
 KMOB     |       -0.51 | 16819 |       -0.58 | 16774
 KSHV     |       -2.15 | 15142 |       -2.13 | 15246
 KTBW     |       -1.42 | 10207 |       -1.37 | 10309
 KTLH     |       -1.77 | 11856 |       -1.74 | 11623
(21 rows)

-- as above, but Convective above BB
 radar_id | meandiff_v6 | n_v6  | meandiff_v7 | n_v7 
----------+-------------+-------+-------------+------
 KAMX     |       -0.32 |  2058 |       -0.45 | 1965
 KBMX     |       -1.38 | 10282 |       -1.48 | 9972
 KBRO     |       -2.12 |  1587 |       -2.24 | 1510
 KBYX     |       -0.81 |  1428 |       -0.88 | 1382
 KCLX     |       -1.62 |  5608 |       -1.73 | 5348
 KCRP     |       -0.15 |  2491 |       -0.28 | 2347
 KDGX     |       -1.02 |  7135 |       -1.05 | 7099
 KEVX     |       -0.69 |  7237 |       -0.75 | 6953
 KFWS     |       -0.81 |  8132 |       -0.86 | 8166
 KGRK     |        0.46 |  4103 |        0.32 | 3981
 KHGX     |          -1 |  3931 |       -1.11 | 3798
 KHTX     |       -0.38 |  8799 |       -0.49 | 8721
 KJAX     |       -2.18 |  2852 |       -2.29 | 2869
 KJGX     |       -1.54 |  5479 |       -1.62 | 5318
 KLCH     |       -1.08 |  7749 |       -1.24 | 7884
 KLIX     |       -0.91 |  7512 |       -1.01 | 7412
 KMLB     |        0.06 |  4160 |          -0 | 4014
 KMOB     |        0.23 |  6341 |        0.25 | 6474
 KSHV     |          -2 |  9096 |       -2.08 | 8713
 KTBW     |       -0.78 |  4715 |       -0.86 | 4675
 KTLH     |       -1.75 |  2884 |       -1.78 | 2560
(21 rows)

-- as above, but Convective below BB
 radar_id | meandiff_v6 | n_v6 | meandiff_v7 | n_v7 
----------+-------------+------+-------------+------
 KAMX     |       -1.14 | 2755 |       -0.74 | 2449
 KBMX     |       -1.11 | 3573 |       -1.08 | 3106
 KBRO     |       -2.11 | 1142 |       -2.13 | 1046
 KBYX     |        -1.8 | 2677 |       -1.64 | 2337
 KCLX     |       -1.58 | 3617 |       -1.39 | 3353
 KCRP     |        -1.4 | 1479 |       -1.28 | 1350
 KDGX     |       -0.58 | 4730 |       -0.37 | 4315
 KEVX     |        0.11 | 3544 |         0.3 | 3133
 KFWS     |       -0.21 | 2265 |       -0.13 | 1980
 KGRK     |        1.03 | 3010 |        1.12 | 2874
 KHGX     |        0.32 | 1694 |        0.49 | 1502
 KHTX     |         0.2 | 2909 |        0.35 | 2747
 KJAX     |       -0.96 | 2558 |       -0.55 | 2240
 KJGX     |        -0.6 | 2690 |        -0.5 | 2521
 KLCH     |       -1.01 | 4709 |       -0.71 | 4253
 KLIX     |        -0.7 | 5622 |       -0.38 | 5055
 KMLB     |        0.29 | 2303 |        0.46 | 2095
 KMOB     |       -0.87 | 4168 |       -0.73 | 3695
 KSHV     |       -1.78 | 2647 |       -1.64 | 2460
 KTBW     |       -1.24 | 4256 |       -0.89 | 3805
 KTLH     |       -1.98 | 1402 |       -1.85 | 1269
(21 rows)

-- as above, but Convective below BB, 1.5 km level only
 radar_id | meandiff_v6 | n_v6 | meandiff_v7 | n_v7 
----------+-------------+------+-------------+------
 KAMX     |       -1.16 | 1958 |       -0.78 | 1581
 KBMX     |       -1.05 | 3127 |       -1.01 | 2630
 KBRO     |       -1.83 |  626 |       -1.94 |  511
 KBYX     |        -1.9 | 1799 |       -1.75 | 1434
 KCLX     |       -1.43 | 2692 |       -1.26 | 2232
 KCRP     |       -1.41 |  852 |        -1.2 |  686
 KDGX     |       -0.52 | 3944 |       -0.33 | 3548
 KEVX     |        0.19 | 2948 |         0.4 | 2515
 KFWS     |       -0.13 | 1781 |       -0.07 | 1510
 KGRK     |        1.19 | 2296 |        1.29 | 1984
 KHGX     |        0.25 |  995 |        0.51 |  789
 KHTX     |        0.24 | 2825 |         0.4 | 2650
 KJAX     |       -0.87 | 1985 |       -0.41 | 1626
 KJGX     |       -0.48 | 2150 |       -0.36 | 1958
 KLCH     |       -0.75 | 3436 |       -0.39 | 2873
 KLIX     |       -0.44 | 4186 |       -0.08 | 3580
 KMLB     |        0.63 | 1652 |        0.96 | 1344
 KMOB     |       -0.73 | 3256 |       -0.48 | 2731
 KSHV     |       -1.59 | 2017 |       -1.44 | 1824
 KTBW     |       -1.22 | 3184 |       -0.81 | 2714
 KTLH     |       -2.09 | 1011 |       -1.88 |  841
(21 rows)
...
-- as above, but all Convective rain type/levels
 radar_id | meandiff_v6 | n_v6  | meandiff_v7 | n_v7  
----------+-------------+-------+-------------+-------
 KAMX     |       -0.79 |  4813 |       -0.61 |  4414
 KBMX     |       -1.31 | 13855 |       -1.39 | 13078
 KBRO     |       -2.12 |  2729 |       -2.19 |  2556
 KBYX     |       -1.46 |  4105 |       -1.36 |  3719
 KCLX     |        -1.6 |  9225 |        -1.6 |  8701
 KCRP     |       -0.62 |  3970 |       -0.65 |  3697
 KDGX     |       -0.85 | 11865 |       -0.79 | 11414
 KEVX     |       -0.43 | 10781 |       -0.42 | 10086
 KFWS     |       -0.68 | 10397 |       -0.72 | 10146
 KGRK     |         0.7 |  7113 |        0.66 |  6855
 KHGX     |        -0.6 |  5625 |       -0.65 |  5300
 KHTX     |       -0.24 | 11708 |       -0.29 | 11468
 KJAX     |        -1.6 |  5410 |       -1.53 |  5109
 KJGX     |       -1.23 |  8169 |       -1.26 |  7839
 KLCH     |       -1.06 | 12458 |       -1.05 | 12137
 KLIX     |       -0.82 | 13134 |       -0.76 | 12467
 KMLB     |        0.15 |  6463 |        0.15 |  6109
 KMOB     |       -0.21 | 10509 |       -0.11 | 10169
 KSHV     |       -1.95 | 11743 |       -1.98 | 11173
 KTBW     |          -1 |  8971 |       -0.87 |  8480
 KTLH     |       -1.82 |  4286 |        -1.8 |  3829
(21 rows)



-- All regimes broken out by site and regime, V6 vs. Prlx:
select  a.rangecat, a.regime, a.radar_id, a.orbit, a.height into temp commontemp from zdiff_stats_by_dist_time_geo a, zdiff_stats_by_dist_time_geo b where a.rangecat<2 and a.radar_id < 'KWAJ' and a.numpts>4 and b.numpts>4 and a.height=b.height and a.orbit=b.orbit and a.radar_id=b.radar_id and a.rangecat = b.rangecat and a.regime = b.regime and a.regime not in ('Total');
...
select radar_id, orbit,round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total into temp statsgeo from zdiff_stats_by_dist_time_geo where regime='S_above' and numpts>5 and gvtype='GeoM' and rangecat<2 and percent_of_bins=100 group by 1,2 order by 1,2;
...
\t \a \f '|' \o /tmp/event_best_diffs_Pct100_Pts25_v7.txt \\select a.radar_id, date_trunc('day',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geov7 a, overpass_event b where regime='S_above' and numpts>25 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id not in ('RGSN','DARW','KWAJ') and percent_of_bins=100 group by 1,2 order by 1,2;

\f '|' \o /tmp/event_best_diffs_Pct100_Pts25_v7s2ku.txt \\select a.radar_id, date_trunc('day',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meanmeandiff, sum(numpts) as total from zdiff_stats_by_dist_time_geo_s2kuv7 a, overpass_event b where regime='S_above' and numpts>25 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id not in ('RGSN','DARW','KWAJ') and percent_of_bins=100 group by 1,2 order by 1,2;


select a.radar_id, date_trunc('day',b.overpass_time at time zone 'UTC'), round((sum(meandiff*numpts)/sum(numpts))*100)/100 as meandiff, round((sum(gvmean*numpts)/sum(numpts))*100)/100 as mean_gv_dbz, round((max(gvmax)*100)/100) as gvmax, round((max(prmax)*100)/100) as prmax, sum(numpts) as total from zdiff_stats_by_dist_time_geo a, overpass_event b where regime='S_above' and numpts>25 and a.orbit=b.orbit and a.radar_id=b.radar_id and a.radar_id not in ('RGSN','DARW') group by 1,2 order by 1,2;

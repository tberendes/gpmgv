...
# file to hold the 2A23 pathnames extracted from the get_PPS_CS_data log file
    filepaths2A23=${DATA_DIR}/tmp/filepaths2A23_temp.txt

# find the pathnames of the 2A23 files listed as downloaded in this log file
    grep 2A23 $MIR_LOG_FILE | grep orbit_subset | grep -v INSERT | cut -f2 -d '>' \
      | sed 's/ `//' | sed "s/'//" | tee $filepaths2A23

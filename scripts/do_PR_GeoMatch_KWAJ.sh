...
###############################################################################
#
# do_PR_GeoMatch_KWAJ.sh    Morris/SAIC/GPM GV    Feb 2015
#
# Wrapper to do PR-GV NetCDF geometric matchups for 1C21/2A25/2B31/1CUF files
# already received and cataloged, for cases meeting predefined criteria.  Only
# considers PR files with the new GPM-era file paths/names.
#
...
#
# 2/2015      Morris         Created from do_PR_GeoMatch.sh.
#
###############################################################################
...
LOG_FILE=${LOG_DIR}/doGeoMatch4NewRainCasesKWAJ.${rundate}.log
export rundate
...
YYMMDD=$1
MATCHUP_LOG=${LOG_DIR}/do_geo_matchup4dateKWAJ.${YYMMDD}.log
DBCATALOGFILE=$2
...
#    rowpost="|${PR_VERSION}|${PARAMETER_SET}|2.1|1|${INSTRUMENT_ID}"
    rowpost="|${PPS_VERSION}|${PARAMETER_SET}|${GEO_MATCH_VERSION}|1|${INSTRUMENT_ID}|${SAT_ID}|NS"
    gzfile=`ls ${ncfile}\.gz`
    if [ $? = 0 ]
      then
   ...
   # for RGSN's mapping to two subsets. Morris, 12/2008
    ...
    DBOUT2=`psql -a -A -t -o $filelist  -d gpmgv -c "select \
     ...
     where cast(nominal at time zone 'UTC' as date) = '${thisdate}' \
       ...
       and c.radar_id = 'KWAJ' and c.subset='${SUBSET}' \

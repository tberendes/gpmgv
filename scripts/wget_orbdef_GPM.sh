...
#    under the top level directory GEO_DATA defined in this script.  The
#    internal function extract_orbits() extracts orbit number and orbit start
#    and end datetimes from each downloaded orbit definition file, formats the
#    data for the PostGRESQL database, and loads the data to the table
#    'gpm_orbits' in the 'gpmgv' database.
#
...
#      PostGRESQL database 'gpmgv', and INSERT privilege on table "gpm_orbits". 
...
ctdate2=`offset_date $today -2`
#  Ditto, for 28 days back
ctdate1=`offset_date $today -28`

#  Trim date string to use a 2-digit year, as in DB timestamp convention
yymmdd=`echo $ctdate | cut -c 3-8`

echo "Time is $now UTC, getting files for date $yymmdd" | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE
  ...
  do
    ...
    if [ -s $FILES2DO ]
      then
        ...
        echo "" | tee -a $LOG_FILE
        echo "Calling extract_orbits $FILES2DO" | tee -a $LOG_FILE
        echo "" | tee -a $LOG_FILE

...
; All Rights Reserved.
;
; polar2dpr_hs_ms_ns_snow.pro          Morris/SAIC/GPM_GV      Feb 2016
;
; DESCRIPTION
; -----------
...
; rays for the 3 scan (swath) types (HS, MS, NS), and in the vertical by the
...
;
; NOTE: THE CODE CONTAINED IN THIS FILE IS NOT THE COMPLETE polar2dpr_hs_ms_ns
;       PROCEDURE.  THE REMAINING CODE IN THE FULL PROCEDURE IS CONTAINED IN
;       THE FILE "polar2dpr_hs_ms_ns_resampling.pro", WHICH IS INCORPORATED INTO
...
;   3) PROCEDURE polar2dpr_hs_ms_ns (this file, with polar2dpr_hs_ms_ns_resampling.pro
...
PRO polar2dpr_hs_ms_ns_snow, control_file, range_threshold_km, GPM_ROOT=gpmroot, $
...
;   DPR_scantype = parsed[6]        ; HS, MS, or NS
  ...
  ; Call gen_gr_hsmsns_geo_match_netcdf_snow with the option to only get current file version
  ; so that it can become part of the matchup file name
   throwaway = gen_gr_hsmsns_geo_match_netcdf_snow( GEO_MATCH_VERS=matchup_file_version )
...
;   idx_uniq_elevs = UNIQ(elev_angle)
  ...
  ; get the numbers of in-range footprints for each combo
   ...
   for swathID = 0, N_ELEMENTS(swathIDs)-1 do begin
     ; get the group structure for the specified scantype, tags vary by swath
      ...
      CASE STRUPCASE(DPR_scantype) OF
         ...
         'MS' : BEGIN
                   ...
                   ptr_swath = data_DPR.NS   ; grab NS lat/lon and cut to inner swath
                END
         'NS' : BEGIN
                   RAYSPERSCAN = RAYSPERSCAN_NS
                   GATE_SPACE = BIN_SPACE_NS_MS
                   ptr_swath = data_DPR.NS
                END
     ...
     ; cut out inner swath NS lat/lon to get MS values that are at identical
     ; locations for a given matched footprint
      IF STRUPCASE(DPR_scantype) EQ 'MS' THEN BEGIN
         msStart = (RAYSPERSCAN_NS-RAYSPERSCAN_MS)/2
         msEnd = msStart+RAYSPERSCAN_MS-1
         prlons = prlons[msStart:msEnd,*]
         prlats = prlats[msStart:msEnd,*]
      ENDIF
...
;   ENDIF
  ...
  ;-----------------------------------------------------------------------------
   ...
   ; gen_gr_hsmsns_geo_match_netcdf_snow()
  ...
  ; attribute values along
   ...
   ncfile = gen_gr_hsmsns_geo_match_netcdf_snow( fname_netCDF, $
                                            num_fp_by_source[0], $
                                            num_fp_by_source[1], $
                                            num_fp_by_source[2], $
                                            tocdf_elev_angle, $
...
; VARIABLES IN THE NETCDF FILE.
   ...
   for swathID = 0, N_ELEMENTS(swathIDs)-1 do begin
     ...
     ; If MS swath, read and cut out inner swath NS lat/lon to get MS values
     ; that are at identical locations as NS for a given matched footprint
      IF STRUPCASE(DPR_scantype) EQ 'MS' THEN BEGIN
         IF PTR_VALID(data_DPR.NS.PTR_DATASETS) THEN BEGIN
            prlons = (*data_DPR.NS.PTR_DATASETS).LONGITUDE
            prlats = (*data_DPR.NS.PTR_DATASETS).LATITUDE
         ENDIF ELSE message, "Invalid pointer to data_DPR.NS"
        ; define an array to hold the NS-relative PR_index for each NS footprint
         dpr_index_NS = LINDGEN(SIZE(prlons, /DIMENSIONS))
        ; clip the NS values to the inner swath of matching MS locations
         msStart = (RAYSPERSCAN_NS-RAYSPERSCAN_MS)/2
         msEnd = msStart+RAYSPERSCAN_MS-1
         prlons = prlons[msStart:msEnd,*]
         prlats = prlats[msStart:msEnd,*]
         dpr_index_NS = dpr_index_NS[msStart:msEnd,*]
      ENDIF ELSE BEGIN
        ; just read and use the HS or NS values for these swaths
         IF PTR_VALID(ptr_swath.PTR_DATASETS) THEN BEGIN
            prlons = (*ptr_swath.PTR_DATASETS).LONGITUDE
            prlats = (*ptr_swath.PTR_DATASETS).LATITUDE
         ENDIF ELSE message, "Invalid pointer to PTR_DATASETS."
...
;      SAVE, dpr_echoes, numDPRrays, file='/tmp/dpr_echoesBy3.sav'
  ...
  ; in code in a separate file, included below:
   ...
   @polar2dpr_hs_ms_ns_resampling_snow.pro

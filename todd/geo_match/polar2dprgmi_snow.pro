...
; All Rights Reserved.
;
; polar2dprgmi_snow.pro          Morris/SAIC/GPM_GV      May 2014
;
; DESCRIPTION
; -----------
...
;    file version, rather than gen_dprgmi_geo_match_netcdf_snow.
...
PRO polar2dprGMI_snow, control_file, range_threshold_km, GPM_ROOT=gpmroot, $
...
;   DPR_scantype = parsed[6]        ; HS, MS, or NS
  ...
  ; so that it can become part of the matchup file name
   throwaway = gen_dprgmi_geo_match_netcdf_snow( GEO_MATCH_VERS=matchup_file_version )
...
;   idx_uniq_elevs = UNIQ(elev_angle)
  ...
  ; Begin first-time inner loop over swath/source types: MS/Ku, MS/Ka, NS/Ku
  ...
  ; generate the netcdf matchup file path/name
   ...
   ; gen_dprgmi_geo_match_netcdf_snow()
  ...
  ; attribute values along
   ...
   ncfile = gen_dprgmi_geo_match_netcdf_snow( fname_netCDF, num_fp_by_source[0],     $
...
; VARIABLES IN THE NETCDF FILE.
   ...
   for swathID = 0, N_ELEMENTS(swathIDs)-1 do begin
     ...
     ; reinitialize flags to skip a DPR ray to zero unless in Ka/MS iteration,
     ; where we'll just re-use the flag values already assigned for Ku/MS
...
;         ptr_free, ptr_swath.PTR_Input
  ...
  ; First, find scans with any point within range of the radar volume, roughly
   ...
   IF ( nscans2do EQ 0 ) THEN BEGIN
      ...
      IF DPR_scantype EQ 'MS' THEN GOTO, skippedSwath ELSE GOTO, emptyFile
   ENDIF
         ...
         IF ( precise_range[ray_num,subset_scan_num] LE max_ranges[0] ) THEN BEGIN
           ...
           ; footprint lat/lon
            ...
            IF idxKuKa[swathID] EQ 0 THEN BEGIN
              ...
              ; - just use what the Ku found for the Ka/MS case (idxKuKa[swathID]=1)
...
;tbSim_183_3v = 12th nemiss index 
  ...
  ; in code in a separate file, included below:
   ...
   @polar2dprgmi_resampling_snow.pro

  ; ============================================================================


   IF idxKuKa[swathID] EQ 0 THEN BEGIN
     ;  Write the swath-invariant MS values the first time through only

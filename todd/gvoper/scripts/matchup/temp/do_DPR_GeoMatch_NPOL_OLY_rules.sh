...
###############################################################################
#
# do_DPR_GeoMatch_NPOL_OLY_rules.sh    Morris/SAIC/GPM GV    August 2017
#
...
# overridden.
#
# This script drives volume matches between GPM DPR and ground radar (GR) data
# for a single user-specified DPR scan type to produce the baseline "GRtoDPR"
...
# capability is "broken" for the NPOL/NPOL_WA file name disconnect between the
...
# --------
#
#    do_DPR_GeoMatch_NPOL_OLY_rules.sh [OPTION]...
#
#
#    OPTIONS/ARGUMENTS:
#    -----------------
...
#
#    -g GRSITE              Override the default GRSITE value "NPOL_WA" to another
#                           NPOL site ID for an alternate NPOL site location, e.g,
#                           MPOL_MD for the Wallops site installation.  DISABLED!
#
...
#
#        - By default, processes matchups for the site ID 'NPOL_WA' only, for
#          the OLYMPEX installation of the NPOL radar. This site ID cannot be
#          changed unless a different VIEW than  collate_npol_wa_1cuf is
#          coded in SQL and defined in the 'gpmgv' database.
#
#
...
# 8/8/2017    Morris       - Created from do_DPR_GeoMatch_NPOLrules.sh. Removed
#                            -g (GRSITE override) option from arguments, and
#                            changed default PPS_VERSION value to V05A.
#
###############################################################################
...
while getopts s:i:v:p:d:w:m:kfrn option
  do
    case "${option}"
      in
        s) SAT_ID=${OPTARG};;
...
#echo "2015-11-11" > $datelist   # edit/uncomment to just run a specific date

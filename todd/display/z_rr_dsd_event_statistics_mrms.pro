...
; All Rights Reserved.
;
; z_rr_dsd_event_statistics.pro    Morris/GPM GV/SAIC    December 2016
;
;
; DESCRIPTION
; -----------
; Driver procedure to run the analysis procedure geo_match_3d_comparisons. Reads
; a file containing a set of parameters to be used, parses the options specified
; in the control file, and runs geo_match_3d_comparisons with these parameters,
; and with default values for the parameters not specified in the control file.
...
; semicolons (;) are generally not present in the control file, except in the
; case where the keyword/value specification is to be ignored (i.e., to disable
; the parameter specification without deleting the line from the file).  Quotes
; are optional in the control file for keywords that take string values (e.g.
; SITE or PS_DIR), except in the case where the empty string is being specified
; as the value (e.g., PS_DIR='').  Leading and trailing spaces, and spaces
; either side of the assignment operator (=) are optional and ignored.
;
; See geo_match_3d_comparisons.pro for a detailed description of each
...
;-------------------- this line not included in control file -----------------
;
; See the end of the z_rr_dsd_event_statistics procedure below for the complete
; geo_match_3d_comparisons calling sequence.
;
;
; INTERNAL MODULES
; ----------------
; 1) z_rr_dsd_event_statistics - Main driver procedure called by user
; 2) parse_ctl_parms           - Parses a control file to get keyword parameters
;
;
; HISTORY
; -------
...
;   Calls new merged display/analysis procedure geo_match_3d_comparisons.
; 01/24/17 Morris, GPM GV, SAIC
; - Added processing of new keyword parameter NCFILELIST.
; - Fixed bug assigning z_blockage_thresh in call to geo_match_3d_comparisons.
; 03/24/17 Morris, GPM GV, SAIC
; - Added LAND_OCEAN=land_ocean keyword/value pair to control file processing
...
FUNCTION parse_ctl_parms, ctlstr, analysis_type, matchup_type, $
                          swath_cmb, kuka_cmb, looprate, $
                          elevs2show, ncpath, sitefilter, ncfilelist, $
                          no_prompt, ppi_vertical, ppi_size, $
                          pctAbvThresh, dpr_z_adjust, gr_z_adjust, $
...
pro z_rr_dsd_event_statistics_mrms, CONTROL_FILE=ctlfile
...
SITE='*'
;NCFILELIST=''        ; leave undefined by default
NO_PROMPT=0
PPI_VERTICAL=0
PPI_SIZE=375
PCT_ABV_THRESH=0
...
RECALL_NCPATH=0
SUBSET_METHOD=""
; MIN_FOR_SUBSET=20  ; leave undefined by default
SAVE_BY_RAY=0
SAVE_DIR=''
STEP_MANUAL=0
DECLUTTER=0
;LAND_OCEAN='Any'  ; leave undefined by default
...
PRINT, ctlstr
  status = parse_ctl_parms( ctlstr, analysis_type, matchup_type, $
                            swath_cmb, kuka_cmb, looprate, $
                            elevs2show, ncpath, sitefilter, ncfilelist, $
                            no_prompt, ppi_vertical, ppi_size, $
                            pctAbvThresh, dpr_z_adjust, gr_z_adjust, $
...
geo_match_3d_comparisons_mrms, ANALYSIS_TYPE=analysis_type, $

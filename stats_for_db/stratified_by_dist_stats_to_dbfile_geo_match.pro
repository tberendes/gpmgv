;+
; stratified_by_dist_stats_to_dbfile_geo_match.pro 
...
pro stratified_by_dist_stats_to_dbfile_geo_match, PCT_ABV_THRESH=pctAbvThresh,  $
...
dbfile = '/data/tmp/StatsByDistToDBbyGeo23Apr2010newBBorigGV.unl'
...
; set PR=Convective type to Other where GV is too low for convective
;idx2other = WHERE( rnType EQ 2 AND max_gvz_per_fp LE 25.0, count2other )
;IF ( count2other GT 0 ) THEN rnType[idx2other] = 3
;print, "Num. footprints switched from Convective to Other: ", count2other
; set PR=Stratiform type to Other where GV is too high for stratiform
;idx2other = WHERE( rnType EQ 1 AND max_gvz_per_fp GE 35.0, count2other )
;IF ( count2other GT 0 ) THEN rnType[idx2other] = 3
;print, "Num. footprints switched from Stratiform to Other: ", count2other
...
IF ( correct_s_band ) THEN BEGIN
   if ( countabovebb GT 0 ) then begin
     ; apply correction to above-BB (snow) S-band GV samples
      gvsnow = gvz[idxabovebb]
      gvsnow2ku = s_band_to_ku_band(gvsnow,'s')
      gvz[idxabovebb] = gvsnow2ku
   endif

   if ( countbelowbb GT 0 ) then begin
     ; apply correction to below-BB (all-rain) S-band GV samples
      gvrain = gvz[idxbelowbb]
      gvrain2ku = s_band_to_ku_band(gvrain,'r')
      gvz[idxbelowbb] = gvrain2ku
   endif
ENDIF

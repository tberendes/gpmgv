...
; All Rights Reserved.
;
; stratified_by_sfc_stats_to_dbfile.pro   Morris/SAIC/GPM_GV      August 2007
;
; DESCRIPTION
; -----------
...
; underlying surface type (land, ocean, or coastal [i.e., mixed]), for a
...
; format ready to be loaded into the 'gpmgv' database.
;
...
; FILES
; -----
; /data/tmp/StatsBySfcToDB.unl       OUTPUT: Formatted ASCII text file holding
...
; CALLS
; -----
; uncomp_file()    stratify_diffs21    printf_stat_struct21
;
;
...
pro stratified_by_sfc_stats_to_dbfile

dbfile = '/data/tmp/StatsBySfcToDB.unl'
   ...
   if (countpos1 gt 0) then begin
      ...
      if (countpos2 gt 0) then begin
        ...
        ; above/below/within BB, Convective and Stratiform
         ...
         sfctyp2 = sfctyp1[idxpos2]
;         stratify_diffs, dbzpr2, dbznx2, rntyp2, bb2Lo, bb2Hi, lev2get, allstats
         this_statsbysfc = {stats21ways}
         stratify_diffs21, dbzpr2, dbznx2, rntyp2, bb2Lo, bb2Hi, sfctyp2, $
                           lev2get, this_statsbysfc
;print, allstats
      endif
	 ...
	 if (countpos3 gt 0) then begin
           ...
           ; Do above/below/within BB, Convective and Stratiform
            ...
            sfctyp3 = sfctyp1[idxpos3]
            this_statsbysfcREO = {stats21ways}
            stratify_diffs21, dbzpr3, dbznx3, rntyp3, bb3Lo, bb3Hi, sfctyp3, $
                            lev2get, this_statsbysfcREO
;print, allstatsREO
	 endif
      endif

;print, 'Level = ', (lev2get + 1)*1.5, '  npts_sea = ', this_statsbysfc.pts_sea
;     Write Delimited Output for database, separate records for 2A55 and REO:
      printf_stat_struct21, this_statsbysfc, '2A55', siteID, orbit, lev2get, DBunit

      if ( haveREO eq 1) then begin
         printf_stat_struct21, this_statsbysfcREO, 'REOR', siteID, orbit, lev2get, DBunit
      endif
   endif
endfor
print, ''

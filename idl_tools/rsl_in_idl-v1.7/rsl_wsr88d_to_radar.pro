...
;              for each Radar Data Acquisition (RDA) elevation (sweep).
...
; Open wsr88d file, uncompressing if necessary.

if is_compressed(filename[0]) then begin
    tmp_file = rsl_uncompress(filename[0], error=error)
    if error then goto, finished
    ; Next line is a safeguard--we wouldn't want to delete a data file.
    if tmp_file eq wsr88d_file then begin
	message, 'Temporary file name returned by' + $
            ' rsl_uncompress is same as original: ' + tmp_file, /informational
        goto, finished
    endif
    openr, iunit, tmp_file, /get_lun, /delete ; this file is deleted on close.
endif else openr, iunit, wsr88d_file, /get_lun

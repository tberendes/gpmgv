...
function rsl_mapcolors, ray, field, imagedata=imagedata
...
;
; Modified 18 August 2016:
; Added keyword IMAGEDATA to allow an image array to be used in place of a
; single ray.  When this keyword is set to a variable, the variable RANGE is
; assigned the contents of IMAGEDATA.
; Note that the argument "ray" is required whether or not IMAGEDATA is set.
...
if n_elements(imagedata) eq 0 then range = ray.range[0:ray.h.nbins-1] else $
    range = imagedata

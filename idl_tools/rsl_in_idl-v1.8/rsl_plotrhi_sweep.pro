...
;    WINDOWSIZE:  Window size in pixels.  Windowsize may be given as a 2-element
;                 array specifying x and y lengths for the window, or it may be
;                 a scalar, which will be used for both x and y values.
...
;TODO: Should these defaults be smaller?
...
for iray = 0, sweep.h.nrays - 1 do begin
    ...
    ; computed at start and end of range bin.
    ...
    frac_bw = sweep.h.beam_width/6.*!DTOR
    elevplus_bw = elev + frac_bw 
    elevminus_bw = elev - frac_bw
	...
	range = range_bin1 + float(ibin)*gate_size - gate_size/2.
        if range lt 0. then range = 0.
	x[0] = range * cos(elevplus_bw)
	y[0] = sqrt(range^2.+erad^2. +2*range*erad*sin(elevplus_bw)) - erad
	x[1] = range * cos(elevminus_bw)
	y[1] = sqrt(range^2.+erad^2. +2*range*erad*sin(elevminus_bw))- erad

	; Get polyfill points for end of this range bin.
	range = range + gate_size
	x[2] = range * cos(elevminus_bw)
	y[2] = sqrt(range^2.+erad^2. +2*range*erad*sin(elevminus_bw))- erad
	x[3] = range * cos(elevplus_bw)
	y[3] = sqrt(range^2.+erad^2. +2*range*erad*sin(elevplus_bw)) - erad

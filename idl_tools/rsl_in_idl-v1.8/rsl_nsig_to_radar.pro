...
; For a complete list see "IRIS Programmer's Manual".
...
DBTE   = 72 ; Total Power Enhanced (via H+V or HV) (2 byte)
DBZE   = 74 ; Clutter Corrected Reflectivity Enhanced (2 byte)
	...
	if datacnt gt 0 then begin
	    case datatype[iparam] of
	      ...
	      DBZV: radar.volume[ivol].sweep[isweep].ray[iray].range[dataloc]= $
		     (uint(raydata[dataloc])-32768.)/100.
	      DBTE: radar.volume[ivol].sweep[isweep].ray[iray].range[dataloc]= $
		     (uint(raydata[dataloc])-32768.)/100.
	      DBZE: radar.volume[ivol].sweep[isweep].ray[iray].range[dataloc]= $
		     (uint(raydata[dataloc])-32768.)/100.
...
;**************************************************************************
; Get Sigmet data types corresponding to RSL field types.
;
; Inputs:
...
; This array contains RSL field types corresponding to the SIGMET data types.
; SIGMET uses numerical constants to identify data types (fields).  The SIGMET
; data type constants are used here to index RSL field types.  For example, data
; type constant 3 identifies SIGMET 1-byte velocity, and is the index for RSL
; field type 'VR'.  You may notice there is another 'VR' at index 10; this
; corresponds to the data type constant for 2-byte velocity.  Question marks (?)
; are used for fields that are not currently used in RSL.
...
for i = 0, nfields-1 do begin
    ...
    if unknown_field and not keyword_set(quiet) then $
        message, string(j, format = $

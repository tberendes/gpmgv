...
;
; DESCRIPTION:
;       Resamples PR 2-D and 3-D data fields in scan (number,angle) coordinates
;       to a 2-D or 3-D Cartesian grid centered on the location of a ground
;       radar.  Writes the grids and various metadata to a supplied netCDF
;       file.
;
; AUTHOR:
;       Liang Liao
...
;
;       09/2008 by Bob Morris, GPM GV (SAIC)
;       - Fixed bug in determining whether bright band gate number is valid, at
;         line 447.
;
;       08/2011 by Bob Morris, GPM GV (SAIC)
;       - Added siteElevation to the groundSite COMMON and PR data processing.
;       - Fixed 1-gate offset of 1C21.
;
; MODULES:
;  1)  PROCEDURE scan_edge_x_and_y
;  2)  PROCEDURE generate_pr_ncgrids
;-

;***********************************************************************************

 pro scan_edge_x_and_y, scan, angle, RAYSPERSCAN, $
                        siteLong, siteLat, lons, lats, X, Y
...
;  slant range to HEIGHT+siteElev+delta_h, in gates, @ 4 gate/km:
   rip = gates_per_km*(HEIGHT+siteElev+delta_h)/cos_inc_angle[angle] 
;  slant range to height+siteElev+delta_h in whole gates, from surface:
...
;   Compute dR = total offset towards nadir point, for given height and angle
    dR = (HEIGHT+siteElev) * tan_inc_angle[angle]
...
;   Apply dX and dY offsets to x's and y's input to analyses at HEIGHT+siteElev
...
;   HEIGHT+siteElev+delta_h.  Height for each gate number is a function of ray
;   angle.  We use the surface-relative IP lookup array we precomputed, indexed
;   to angle and h.
...
;     height for 1C21, guarding against negative gate index numbers:
      gateS = 0 > ( (binS[scan,angle]-rayStart[angle])/2 - ip[angle,h+hh] )
...
;     the layer average value and deal with it after analysis.
      gateS = 0 > ((binS[scan,angle]-rayStart[angle])/2 - ip[angle,hh])
...
;      if ( zdata_BB_Hgt[countBB] lt 79 ) then begin
      if ( brightBand[scan,angle] lt 79 ) then begin
...
;   Already computed the x and y point for off-edge, use them here
    if (angle eq 0) OR (angle eq RAYSPERSCAN-1) then begin 
       xdataBB[countBB] = XX2
       ydataBB[countBB] = YY2
       xdataRF[countBB] = XX2
...
;   in scan were in grid but neither the 1st or last points were.
    ...
    if (do_edges eq 1) AND (did_edges eq 0) then begin 
      ; which half of the scan was most within the grid?
       ...
       endelse
       xdatarain_at_h[countRR] = XX
       ydatarain_at_h[countRR] = YY
       zdata_2a25_rain[countRR] = SRAIN_OFF_EDGE
       countRR = countRR + 1
       if (LEVEL EQ 1) then begin
            xdataBB[countBB] = XX

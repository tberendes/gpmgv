;+
;-------------------------------------------------------------------------------
; update_pr_ncgrids.pro    Bob Morris, GPM GV (SAIC)    December 2006
;
;
; DESCRIPTION
; -----------
; Reads an interleaved, delimited text file listing 1C21 and 2A25 file names,
...
; "generate_1C21_2A25_ncgrids.pro" is a highly-modified version of Liang's
...
; FILES
; -----
; TMP_DIR/PR_files_sites4grid.YYMMDD.txt (INPUT) - lists 1C21 and 2A25 files,
;    their orbit, and information on the overpassed NEXRAD sites and the
;    (one or more) corresponding 2A55 radar site files, where YYMMDD is
...
;
;       1C21_filename|2A25_filename|2B31_filename|ORBIT_NUMBER|NSITES       (first block)
;       event_num_1|siteID_1|siteLat_1|siteLon_1|2A55_filepath1|2A54_filepath1|2A53_filepath1
;       event_num_2|siteID_2|siteLat_2|siteLon_2|2A55_filepath2|2A54_filepath2|2A53_filepath2
;         . 
;         .   repeats M = NSITES times [1..M]
;         . 
;       event_num_M|siteID_M|siteLat_M|siteLon_M|2A55_filepathM|2A54_filepathM|2A53_filepathM
;       1C21_filename|2A25_filename|2B31_filename|ORBIT_NUMBER|NSITES       (second block)
;       event_num_1|siteID_1|siteLat_1|siteLon_1|2A55_filepath1|2A54_filepath1|2A53_filepath1
;       event_num_2|siteID_2|siteLat_2|siteLon_2|2A55_filepath2|2A54_filepath2|2A53_filepath2
;         . 
;         .   repeats N = NSITES times [1..N]
;         . 
;       event_num_N|siteID_N|siteLat_N|siteLon_N|2A55_filepathN|2A54_filepathN|2A53_filepathN
;
;   Thus, a "pattern block" is defined in the control file for each orbit whose
;   data are to be processed: one row listing the three orbit subset PR filenames,
;   the orbit number, and the number of sites to process for the orbit; followed
...
;   Pattern block repeats for each ORBIT to be processed.  For a given orbit
;   (and its associated 1C21/2A25/2B31 file triplet) there are NSITES sites
;   overpassed, and output netCDF grids will be produced for each of these sites.
;
...
;    copied into this directory to be unzipped, and then the copy deleted.  THE
;    WORKING DIRECTORY FROM WHICH THE CODE IS RUN MUST NOT BE THE SAME DIRECTORY
;    AS WHERE THE DATA FILES ARE LOCATED, OR THE DATA FILES MAY BE DELETED!!!
...
pro update_pr_ncgrids, DATESTAMP, FILES4NC
...
; Create and make sure PR netCDF template file exists
pr_template_file = ''
...
;   origFile31Name = parsed[2] ; filename as listed in/on the database/disk
   ...
   if ( n_elements(parsed) gt 5 ) then begin
      print, "Overriding DATESTAMP from/to: ", DATESTAMP, parsed[5]
      DATESTAMP = parsed[5]
   endif
...
;        Read the uncompressed 1C21 file copy
         read_1c21, file21_2do, BinS=binS, RAY_START=rayStart
...
;
      ...
      ncfiletrygz = ncfile + ".gz"
      ncfiles = file_search(ncfiletrygz,COUNT=nf)
      if(nf eq 1) then begin
        command = "gzip -fd " + ncfiletrygz
      print, "NetCDF file name:  ", ncfiletrygz
;      command = "cp " + pr_template_file + ' ' + ncfile
;      print, command
      spawn,command,exit_status=status
      if(status ne 0) then begin
         flag = 'Error decompressing file: ' + ncfiletrygz
         goto, errorExit
      endif
;
;     Call fix_pr_3d_rain_ncgrids to analyze/write grids to netCDF file
;
;     The following test allows PR grid generation to proceed without the
;     2B-31 data file being available.  This is for the interim where the
;     2B-31's are not yet filled in back to the start of GPMGV TSDIS data.

      fix_pr_3d_rain_ncgrids, nAvgHeight=2, ncfile, lons, lats,   $
                              rain_2a25, $
                              rangeBinNums, binS, rayStart
;
...
;
         ...
         print, "PR netCDF file not found: ", ncfiletrygz
      endelse
   endfor

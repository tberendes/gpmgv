...
;-
function grib_print_parameternames, grib_file
   compile_opt idl2
   ...
   while (i_record lt n_records) do begin
      ...
      while grib_keys_iterator_next(iter) do begin
         ...
         IF i_record GT 151 THEN BEGIN
            IF strlowcase(key) ne 'values' and strlowcase(key) ne 'value' THEN $
               PRINT, 'record, key, value: ', i_record, ", ", key, ", ", grib_get(h, key) $
            ELSE PRINT, 'record, key: ', i_record, ", ", key
         ENDIF

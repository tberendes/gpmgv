...
; geo_match_3d_dsd_comparisons, parses the options specified in
...
;
; See geo_match_3d_dsd_comparisons.pro for a detailed description of each
...
;-------------------- this line not included in control file -----------------
;
; here's the geo_match_3d_dsd_comparisons calling sequence:
;
;     geo_match_3d_dsd_comparisons, MATCHUP_TYPE=matchup_type, $
;                                   SWATH_CMB=swath_CMB, $
;                                   KUKA_CMB=KuKa_CMB, $
;                                   SPEED=looprate, $
...
; INTERNAL MODULES
; ----------------
; 1) dsd_comparisons - Main driver procedure called by user
; 2) parse_dsd_parms - Parses the control file to get keyword parameters
;
; HISTORY
; -------
; 07/17/15 Morris, GPM GV, SAIC
; - Created from rr_or_z_comparisons.pro, to call geo_match_3d_dsd_comparisons.
; 06/07/16 Morris, GPM GV, SAIC
; - Added parameters gr_dm_field, gr_nw_field to processed arguments.
...
; - Added keyword variables SWATH_CMB, KUKA_CMB, and SAVE_BY_RAY to support
;   analysis of DPRGMI matchup datasets.
...
FUNCTION parse_dsd_parms, ctlstr, matchup_type, swath_cmb, kuka_cmb, looprate, $
...
pro dsd_comparisons, CONTROL_FILE=ctlfile
...
while (eof(ctlunit) ne 1) DO BEGIN
  ...
  status = parse_dsd_parms( ctlstr, matchup_type, swath_cmb, kuka_cmb, looprate, $
                            elevs2show, ncpath, sitefilter, no_prompt, $
                            ppi_vertical, ppi_size, pctAbvThresh, $
                            dpr_z_adjust, gr_z_adjust, $
                            max_range, show_thresh_ppi, z_only_ppi, $

...
; All Rights Reserved.
;
; check_geo_matchups5.pro         Morris/SAIC/GPM_GV      February 2009
;
;
; DESCRIPTION
...
pro check_geo_matchups5, SPEED=looprate, ELEVS2SHOW=elevs2show, NCPATH=ncpath, $
...
uf_file = get_uf_pathname2(ncfilepr, gvbegin, gv_base_dir)
...
;   RSL_in_IDL radar structure, and then close UF file and delete copy:
  ...
  ;   and the ID of the field itself
   ...
   xinteranimate, set=[xsize, ysize, 4*nframes], /TRACK
   ...
   FOR ifram=0,nframes-1 DO BEGIN
      ...
      device, decomposed=0
      TV, myprbuf
      xinteranimate, frame = ifram*4, window=0
      xinteranimate, frame = 3+ifram*4, window=0
      ;print, "Loaded pr frame ", ifram+1
      TV, mygvbufhi
      xinteranimate, frame = 1+ifram*4, window=0
      TV, mygvbuf
      xinteranimate, frame = 2+ifram*4, window=0

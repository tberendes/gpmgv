...
; All Rights Reserved.
;
; pr_and_geo_match_x_sections_merged_gif.pro    Morris/SAIC/GPM_GV    March 2015
;
; DESCRIPTION
; -----------
...
;
; gifpath      - local directory path where animated GIFs will be written to
;                when walking through scans.  If unset, then animated GIFs will
;                not be output.
;
...
;
; alt_bb_hgt    - Manually-specified Bright Band Height source to be used if the
...
; 03/24/15  Morris/GPM GV/SAIC
; - Added GIF_PATH keyword parameter to control output of an animated GIF when
;   automatically stepping through the scans.
; - Added ALT_BB_HGT keyword parameter to supply an alternate source of mean BB
;   height when one cannot be determined from the DPR data.
;
...
pro gen_pr_and_geo_match_x_sections, ncfilepr, use_db, skip_2a25_orig, $
                  pr_or_dpr, ELEV2SHOW=elev2show, PCT_ABV_THRESH=pctAbvThresh, $
                  ...
                  ALT_BB_HGT=alt_bb_hgt, VERBOSE=verbose
;
;
; DESCRIPTION
; -----------
...
;  'PR' : @pr_params.inc
; 'DPR' : @dpr_params.inc
;  ELSE : message, "Illegal value for pr_or_dpr: "+pr_or_dpr
;ENDCASE
@dpr_params.inc
...
CASE pr_or_dpr OF
  'PR' : verstring = "V"+STRTRIM( STRING(mygeometa.PR_Version), 2 )
 'DPR' : BEGIN
...
;           if count31 EQ 1 THEN file_2b31 = STRTRIM(parsepr[idx31],2) ELSE file_2b31='no_2B31_file'
     ...
     ENDIF
  END
  ELSE : skip_2a25 = 1  ; haven't coded for DPR yet
ENDCASE
...
xsize = windowsize[0]
ysize = xsize
IF ( pctAbvThresh GT 0 ) THEN title0 = STRING(pctAbvThresh,FORMAT='(i0)')+"% above-threshold samples shown" $
...
CASE pr_or_dpr OF
   'PR' : BEGIN
            ...
            ENDIF ELSE BEGIN
               ...
               prtitle = "PR "+verstring+" 2A-25 Ze along "+mysite.site_ID+" " $
                         +elevstr+" degree sweep"
            ENDELSE
          END
  'DPR' : BEGIN
            IF (cref) THEN BEGIN
               prtitle = '2A'+sourceLabel+' '+verstring+" Composite Ze"
            ENDIF ELSE BEGIN
               elevstr =  string(mysweeps[ifram].elevationAngle, FORMAT='(f0.1)')
               prtitle = '2A'+sourceLabel+' '+verstring+" Ze along "+mysite.site_ID+" " $
                         +elevstr+" degree sweep"
            ENDELSE
          END
ENDCASE
         ...
         250B : BEGIN
                  ...
                  ; specify a GIF file to write the animation frames to if GIF_PATH is defined
                   IF N_ELEMENTS(gif_path) EQ 1 THEN BEGIN
                      gif_file = GIF_PATH+'/' + gif_base  ;XsectionSequence.gif'
...
;                      IF scanNumPR EQ scanoff AND havewin2 EQ 1  THEN WDELETE, 5
                     ...
                     ; show where the cross section will be generated
                      ...
                      PRINT, '' & doodah = ''
                      IF N_ELEMENTS(gif_path) EQ 1 THEN BEGIN
                         ;READ, doodah, PROMPT='Hit Return to do next scan, '+ $
                         ;      'W to write this scan to GIF, Q to Quit: '
                         ;IF STRUPCASE(doodah) EQ 'Q' THEN break
                         ;IF STRUPCASE(doodah) EQ 'W' THEN BEGIN
...
;                           ypos=50, TITLE = "PR and GR Volume Match X-Sections"
...
pro pr_and_geo_match_x_sections_merged_gif, ELEV2SHOW=elev2show, SITE=sitefilter, $
                                 NO_PROMPT=no_prompt, NCPATH=ncpath, PRPATH=prpath, $
                                 UFPATH=ufpath, GIF_PATH=gif_path, USE_DB=use_db, $
                                 SKIP_2A25=skip_2a25, PCT_ABV_THRESH=pctAbvThresh, $
                                 BBBYRAY=BBbyRay, PLOTBBSEP=plotBBsep, $
                                 BBWIDTH=bbwidth, ALT_BB_HGT=alt_bb_hgt, $
                                 HIDE_RNTYPE=hide_rntype, CREF=cref, $
                                 PAUSE=pause, ZOOMH=zoomh,  INSTRUMENT=instrument, $
...
ENDIF ELSE BEGIN
   CASE STRUPCASE(instrument) OF
      'PR' : pr_or_dpr = 'PR'
     'DPR' : pr_or_dpr = 'DPR'
      ELSE : message, "Only allowed value for INSTRUMENT is DPR or PR"
   ENDCASE
ENDELSE
...
IF ( N_ELEMENTS(sitefilter) NE 1 ) THEN BEGIN
   ...
   print, ""
   ncfilepatt = 'GRto'+pr_or_dpr+'*.nc*'
ENDIF ELSE BEGIN
   IF ( STRPOS(sitefilter, GEO_MATCH_PRE) NE 0 ) THEN $
      ncfilepatt = 'GRto'+pr_or_dpr+'*'+sitefilter+'*' $
   ...
   if nf eq 0 then begin
      ...
      for fnum = 0, nf-1 do begin
        ...
        ;
         ...
         gen_pr_and_geo_match_x_sections, ncfilepr, use_db, skip_2a25, pr_or_dpr, $

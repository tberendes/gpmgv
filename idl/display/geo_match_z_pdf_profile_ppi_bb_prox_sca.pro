...
; proximity levels for points within 100 km of the ground radar and reports
; the results in a table to stdout.  Also produces graphs of the
; Probability Density Function of PR and GV reflectivity at each of these 3
...
; - define the BB as +/- 500m of the mean BB height.  Any sample touching that layer
;   is within the BB
num_in_BB_Cat = LONARR(4)
bbHalfThick = 0.500    ; BB half thickness
bbProx = distcat
bbProx[*] = 0  ; re-init to Not Defined
idxabv = WHERE( botm GT (meanbb+bbHalfThick), countabv )
num_in_BB_Cat[3] = countabv
IF countabv GT 0 THEN bbProx[idxabv] = 3
idxblo = WHERE( top LT (meanbb-bbHalfThick), countblo )
num_in_BB_Cat[1] = countblo
IF countblo GT 0 THEN bbProx[idxblo] = 1
idxin = WHERE( (botm LE (meanbb+bbHalfThick)) AND (top GE (meanbb-bbHalfThick)), countin )
...
for lev2get = 0, 12 do begin
   ...
   IF ( num_in_hgt_cat[lev2get] GT 0 ) THEN BEGIN
      ...
      diffstruc = the_struc
      calc_geo_pr_gv_meandiffs_idx, zcor, gvz, rnType, dist, distcat, hgtcat, $
                             lev2get, dbzcut, rangecut, bs, mnprarr, mngvarr, $
                             havematch, diffstruc,  idx4hist
...
Window, xsize=700, ysize=700, TITLE = strmid( bname, 7, 17)+"  --  " $
        +"With % of averaged bins above dBZ thresholds "+'>= '+pctString+"%", RETAIN=2
...
prop_max_y = max_hgt_w_data*2.0 > (FIX((meanbb*2)/1.5) + 1) *1.5
...
XYOutS, 0.34, 0.8, 'Mean BB Hgt', COLOR = 255, CHARSIZE=1, /NORMAL
...
print, ''
print, 'Statistics by proximity to Bright Band:'
print, ''
print, 'Proxim.|   Any Rain Type  |    Stratiform    |    Convective     |     Dataset Statistics      |     |'
print, ' to BB |  PR-GV    NumPts |  PR-GV    NumPts |  PR-GV    NumPts  | AvgDist   PR MaxZ   GV MaxZ | BB? |'
print, ' ----- | -------   ------ | -------   ------ | -------   ------  | -------   -------   ------- | --- |'

; array to capture indices of array values used for point-to-point
;   mean diffs, by bbProx level
...
for bblev2get = 1, 3 do begin
   ...
   IF ( num_in_BB_cat[bblev2get] GT 0 ) THEN BEGIN
      ...
      diffstruc = the_struc
      calc_geo_pr_gv_meandiffs_idx, zcor, gvz, rnType, dist, distcat, bbProx, $
                             bblev2get, dbzcut, rangecut, bs, mnprarrbb, mngvarrbb, $
                             havematch, diffstruc,  idx4hist
      if(havematch eq 1) then begin
...
;         type2 = type4hist[0:diffstruc.fullcount-1]
         mndifhstr = string(diffstruc.AvgDifByHist, FORMAT='(f0.3)')
         mndifstr = string(diffstruc.meandiff, diffstruc.fullcount, FORMAT='(f0.3," (",i0,")")')
         ...
         idx4hist[*] = -1
         print, bblevstr[bblev2get], stats55, flag, FORMAT='(a0," ",a0," ",a0)'
...
;         nxhist = histogram(dbzgv2, min=minz4hist, max=maxz4hist, binsize = bs)
         ...
         IF ( ~ hideTotals ) THEN BEGIN
            ...
            xyouts, 0.34, 0.95, 'PR (all)', COLOR = 30, /NORMAL, CHARSIZE=1
            plots, [0.29,0.33], [0.955,0.955], COLOR = 30, /NORMAL
            xyouts, 0.34, 0.925, siteID+' (all)', COLOR = 70, /NORMAL, CHARSIZE=1
            plots, [0.29,0.33], [0.93,0.93], COLOR = 70, /NORMAL
         ENDIF

         headline = 'PR-'+siteID+' Biases:'
         xyouts, 0.775+xoff[bblev2get],0.925+yoff[bblev2get], headline, COLOR = 255, /NORMAL, CHARSIZE=1
         ...
         xyouts, 0.775+xoff[bblev2get],0.9+yoff[bblev2get], mndifline, COLOR = 255, /NORMAL, CHARSIZE=1
         xyouts, 0.775+xoff[bblev2get],0.875+yoff[bblev2get], mndiflinec, COLOR = 255, /NORMAL, CHARSIZE=1
         xyouts, 0.775+xoff[bblev2get],0.85+yoff[bblev2get], mndiflines, COLOR = 255, /NORMAL, CHARSIZE=1
         xyouts, 0.775+xoff[bblev2get],0.825+yoff[bblev2get], mndifhline, COLOR = 255, /NORMAL, CHARSIZE=1
...
;print, "BBlev, nconv, diffstruc.countc: ", bblev2get, nconv, diffstruc.countc
           ...
           oplot, prhiststart, prhist, COLOR = 30, LINESTYLE=2, thick=3
           oplot, prhiststart, nxhist, COLOR = 70, LINESTYLE=2, thick=3
           xyouts, 0.34, 0.85, 'PR (Conv)', COLOR = 30, /NORMAL, CHARSIZE=1
           xyouts, 0.34, 0.825, siteID+' (Conv)', COLOR = 70, /NORMAL, CHARSIZE=1
           plots, [0.29,0.33], [0.855,0.855], COLOR = 30, /NORMAL, LINESTYLE=2, thick=3
           plots, [0.29,0.33], [0.83,0.83], COLOR = 70, /NORMAL, LINESTYLE=2, thick=3
         ENDIF

        ; OVERLAY STRATIFORM RAINTYPE PDFS, IF ANY POINTS
         idxstrathist= WHERE( type2 EQ RainType_stratiform, nstrat )
;print, "BBlev, nstrat, diffstruc.counts: ", bblev2get, nstrat, diffstruc.counts
         IF ( nstrat GT 0 ) THEN BEGIN
           ...
           oplot, prhiststart, prhist, COLOR = 30, LINESTYLE=1, thick=3
           oplot, prhiststart, nxhist, COLOR = 70, LINESTYLE=1, thick=3
           xyouts, 0.34, 0.9, 'PR (Strat)', COLOR = 30, /NORMAL, CHARSIZE=1
           xyouts, 0.34, 0.875, siteID+' (Strat)', COLOR = 70, /NORMAL, CHARSIZE=1
           plots, [0.29,0.33], [0.905,0.905], COLOR = 30, /NORMAL, LINESTYLE=1, thick=3
           plots, [0.29,0.33], [0.88,0.88], COLOR = 70, /NORMAL, LINESTYLE=1, thick=3
         ENDIF
...
SCATITLE = strmid( bname, 7, 17)+"  --  "+"% bins above thresholds " $
           +'>= '+pctString+"%"

IF KEYWORD_SET( ps_dir ) THEN BEGIN
   do_ps = 1
   PSFILE = ps_dir+'/'+strmid( bname, 7, 17)+'Pct'+pctString+'.ps'
   print, "Output sent to ", PSFILE
   plot_scatter_by_bb_prox_ps, PSFILE, SCATITLE, siteID, zcor, gvz, $
                            rnType, bbProx, num4hist3, idx4hist3
ENDIF ELSE BEGIN
   do_ps = 0
...
;===============================================================================
;
; geo_match_z_pdf_profile_ppi_bb_prox_sca.pro    Morris/SAIC/GPM_GV    February 2009
;
; DESCRIPTION
; -----------
...
pro geo_match_z_pdf_profile_ppi_bb_prox_sca, SPEED=looprate, $
...
IF ( N_ELEMENTS( ps_dir ) NE 1 ) THEN BEGIN

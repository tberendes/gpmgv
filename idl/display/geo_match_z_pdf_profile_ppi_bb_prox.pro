...
; All Rights Reserved.
;
; geo_match_z_pdf_profile_ppi_bb_prox.pro    Morris/SAIC/GPM_GV    February 2009
;
; DESCRIPTION
; -----------
...
;-
;===============================================================================
pro geo_match_z_pdf_profile_ppi_bb_prox, SPEED=looprate, $
      ...
      for fnum = 0, nf-1 do begin
        ...
        ;
         ...
         action = geo_match_z_plots( ncfilepr, looprate, elevs2show, startelev, $
                                     ...
                                     gvconvective, gvstratiform, histo_Width, hideTotals )
...
FUNCTION geo_match_z_plots, ncfilepr, looprate, elevs2show, startelev, PPIorient, $
                            ...
                            gvstratiform, histo_Width, hideTotals
;
; DESCRIPTION
; -----------
...
; Computes mean PR-GV reflectivity
; differences for each of the 3 bright band proximity levels for points within 100 km of the
; ground radar and reports the results in a table to stdout.  Also produces
; graphs of the Probability Density Function of PR and GV reflectivity at each of these 3
...
Window, xsize=700, ysize=700, TITLE = strmid( bname, 7, 17)+"  "+'>= ' $
        +pctString+"% of averaged bins above dBZ thresholds"
...
for bblev2get = 1, 3 do begin
   ...
   IF ( num_in_BB_cat[bblev2get] GT 0 ) THEN BEGIN
      flag = ''
      prz4hist = fltarr(num_in_BB_cat[bblev2get])  ; PR dBZ values used for point-to-point mean diffs
      gvz4hist = fltarr(num_in_BB_cat[bblev2get])  ; GV dBZ values used for point-to-point mean diffs
      type4hist = intarr(num_in_BB_cat[bblev2get])  ; rain type values used for point-to-point mean diffs
      prz4hist[*] = 0.0
      gvz4hist[*] = 0.0
      type4hist[*] = RainType_missing
      if (bblev2get eq 2) then flag = ' @ BB'
      diffstruc = the_struc
      calc_geo_pr_gv_meandiffs, zcor, gvz, rnType, dist, distcat, bbProx, $

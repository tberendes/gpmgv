...
; All Rights Reserved.
;
; geo_match_z_pdf_profile_ppi_2types.pro     Morris/SAIC/GPM_GV    February 2009
;
; DESCRIPTION
; -----------
...
;
; This version encodes the rain type as a fill pattern in the PPI image plots:
; solid=Other, vertical=Convective, horizontal=Stratiform.
;
; PARAMETERS
; ----------
...
;-
;===============================================================================
pro geo_match_z_pdf_profile_ppi_2types, PDF_LEVEL=pdf_level, SPEED=looprate, $
...
; graphs of the Probability Density Function of PR and GV reflectivity at a given
; height level (input) if data exists at that level, and vertical profiles of
...
;               TITLE = strmid( bname, 7, prlen-10), CHARSIZE=1
         ...
         xyouts, 0.19, 0.95, 'PR (all)', COLOR = 30, /NORMAL, CHARSIZE=1
...
;            xyouts, 0.6,0.675, tdiffline, COLOR = 255, /NORMAL, CHARSIZE=1
         ...
         xyouts, 0.19, 0.925, siteID+' (all)', COLOR = 70, /NORMAL, CHARSIZE=1
        ...
        ; OVERLAY CONVECTIVE RAINTYPE PDFS, IF ANY POINTS
         idxconvhist= WHERE( type4hist EQ RainType_convective, nconv )
         IF ( nconv GT 0 ) THEN BEGIN
           ...
           xyouts, 0.19, 0.85, 'PR (Conv)', COLOR = 30, /NORMAL, CHARSIZE=1
           xyouts, 0.19, 0.825, siteID+' (Conv)', COLOR = 70, /NORMAL, CHARSIZE=1
           plots, [0.14,0.18], [0.855,0.855], COLOR = 30, /NORMAL, LINESTYLE=2, thick=3
           plots, [0.14,0.18], [0.83,0.83], COLOR = 70, /NORMAL, LINESTYLE=2, thick=3
         ENDIF

        ; OVERLAY STRATIFORM RAINTYPE PDFS, IF ANY POINTS
         idxstrathist= WHERE( type4hist EQ RainType_stratiform, nstrat )
         IF ( nstrat GT 0 ) THEN BEGIN
           ...
           xyouts, 0.19, 0.9, 'PR (Strat)', COLOR = 30, /NORMAL, CHARSIZE=1
           xyouts, 0.19, 0.875, siteID+' (Strat)', COLOR = 70, /NORMAL, CHARSIZE=1
           plots, [0.14,0.18], [0.905,0.905], COLOR = 30, /NORMAL, LINESTYLE=1, thick=3
           plots, [0.14,0.18], [0.88,0.88], COLOR = 70, /NORMAL, LINESTYLE=1, thick=3
         ENDIF
...
myprbuf = plot_sweep_2_zbuf( zcor_in, mysite.site_lat, mysite.site_lon, xCorner, $
                             yCorner, pr_index, mygeometa.num_footprints, ifram, rntype4ppi, $

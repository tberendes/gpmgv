...
;   include file when it is specified.
;
;
...
pro gen_dpr_and_geo_match_x_sections, ncfilepr, use_db, show_orig_in, $
...
; cross sections of full-resolution DPR data.
;
...
status = fprep_dpr_geo_match_profiles( ncfilepr, $
   ...
   ; *** science, geolocation, and derived datasets ***
    ...
    PTRpctgoodrain=ptr_pctgoodrain, PTRclutterStatus=ptr_clutterStatus, $
  ...
  ; blank out reflectivity for clutter samples, if indicated
   IF KEYWORD_SET(declutter) THEN BEGIN
      ...
      ENDIF ELSE BEGIN
         ...
         goto, errorExit
      ENDELSE
     ; blank out reflectivity for all cluttered samples
      idx3d = pr_index      ; just for creation/sizing
      idx3d[*,*] = 0L       ; initialize all points to 0 (blank-out flag)
      idx3d[idxgoodenuff] = 2L  ; points to keep
      idx2blank = WHERE( idx3d EQ 0L, count2blank )
...
IF KEYWORD_SET(rhi_mode) THEN BEGIN
   ...
   ; flag whether to force RHI along a fixed azimuth
   IF N_ELEMENTS(fixed_RHI) EQ 1 THEN do_fixed_RHI = 1 ELSE do_fixed_RHI = 0
ENDIF ELSE do_fixed_RHI = 0    ; just define variable
...
WHILE ( !Mouse.Button EQ 1 ) DO BEGIN
   ...
   IF (do_fixed_RHI) THEN BEGIN
      print, "X: ", xppi, "  Y: ", yppi MOD ysize
     ; on the first click compute xppi and yppi at the angle fixed_RHI, then
     ; increment fixed angle outward by +/- 0.05 degrees with each subsequent
     ; cursor click
      offset = 0.1*(nclicks/2)*((-1)^nclicks)
      print, "RHI angle: ", STRING(fixed_RHI+offset, FORMAT='(F0.2)')
     ; use the point clicked on as the radial distance from GR
      cursrange=SQRT( (xppi-x_gr)^2 + ((yppi MOD ysize)-y_gr)^2 )
      xppif = cursrange * SIN((fixed_RHI+offset) * !DTOR) + x_gr
      yppif = cursrange * COS((fixed_RHI+offset) * !DTOR) + y_gr
      print, "X: ", xppif, "  Y: ", yppif MOD ysize
   ENDIF
     ...
     ; and STARTEND2SHOW)
      ...
      IF ( show_orig ) THEN BEGIN
        ; generate the PR full-resolution vertical cross section plot
         ...
         olddevice = !D.NAME
         scaledpr=1.
         dprxsect_struct = plot_dpr_xsection_zbuf( scanNumpr, raystartpr, rayendpr, $
...
pro dpr_and_geo_match_x_sections, ELEV2SHOW=elev2show, SITE=sitefilter, $
                                  ...
                                  RECALL_NCPATH=recall_ncpath, AZIMUTH=azimuth
...
print
print, "###############################################"
print, "#  DPR_AND_GEO_MATCH_X_SECTIONS: Version 2.0  #"
...
; if in RHI_MODE, validate any supplied AZIMUTH value
IF (rhi_mode) AND N_ELEMENTS(azimuth) EQ 1 THEN BEGIN
   IF FLOAT(azimuth) GE 0.0 AND FLOAT(azimuth) LE 360.0 THEN BEGIN
      fixed_RHI = FLOAT(azimuth)
   ENDIF ELSE BEGIN
      message, "Invalid AZIMUTH value, disabling option.", /info
   ENDELSE
ENDIF
   ...
   if nf eq 0 then begin
      ...
      for fnum = 0, nf-1 do begin
        ...
        ;
         ...
         gen_dpr_and_geo_match_x_sections, ncfilepr, use_db, show_orig, $
                        ELEV2SHOW=elev2show, PCT_ABV_THRESH=pctAbvThresh, $

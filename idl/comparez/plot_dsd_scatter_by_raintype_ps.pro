...
;
;       Internal Module: plot_dsd_scat_ps, pos, sur_title, siteID, xdata, ydata, ndata, $
;                            S2KU=s2ku, MIN_XY=minXY, MAX_XY=maxXY, UNITS=units, $
;                            Y_TITLE=y_title, X_TITLE=x_title, SAT_INSTR=sat_instr, $
;                            HEIGHTS=hgtStruc
;
...
;         of plotted points, in plot_dsd_scat_ps.
...
;       01/24/17, Bob Morris/GPM GV (SAIC)
;       - Changed name of plot_scat_ps to plot_dsd_scat_ps to avoid conflicts
;         with this procedure defined in other code.
;
...
pro plot_dsd_scat_ps, pos, sur_title, siteID, xdata, ydata, ndata, S2KU=s2ku, $
  ...
  for j=0,1 do begin   ;column, count from left -- also, strat/conv rain type index
    ...
    IF ( npts[0] GT 0 ) THEN BEGIN
         ...
         IF N_ELEMENTS(heights) NE 0 THEN BEGIN
           ; set up to add height/color annotations if on convective/Dm plot
            ...
            hgtstruc = { HEIGHTS:heights[idxsub], $
                         ...
                         PLOTLAB:plotlab }
         ENDIF
         plot_dsd_scat_ps, pos, sur_title, siteID, gvsub, prsub, nfound, $
      ...
      ENDIF ELSE BEGIN
         ...
         plot_dsd_scat_ps, pos, sur_title, siteID, S2KU=s2ku4sca, MIN_XY=minXY, $
                       MAX_XY=maxXY, UNITS=units[i], Y_TITLE=y_title, X_TITLE=x_title, $
                       SAT_INSTR=sat_instr
      ENDELSE
    ENDIF ELSE BEGIN
     ; print, 'no points at BB proximity level ', bblevstr[proxim]
      plot_dsd_scat_ps, pos, sur_title, siteID, S2KU=s2ku4sca, UNITS=units[i], $
                    Y_TITLE=y_title, X_TITLE=x_title, SAT_INSTR=sat_instr
    ENDELSE
  endfor

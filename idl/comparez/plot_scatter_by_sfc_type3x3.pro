...
;
;       Primary Module:  plot_scatter_by_sfc_type3x3, title, prz_in, gvz_in, $
...
;       - Created from plot_scatter_by_sfc_type, modified to do 3 columns
...
IF N_ELEMENTS(y_title_in) NE 1 THEN BEGIN
   ...
   ELSE y_title='TMI rainate, '+units
...
pro plot_scatter_by_sfc_type3x3, title, siteID, prz_in, gvz_in, raintype_in, $
...
pos = fltarr(4)
IF N_ELEMENTS(units) EQ 1 THEN unistring=units+' ' else unistring=''
...
for i=0,2 do begin     ;row, count from bottom -- also, surface type
  ...
  ENDIF ELSE BEGIN
     print, 'No '+unistring+'scatter points over ', SfcType_str[proxim]
  ENDELSE
  ...
  for j=0,2 do begin   ;column, count from left; also, strat/conv raintype index
    ...
    IF ( npts[i] GT 0 ) THEN BEGIN
      ...
      IF ( nfound GT 0 ) THEN BEGIN
         ...
         plot_scat, pos, subtitle, siteID, ncolor, charsz, gvsub, prsub, $
                    nfound, S2KU=s2ku4sca, MIN_XY=minXY, MAX_XY=maxXY, $
                    UNITS=units, Y_TITLE=y_title, X_TITLE=x_title
      ENDIF ELSE BEGIN
         print, 'No '+unistring+'scatter points in category ', $
                typestr[raincat], SfcType_str[proxim]
         plot_scat, pos, subtitle, siteID, ncolor, charsz, MIN_XY=minXY, $

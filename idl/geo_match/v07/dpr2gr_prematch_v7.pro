...
; Missing value same as that used ZFactor (e.g., -999.99 or -888)
      ...
      ; new V7 variables
      precipWater = (*ptr_swath.PTR_SLV).precipWater
...
;  PRINT, "numDPRrays for ", siteID, " ", numDPRrays
  print, "Matching scan ", DPR_scantype
  ...
  ; dpr_master_idx = data_GR2DPR.RAYNUM + data_GR2DPR.SCANNUM*RAYSPERSCAN 
   dpr_master_idx = data_GR2DPR.RAYNUM + (data_GR2DPR.SCANNUM-scan_offset)*RAYSPERSCAN ; TAB 2/22/22 subtract scan_offset
  ...
  ; in the ray.  If none, we'll skip the time-consuming GR LUT computations.
   ...
   for ifp = 0, numDPRrays-1 do begin
      ray_num = data_GR2DPR.RAYNUM[ifp]
      scan_num = data_GR2DPR.SCANNUM[ifp]-scan_offset ; TAB 2/22/22 scan offset fixes Ka difference in number of scans
 ...
 ; new DFR variables
     ...
     ;   (no more averaging/processing needed) with data from the product arrays
      ...
      IF ( countprgood GT 0 ) THEN BEGIN
         ...
         ; V7 variables
         tocdf_flagInversion[prgoodidx] = flagInversion[pr_idx_2get]
         if IS_DPR_FS then begin
...
;                  THEN print, "Clutter found at level,ray,scan ", ielev, raydpr, scandpr
               ...
               numDPRgates = 0
               precipWater_avg = get_dpr_layer_average( topCorrGate, botmCorrGate, $
                                                    scandpr, raydpr, precipWater, $
                                                    1.0, 0.0,  numDPRgates, $
...
;  Write sweep-level results/flags to netcdf file & close it up
   ...
   NCDF_VARPUT, ncid, 'ZFactorFinal', tocdf_corr_dbz
    NCDF_VARPUT, ncid, 'have_ZFactorFinal', DATA_PRESENT
   NCDF_VARPUT, ncid, 'precipWater', tocdf_precipWater
   ...
   NCDF_VARPUT, ncid, 'flagHail', tocdf_flagHail
    NCDF_VARPUT, ncid, 'have_flagHail', IS_DPR_FS ; This is present only for DPR FS swaths (Ku and Ka)
   NCDF_VARPUT, ncid, 'flagGraupelHail', tocdf_flagGraupelHail
    NCDF_VARPUT, ncid, 'have_flagGraupelHail', IS_DPR_FS ; This is present only for DPR FS swaths (Ku and Ka)
   NCDF_VARPUT, ncid, 'mixedPhaseTop', tocdf_mixedPhaseTop
...
;IF N_ELEMENTS(gr_nc_version) EQ 0 THEN grverstr = '2_3' ELSE BEGIN
IF N_ELEMENTS(gr_nc_version) EQ 0 THEN grverstr = '2_4' ELSE BEGIN

...
; All Rights Reserved.
;
; dpr2gr_prematch_v6.pro          Morris/SAIC/GPM_GV      March 2016
;
; DESCRIPTION
; -----------
...
; types (HS, MS, NS) that apply to the 2A file type (2ADPR, 2AKa, or 2AKu), and
...
; the function gen_dpr_geo_match_netcdf_v6().
;
...
;                       GRtoDPR_HS_MS_NS netCDF file for the site and orbit.
...
;                       file environs.inc.
;
; GR_NC_VERSION=gr_nc_version - Non-default value of the GRtoDPR_HS_MS_NS netCDF
...
;                               used for the version field in GRtoDPR_HS_MS_NS
;                               netCDF file names.
;
...
;                                identify the GRtoDPR_HS_MS_NS netCDF file that
...
;   2) PROCEDURE dpr2gr_prematch_scan_v6    (this file)
;   3) PROCEDURE dpr2gr_prematch_v6         (this file)
;
;
; CONSTRAINTS
; -----------
...
;         and written to a GRtoDPR_HS_MS_NS netCDF file by the program
;         polar2dpr_hs_ms_ns.pro, for each of the DPR and GR files listed
...
;         must have been used to create the GRtoDPR_HS_MS_NS netCDF file names
;         or the matching GRtoDPR_HS_MS_NS file will not be found.  Likewise,
;         if a NC_NAME_ADD parameter value was used in the generation of the
;         GRtoDPR_HS_MS_NS netCDF file names, the same value must be specified
;         as the NC_NAME_ADD parameter value for dpr2gr_prematch_v6().
;
;
; HISTORY
; -------
...
;*******************************************************************************
;
; dpr2gr_prematch_scan_v6
;
; DESCRIPTION
; -----------
; Performs a resampling and merger of DPR data to 3-D volumes in common with GR
; data previously volume matched to the DPR footprints, for any of the three DPR
; scan types (HS, MS, NS).  Only one scan type is processed at a time in a call
; to this procedure.

PRO dpr2gr_prematch_scan_v6, dpr_data, data_GR2DPR, dataGR, DPR_scantype, $
...
@dpr_geo_match_nc_structs_v6.inc
...
;      print, "Points with no regard to mean BB:"
  ...
  ; dimensions, also passing the global attribute values along
   ncfile = gen_dpr_geo_match_netcdf_v6( fname_netCDF, numDPRrays, $
...
PRO dpr2gr_prematch_v6, control_file, GPM_ROOT=gpmroot, DIRDPR=dir2adpr, $
...
WHILE NOT (EOF(lun0)) DO BEGIN 
  ...
  ; determine which PPS Version of GRtoDPR_HS_MS_NS netCDF file to match up
  ...
  ; so that it can become part of the matchup file name
   throwaway = gen_dpr_geo_match_netcdf_v6( GEO_MATCH_VERS=matchup_file_version )
...
;print, origFileDPRName
   ...
   CASE STRUPCASE(Instrument_ID) OF
       'KA' : BEGIN
                 ...
                 IF FILE_BASENAME(origFileKaName) EQ 'no_2AKA_file' THEN BEGIN
                    ...
                    goto, bailOut
                 ENDIF
                 ; 2AKA has HS and MS scan/swath types, read them all at once
                 DPR_scans = ['HS', 'MS']
      ...
      'DPR' : BEGIN
                 IF FILE_BASENAME(origFileDPRName) EQ 'no_2ADPR_file' THEN BEGIN
                    ...
                    goto, bailOut
                 ENDIF
                 ; 2ADPR has all 3 scan/swath types, read them all at once
                 DPR_scans = ['HS', 'MS', 'NS']
  ...
  ; matchup netCDF file for each site/scan
   ...
   FOR igv=0,nsites-1  DO BEGIN
     ...
     ; gen_dpr_geo_match_netcdf_v6() via dpr2gr_prematch_scan_v6()
     ...
     ; Get an uncompressed copy of the netCDF file - we never touch the original
      ...
      if (cpstatus eq 'OK') then begin
         ...
         data_MS=1  ; initialize to anything
         data_NS=1  ; initialize to anything
         ...
         IF error EQ 0 THEN BEGIN
            status=read_gr_hs_ms_ns_geo_match_netcdf_v6( ncfile1, $
...
;         IF (status EQ 1) then GOTO, bailOut
      ...
      for iscan = 0, N_ELEMENTS(DPR_SCANS)-1 DO BEGIN
         ...
         CASE STRUPCASE(DPR_scantype) OF
           ...
           'MS' : data_GR2DPR = DATA_MS
           'NS' : data_GR2DPR = DATA_NS
         ENDCASE
        ...
        ; check for the case of no footprints for the swath
         IF data_GR2DPR.NUMRAYS GT 0 THEN BEGIN
           ...
           ; generate the GRtoDPR netcdf matchup file path/name
            ...
            dpr2gr_prematch_scan_v6, dpr_data, data_GR2DPR, dataGR, DPR_scantype, $
               DPR_version, mygeometa, mysweeps, mysite, myflags, myfiles, $
               grverstr, Instrument_ID, infileNameArr, fname_netCDF, $
               PLOT_PPIS=plot_PPIs, SCORES=run_scores, $
               NC_DIR=nc_dir, FLAT_NCPATH=flat_ncpath, NC_NAME_ADD=ncnameadd, $

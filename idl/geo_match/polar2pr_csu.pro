...
; All Rights Reserved.
;
; polar2pr_csu.pro          Morris/SAIC/GPM_GV      September 2008
;
; DESCRIPTION
; -----------
...
PRO polar2pr_csu, control_file, range_threshold_km, PR_ROOT=prroot, DIR1C=dir1c, $
              DIR2A=dir2a, DIR2B=dir2b, DIRGV=dirgv, SCORES=run_scores, $
              PLOT_PPIS=plot_PPIs, PLOT_BINS=plot_bins, NC_DIR=nc_dir, $
              NC_NAME_ADD=ncnameadd

FORWARD_FUNCTION plot_bins_bailout
...
DATA_PRESENT = 0
NO_DATA_PRESENT = 1  ; default fill value, defined in gen_geo_match_netcdf.pro
...
FOR igv=0,nsites-1  DO BEGIN
  ...
  ; 1CUF file unique pathname
   ...
   parsed=STRSPLIT( dataGV, '|', /extract )
  ...
  ; adding the well-known (or local) path to get the fully-qualified file name:
   ...
   IF ( siteID EQ lastsite ) THEN BEGIN
      PRINT, "Multiple 1CUF files for event = ", event_num, ", site = ", $
              siteID, ", skipping."
      lastsite = siteID
      GOTO, nextGVfile
   ENDIF
...
;   PRINT, igv+1, ": ", file_1CUF
  ...
  ; find the CP2 radar volume with the attenuation field
   gv_atten_field = 'AH'
   atten_vol_num = get_site_specific_z_volume( siteID, radar, UF=gv_atten_field )
   IF ( atten_vol_num LT 0 )  THEN BEGIN
      PRINT, ""
      PRINT, "Error finding attenuation volume in radar structure from file ", file_1CUF
      PRINT, ""
      GOTO, nextGVfile
   ENDIF
...
;-------------------------------------------------------------------------------
  ...
  ; the in-range subset of scans.  THESE ARE NOT WRITTEN TO NETCDF FILE - YET.
   pr_x0 = FLTARR(RAYSPERSCAN, nscans2do)
   pr_y0 = FLTARR(RAYSPERSCAN, nscans2do)
   subset_scan_num = 0
; DON'T NEED TO DO THIS IN A LOOP IF SUBARRAYS OF LAT AND LON ARE PASSED TO MAP_PROJ_FORWARD !!
   FOR scan_num = start_scan,end_scan  DO BEGIN
      FOR ray_num = 0,RAYSPERSCAN-1  DO BEGIN
         XY_km = map_proj_forward( prlons[ray_num,scan_num],$
                                   prlats[ray_num,scan_num], $
                                   map_structure=smap ) / 1000.
         pr_x0[ray_num, subset_scan_num] = XY_km[0]
         pr_y0[ray_num, subset_scan_num] = XY_km[1]
      ENDFOR
      subset_scan_num = subset_scan_num + 1
   ENDFOR
...
;-------------------------------------------------------------------------------
      ...
      FOR ray_num = 0,RAYSPERSCAN-1  DO BEGIN
	   ...
	   ; while we are here, compute bright band height for the ray
           ...
           ;   "off-scan-edge", and compute the extrapolated location parameters
            IF ( ray_num EQ 0 OR ray_num EQ RAYSPERSCAN-1 ) THEN BEGIN
  ...
  ; NOTE WE DON'T SAVE THE SURFACE X,Y FOOTPRINT CENTERS AND CORNERS TO NETCDF
   ...
   IF ( numPRinrange GT 0 ) THEN BEGIN
     ...
     ;   3-D science and status variable: 
      tocdf_gv_dbz = MAKE_ARRAY(numPRrays, num_elevations_out, /float, VALUE=FLOAT_RANGE_EDGE)
      tocdf_gv_atten = MAKE_ARRAY(numPRrays, num_elevations_out, /float, VALUE=FLOAT_RANGE_EDGE)
      tocdf_max_gv_atten = MAKE_ARRAY(numPRrays, num_elevations_out, /float, VALUE=FLOAT_RANGE_EDGE)
  ...
  ; in code in a separate file, included below:
   ...
   @polar2pr_resampling_csu.pro

  ; ================================================================================================

  ; Create a netCDF file with the proper numPRrays and num_elevations_out dimensions
   IF N_ELEMENTS(ncnameadd) EQ 1 THEN BEGIN
      fname_netCDF = NCGRIDSOUTDIR+'/'+GEO_MATCH_PRE+siteID+'.'+DATESTAMP+'.' $
                      +orbit+'.'+ncnameadd+NC_FILE_EXT
   ENDIF ELSE BEGIN
      fname_netCDF = NCGRIDSOUTDIR+'/'+GEO_MATCH_PRE+siteID+'.'+DATESTAMP+'.' $
                      +orbit+NC_FILE_EXT
   ENDELSE
   ncfile = gen_geo_match_netcdf_csu( fname_netCDF, numPRrays, tocdf_elev_angle, $
                                  gv_z_field, PR_version )
...
;  Write sweep-level results/flags to netcdf file
   ...
   NCDF_VARPUT, ncid, 'GR_attenuation', tocdf_gv_atten         ; data
    NCDF_VARPUT, ncid, 'have_GR_attenuation', DATA_PRESENT     ; data presence flag
   NCDF_VARPUT, ncid, 'max_GR_attenuation', tocdf_max_gv_atten         ; data
    NCDF_VARPUT, ncid, 'have_max_GR_attenuation', DATA_PRESENT     ; data presence flag

...
; All Rights Reserved.
;
; polar2dpr_hs_ms_ns.pro          Morris/SAIC/GPM_GV      Feb 2016
;
; DESCRIPTION
; -----------
...
PRO polar2dpr_hs_ms_ns, control_file, range_threshold_km, GPM_ROOT=gpmroot, $
...
;   DPR_scantype = parsed[6]        ; HS, MS, or NS
  ...
  ; Call gen_gr_hsmsns_geo_match_netcdf with the option to only get current file version
  ; so that it can become part of the matchup file name
   throwaway = gen_gr_hsmsns_geo_match_netcdf( GEO_MATCH_VERS=matchup_file_version )
...
;   ENDIF
  ...
  ;-----------------------------------------------------------------------------
   ...
   ; gen_gr_hsmsns_geo_match_netcdf()
  ...
  ; attribute values along
   ...
   ncfile = gen_gr_hsmsns_geo_match_netcdf( fname_netCDF, $
                                            num_fp_by_source[0], $
                                            num_fp_by_source[1], $
                                            num_fp_by_source[2], $
                                            tocdf_elev_angle, $
...
;      SAVE, dpr_echoes, numDPRrays, file='/tmp/dpr_echoesBy3.sav'
  ...
  ; in code in a separate file, included below:
   ...
   @polar2dpr_hs_ms_ns_resampling.pro

...
max_sep = 7.95    ; use fixed radius of influence from a real case
...
; replace /1_1/ in pathname with /1_11/
len=STRLEN(mygeomatchfile)
idxverdir=STRPOS(mygeomatchfile, '/1_1/')
file121=STRMID(mygeomatchfile,0,idxverdir) + '/1_11/' $
...
IF idxncvers EQ -1 THEN BEGIN
   message, "File "+mygeomatchfile+" not named *.1_1.nc", /info
   stop
   GOTO, regularExit
ENDIF

outfile = STRMID(file121,0,idxncvers)+'1_11.nc'
;outfile = file121
...
if(cpstatus eq 'OK') then begin
  ...
  ; create data field arrays of correct dimensions and read data fields
   pr_index=lonarr(nfp)
   latitude = FLTARR(nfp,nswp)   ; along-sweep parallax-adjusted Lat/Lons
   longitude = FLTARR(nfp,nswp)
   XMIlatitude = FLTARR(nfp)     ; at-surface lat/lons for VPR
   XMIlongitude = FLTARR(nfp)

   status = read_gprof_geo_match_netcdf( myfile, latitude=latitude, $
               longitude=longitude, XMIlatitude=XMIlatitude, $
               XMIlongitude=XMIlongitude, tmi_idx_long=pr_index )
...
avg_blockage_vpr = avg_blockage

; creat a sweep-level version of the at-surface lat/lons to be populated and
; passed to compute_blockage()
latitude_vpr = FLTARR(nfp,nswp)
longitude_vpr = FLTARR(nfp,nswp)
...
for iswp = 0, nswp-1 do begin
  ; fill this level's VPR lat/lon arrays with the surface values
   latitude_vpr[*,iswp] = XMIlatitude
   longitude_vpr[*,iswp] = XMIlongitude
 ...
 ; find the nearest fixed angle to this measured sweep elevation
  ...
  ; find the blockage file for this sweep
   ...
   IF nblkfil EQ 1 THEN BEGIN
      ...
      compute_blockage, iswp, idxpractual, avg_blockage_vpr, thisfile, $
                        site_lat, site_lon, latitude_vpr, longitude_vpr, ZERO_FILL=0
   ENDIF ELSE BEGIN
     ...
     ; we are above the last blocked level and just need to set blockages to zero
      IF have_GR_blockage EQ DATA_PRESENT THEN BEGIN
         ...
         compute_blockage, iswp, idxpractual, avg_blockage_vpr, ZERO_FILL=1
      ENDIF ELSE BEGIN
...
versid = NCDF_VARID(cdfid, 'version')
ncdf_varput, cdfid, versid, 1.11
NCDF_VARPUT, cdfid, 'GR_blockage_slantPath', avg_blockage
NCDF_VARPUT, cdfid, 'have_GR_blockage_slantPath', have_GR_blockage
NCDF_VARPUT, cdfid, 'GR_blockage_VPR', avg_blockage_VPR
NCDF_VARPUT, cdfid, 'have_GR_blockage_VPR', have_GR_blockage
...
pro append_blockage_geomatch_gprof, ncsitepath, DIR_BLOCK=dir_block, $
                                    VERBOSE=verbose

IF N_ELEMENTS(ncsitepath) EQ 1 THEN pathpr=ncsitepath+'*' $
ELSE pathpr = '/data/gpmgv/netcdf/geo_match/GPM/GMI/2AGPROF/V03D/1_1/2015'

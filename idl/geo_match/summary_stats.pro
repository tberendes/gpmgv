...
;returns mean,stdev,max  
  ...
  ENDIF
  if (total(weights) eq 0.0) then begin
  ...
  max_uncensored=max(data_uncensored)   
  IF(log_in) THEN BEGIN
      ;var=1.0/scale*alog10(1+var/(avg^2)) ;estimate of stddev of lognormal distr. (Quan & Zhang, 2003, doi: 10.1002/sim.1525)      
      ;avg=1.0/scale*alog10(avg)-var*1.0/2
      var=(sqrt(var)/(avg*alog(10)))^2 ;propagation of uncertainty Bevington and Robinson (2002, 48)      
      avg=1.0/scale*alog10(avg)
      if(max_uncensored gt 0.0) THEN BEGIN
          max_uncensored=1.0/scale*alog10(max_uncensored)
      endif else max_uncensored=-999.
  ENDIF  

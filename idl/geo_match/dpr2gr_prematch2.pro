...
; All Rights Reserved.
;
; dpr2gr_prematch.pro          Morris/SAIC/GPM_GV      March 2016
;
; DESCRIPTION
; -----------
...
;   2) PROCEDURE dpr2gr_prematch_scan    (this file)
;   3) PROCEDURE dpr2gr_prematch         (this file)
;
;
; CONSTRAINTS
; -----------
...
;         as the NC_NAME_ADD parameter value for dpr2gr_prematch().
;
;
; HISTORY
; -------
...
; 03/22/16 by Bob Morris, GPM GV (SAIC)
...
PRO dpr2gr_prematch_scan, dpr_data, data_GR2DPR, dataGR, DPR_scantype, $
...
PRO dpr2gr_prematch2, control_file, GPM_ROOT=gpmroot, DIRDPR=dir2adpr, $
                      DIRKU=dir2aku, DIRKA=dir2aka, DIR_GV_NC=dir_gv_nc,  $
                      GR_NC_VERSION=gr_nc_version, $
...
WHILE NOT (EOF(lun0)) DO BEGIN 
  ...
  ; generate the GR netcdf matchup file paths
   IF KEYWORD_SET(flat_ncpath) THEN BEGIN
      NC_GR_DIR = dir_gv_nc
   ENDIF ELSE BEGIN
      NC_GR_DIR = dir_gv_nc+'/GPM/2ADPR/'+DPR_version+'/'+grverstr+ $
  ...
  ; matchup netCDF file for each site/scan
   ...
   FOR igv=0,nsites-1  DO BEGIN
     ...
     ; gen_dpr_geo_match_netcdf() via dpr2gr_prematch_scan()
     ...
     ; generate the GR netcdf matchup file path/name CHECK DIRECTORY STRUCTURE!!!
      IF N_ELEMENTS(ncnameadd) EQ 1 THEN BEGIN
         gr_netcdf_file = NC_GR_DIR+'/'+GR_DPR_GEO_MATCH_PRE+siteID+'.'+DATESTAMP+'.' $
                         +orbit+'.'+DPR_version+'.'+grverstr+'.'+ncnameadd+NC_FILE_EXT
      ENDIF ELSE BEGIN
         gr_netcdf_file = NC_GR_DIR+'/'+GR_DPR_GEO_MATCH_PRE+siteID+'.'+DATESTAMP+'.' $
                         +orbit+'.'+DPR_version+'.'+grverstr+NC_FILE_EXT
      ENDELSE
     ...
     ; Get an uncompressed copy of the netCDF file - we never touch the original
      ...
      for iscan = 0, N_ELEMENTS(DPR_SCANS)-1 DO BEGIN
        ...
        ; generate the GRtoDPR netcdf matchup file path/name
         ...
         dpr2gr_prematch_scan, dpr_data, data_GR2DPR, dataGR, DPR_scantype, $
               DPR_version, mygeometa, mysweeps, mysite, myflags, myfiles, $
               grverstr, Instrument_ID, infileNameArr, fname_netCDF, $
               PLOT_PPIS=plot_PPIs, SCORES=run_scores, $
               NC_DIR=nc_dir, FLAT_NCPATH=flat_ncpath, NC_NAME_ADD=ncnameadd, $

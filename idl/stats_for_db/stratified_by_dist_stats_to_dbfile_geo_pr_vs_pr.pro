;+
; stratified_by_dist_stats_to_dbfile_geo_pr_vs_pr.pro 
; - Morris/SAIC/GPM_GV   October 2008
;
; DESCRIPTION
; -----------
; Reads PR raw and attenuation-corrected reflectivity and spatial fields from
; geo_match netCDF files, builds index arrays of categories of range, rain type,
; bright band proximity (above, below, within), and height (13 categories, 
; 1.5-19.5 km levels); and an array of range category (0-50,50-100,100-150 km).
; Computes max and mean raw and corrected reflectivity and mean corrected-raw
; reflectivity differences and standard deviation of the differences for each
; of the 13 height levels for points within 100 km of the ground radar.
;
...
pro stratified_by_dist_stats_to_dbfile_geo_pr_vs_pr, PCT_ABV_THRESH=pctAbvThresh, $
...
dbfile = '/data/tmp/StatsByDistToDBbyGeoPRx2newBBnew1C21rntypePR.unl'
...
IF ( pctAbvThreshF GT 0.0 ) THEN BEGIN
   IF ( pctAbvThreshF EQ 100.0 ) THEN BEGIN
     ; clip to the 'good' points, where ALL bins in average were above threshold
      idxallgood = WHERE( prexp GT 0 AND zrawrej EQ 0 $
                     AND zcorrej EQ 0 AND gvrej EQ 0, countgood )
   ...
   ENDIF ELSE BEGIN
     ...
     ; were above threshold
      ...
      ENDIF ELSE BEGIN
         ...
         pctgoodraw = 100.0 * FLOAT( prexp[idxexpgt0] - zrawrej[idxexpgt0] ) / gvexp[idxexpgt0]
         idxgoodpct = WHERE( pctgoodpr GE pctAbvThreshF AND pctgoodraw GE pctAbvThreshF $
...
print, 'Mean BB old way = ', FIX(MEAN(bb[idxbbdef]))/1000.
ENDIF ELSE BEGIN

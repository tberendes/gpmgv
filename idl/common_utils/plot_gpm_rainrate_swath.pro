...
; All Rights Reserved.
;
; plot_gpm_rainrate_swath.pro
;
; DESCRIPTION
; -----------
; Produces map plot of along-swath, volume-matched PR, COM, and GMI
; rainrate ('zdata'), in either a static window (GMI only), or 
...
;                 within the narrower PR swath (PRECUT=1), or all the GMI
...
; - Created from plot_rainrate_swath.pro
;
...
pro plot_gpm_rainrate_swath, matchup, ANIMATE=animate, PRECUT=precut, DO_PS=do_ps
...
IF (precut) THEN BEGIN
   ...
   IF WHERE(tag_names(matchup) EQ 'MATCHUPMETA') NE -1 THEN BEGIN
      ...
      file2a12 = FILE_BASENAME(matchup.filesmeta.file_2agprof)
      parsed = STRSPLIT(file2a12, '.', /EXTRACT)
      orbit = LONG(parsed(5))
      version = matchup.matchupmeta.PPS_version
   ENDIF ELSE BEGIN
...
; plot the GMI rainrate on a map, either in a static window (animate=0) or
; in the Z-buffer if building an animation of all 3 rainrate sources (animate=1)
zdata = matchup.gmirain[min_scan:max_scan, min_ray:max_ray]
buffgmi = plot_swath_2_map(zdata, center_lat, center_lon, $

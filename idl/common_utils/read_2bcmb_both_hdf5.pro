...
; All Rights Reserved.
;
; read_2bcmb_hdf5.pro         Bob Morris, GPM GV/SAIC   June 2013
;
; DESCRIPTION
; -----------
...
FUNCTION read_2bcmb_both_hdf5, file, DEBUG=debug, READ_ALL=read_all, SCAN=scan2read
   ...
   IF prodname NE '2BCMB' AND prodname NE '2BCMBT' THEN BEGIN
      h5g_close, group_id
      h5f_close, file_id
      message, "Illegal product type '" + prodname + "', must be '2BCMB' or '2BCMBT'"
   ENDIF

   ; define the swath groups according to product type

   IF prodname EQ '2BCMB' THEN nscans=2 ELSE nscans=1
   IF filestruc.NUMBEROFSWATHS NE nscans THEN BEGIN
      h5g_close, group_id
      h5f_close, file_id
      IF prodname EQ '2BCMB' THEN BEGIN
         message, "Expect 2 swaths in 2BDPRGMI product, have " + $
                  STRING(filestruc.NUMBEROFSWATHS, FORMAT='(I0)')
      ENDIF ELSE BEGIN
         message, "Expect one swath in 2BPRTMI product, have " + $
                  STRING(filestruc.NUMBEROFSWATHS, FORMAT='(I0)')
      ENDELSE
   ENDIF
   ...
   IF N_ELEMENTS(onescan) NE 0 THEN BEGIN
      IF prodname EQ '2BCMB' THEN BEGIN
        ; make sure onescan value is 'MS' or 'NS', if specified
         IF TOTAL( STRCMP(onescan,['MS','NS']) ) NE 1.0 THEN BEGIN
            message, "Requested Scan type " + onescan + $
                     " not valid for 2BDPRGMI product - must be MS, NS or left unspecified."
         ENDIF
      ENDIF ELSE BEGIN
        ; make sure onescan value is 'NS', if specified
         IF STRMATCH(onescan, 'NS') EQ 0b THEN BEGIN
            message, "Requested Scan type " + onescan + $
                     " not valid for 2BPRTMI product, must be NS or left unspecified."
         ENDIF
      ENDELSE
   ENDIF ELSE BEGIN
     ; set onescan to NS for 2BPRTMI product so that we only attempt to read
     ; this scan and set the MS structure to UNREAD
      IF prodname EQ '2BCMBT' THEN onescan = 'NS'
   ENDELSE
   ...
   h5g_close, group_id
   h5f_close, file_id
   outStruc = { FileHeader:filestruc, MS:MS, NS:NS }

...
FUNCTION read_dpr_geo_match_netcdf_new, ncfile, DIMS_ONLY=dims_only,          $
...
IF KEYWORD_SET(dims_only) THEN BEGIN
   ...
   ENDIF ELSE BEGIN
      IF STRUPCASE(matchupmeta.TYPENAME) NE 'DPR_GEO_MATCH_META' THEN BEGIN
...
ENDIF ELSE BEGIN
  ; check the type of any structures to be populated
   ...
   IF N_ELEMENTS(sweepsmeta) NE 0 THEN BEGIN
      IF STRUPCASE(sweepsmeta.TYPENAME) NE 'GR_SWEEP_META' THEN BEGIN
   ...
   IF N_ELEMENTS(sitemeta) NE 0 THEN BEGIN
      IF STRUPCASE(sitemeta.TYPENAME) NE 'GR_SITE_META' THEN BEGIN
   ...
   IF N_ELEMENTS(fieldFlags) NE 0 THEN BEGIN
      IF STRUPCASE(fieldFlags.TYPENAME) NE 'DPR_GR_FIELD_FLAGS' THEN BEGIN
   ...
   IF N_ELEMENTS(filesmeta) NE 0 THEN BEGIN
      IF STRUPCASE(filesmeta.TYPENAME) NE 'DPR_GR_INPUT_FILES' THEN BEGIN
...
; Get the names of the global attributes in the file.  Output has a bunch of other
; lines preceding the list of global attributes, and the list has to be parsed to
...
FOR ncattnum = 0, N_ELEMENTS(parsed)-1 DO BEGIN
   ncfileatts=parsed[ncattnum]    ; grab this 3-element STRARR
   thisncatt=ncfileatts[1]        ; pull the attribute name from the array
   CASE thisncatt OF

...
; beginning and ending DPR scan line and beginning and ending DPR rays in the
; data product for which a cross section of data is to be plotted, and the
; bright band mean height and an optional plot title, and generates a pair of
; vertical cross sections of full-resolution DPR reflectivity in the form of a
; two-panel Z-buffer image and an associated color table.  The top plot uses the
; discrete color table of the PPI plots with a 3 dBZ color binning, and the
; lower plot uses a 'smooth' IDL color table showing 1 dBZ color separation.
; A parallax correction for the scan angle of the DPR is applied to the height
; of each 125- or 250-m DPR gate.
;
; The normal (legacy) mode is to plot a cross section along a DPR scan line, in
; which case the values for the beginning and ending scan line must be equal.
; The new RAY_MODE option for cross sections plots cross sections parallel to
; the satellite orbit track at a fixed ray number (scan angle), in which case
; the values for the beginning and ending ray number must be equal.
;
...
; to the number of rays to be plotted, either in the normal mode of plotting
; along one DPR scan, or the mode of plotting at a fixed ray number.  If the
; PLOTBOUNDS parameter is specified, the window is sized for a fixed number of
; DPR footprints (normally to a limit of all 49/25/24 rays in the scan for plots
; along a scan line, or a range equal to the difference between the first and
; last scan lines in the full data set for plots at a fixed ray number), a fixed
; width is assigned for each ray and the window, and rays are plotted in fixed
; locations as defined by their ray (scan) number in the scan (along the track).
;
...
; PARAMETERS
; ----------
; scanStart  - DPR-product-relative number of the starting DPR scan of the data
;              to be plotted in the cross section.  Zero-based array index.
;
; scanEnd    - DPR-product-relative number of the ending DPR scan of the data
;              to be plotted in the cross section.  Zero-based array index.
;
; raystart   - Starting ray of the scan to plot in the cross section.  Zero-
;              based array index into the full data array.
;
; rayend     - As above, but ending ray to plot in the cross section.
;
...
;              according to the number of rays between plotbounds[0] and
;              plotbounds[1]. In essence, the cross-section domain is set
;              up in a fixed manner based on the this number of rays, where
;              the plotted locations of each ray depends on the values of
;              plotbounds, raystart, and rayend. 
;
...
;                   raystart and rayend are used to label the L/R edges of
;                   the x-section.
;
...
; 02/19/16  Morris/GPM GV/SAIC
; - Added RHI_MODE parameter to disable check that plotbounds[1] is LE the
;   number of rays in the data, since we can clip the number of rays for RHIs
;   but still want a fixed window size for looping over the RHI angles.
; 04/12/16  Morris/GPM GV/SAIC
; - Replaced single scanNumpr parameter with scanStart and scanEnd parameters to
;   support cross sections at a fixed ray number.
; 05/04/16  Morris/GPM GV/SAIC
; - Changed initialization of background color of cross-section area of the plot
;   to below-threshold color rather than black (0).
; - Added logic to prevent error in case of a single footprint (start and end
;   values are the same for both ray and scan).
;
;
...
function plot_dpr_xsection_zbuf, scanStart, scanEnd, raystart, rayend, z_data, $
                                 ...
                                 SOURCELABEL=sourceLabelIn, RHI_MODE=rhi_mode

; "Include" file for PR-product-specific parameters (i.e., RAYSPERSCAN):
@dpr_params.inc

; check the necessary constraints on scanStart, scanEnd, raystart, and rayend
; -- if we have a range of scans (scanStart NE scanEnd), then raystart must
;    equal rayend (cross section at one fixed ray angle)
; -- if we have a range of rays (rayStart NE rayEnd), then scanStart must
;    equal scanEnd (cross section along one scan line)
IF (scanStart NE scanEnd) AND (raystart NE rayEnd) THEN $
   message, "Illegal ranges for starting/ending ray/scan."

; which dimension is the cross section is along?:  scan (crossTrack) or ray
IF (raystart NE rayEnd) THEN BEGIN
   crossTrack=1
ENDIF ELSE BEGIN
  ; check situation where we might have a single footprint
   IF scanStart EQ scanEnd THEN BEGIN
     ; one or the other of the single scan or ray number must be between
     ; the plotbounds, see which one it is to determine crossTrack
     ; -- THIS IS NOT GUARANTEED TO WORK IN ALL CASES IF THE RANGE OF SCAN
     ;    NUMBERS OVERLAPS THE RANGE OF RAY NUMBERS
      IF raystart GE plotbounds[0] AND rayEnd LE plotbounds[1] $
         THEN crossTrack=1 ELSE crossTrack=0
   ENDIF ELSE crossTrack=0 
ENDELSE
...
skip_check = KEYWORD_SET(rhi_mode)

;IF N_ELEMENTS( flagEchoIn ) NE 0 THEN BEGIN
;   save, scanStart, scanEnd, raystart, rayend, z_data, flagEchoIn, $
;         clutterfreebin, file='/tmp/savedVars.sav'
; FILTER_CLUTTER IS NOT SET UP FOR scanStart, scanEnd, raystart, rayend, 
; DO NOT CALL IT HERE !!!
;   sstatus = filter_clutter( scanNumpr, raystart, rayend, z_data, $
...
cos_inc_angle = REFORM(cos_inc_angle_all[raystart:rayend, scanStart:scanEnd])

; from the full DPR Z array, extract the ray profiles to be plotted
; for the fixed scan or ray
xsect2d = z_data[*, raystart:rayend, scanStart:scanEnd]  ;DPR order is bin,ray,scan
idxclutter = WHERE( xsect2d LT 0.0, nclutr )
IF ( nclutr GT 0 ) THEN xsect2d[idxclutter] = 0.0

; from the same array, find the number of rays per scan (2nd dimension) if doing
; cross-track, or the number of scans in dataset (3rd dimension) if along-track
sz_z = SIZE(z_data, /DIMENSIONS)
IF crossTrack THEN nraysper = sz_z[1] ELSE nraysper = sz_z[2]  ; use later in error check
...
; get rid of dimension(s) of size 1
trimmed = REFORM( xsect2d )
ts = SIZE(trimmed)

; how many ray profiles are we plotting this time?
IF ts[0] EQ 2 THEN BEGIN
  ; 2 or more rays to plot, have 2-D xsect2d array
   nrays = ts[2]
   nbins = ts[1]
ENDIF ELSE BEGIN
  ; only one ray profile to plot, have 1-D xsect2d array
   nrays = 1
   nbins = ts[1]
ENDELSE
...
IF (N_ELEMENTS(clutterfreebin) GT 0) THEN BEGIN
   clutterbinnum = clutterfreebin[raystart:rayend, scanStart:scanEnd]
...
IF (N_ELEMENTS(surfBin) GT 0) THEN BEGIN
  ; compute surface gate number based on gate 0 at top
   sfcbinnum = surfbin[raystart:rayend, scanStart:scanEnd]
  ; adjust surface gate number for flipped Z array
   sfcbinnum = (nbins-sfcbinnum)>0
ENDIF

IF N_ELEMENTS( BB_hgt_in ) GT 0 THEN BEGIN
   BB_hgt = BB_hgt_in[raystart:rayend, scanStart:scanEnd]/1000.  ; scale m to km
...
;    of rays defined in plotbounds, or on the number of PR/GR overlap rays
;    in the current scan
      ...
      2 : BEGIN
           ; error check the values in plotbounds
            IF plotbounds[0] GT plotbounds[1] THEN BEGIN
               doFixed = 0
               print, "In plot_dpr_xsection_zbuf, illegal value for PLOTBOUNDS ", $
                  "must be INTARR(2), with plotbounds[0] LE plotbounds[1]"
               print, "plotbounds: ", plotbounds
               goto, errorExit
            ENDIF ELSE BEGIN
               IF ( plotbounds[0] LT 0 OR plotbounds[1] GT (nraysper-1) ) $
               AND skip_check NE 1 THEN BEGIN
                  doFixed = 0
                  print, "In plot_dpr_xsection_zbuf, illegal value for ", $
                     "PLOTBOUNDS, must have plotbounds[0] GE 0, and ", $
                     "plotbounds[1] LE ", str_nray
                  print, "plotbounds: ", plotbounds
                  goto, errorExit
               ENDIF
            ENDELSE
            IF crossTrack THEN BEGIN
              ; check that raystart, rayend values are between plotbounds
               IF (plotbounds[0] LE raystart) AND $
                  (plotbounds[1] GE rayend) THEN BEGIN
                  doFixed = 1
               ENDIF ELSE BEGIN
                  doFixed = 0
                  print, "In plot_dpr_xsection_zbuf, values for raystart, rayend ", $
                         "exceed plot limits defined in plotbounds."
                  print, "plotbounds: ", plotbounds
                  print, "raystart, rayend: ", raystart, rayend
                  goto, errorExit
               ENDELSE
             ENDIF ELSE BEGIN
              ; check that scanstart, scanend values are between plotbounds
               IF (plotbounds[0] LE scanstart) AND $
                  (plotbounds[1] GE scanend) THEN BEGIN
                  doFixed = 1
               ENDIF ELSE BEGIN
                  doFixed = 0
                  print, "In plot_dpr_xsection_zbuf, values for scanstart, scanend ", $
                         "exceed plot limits defined in plotbounds."
                  print, "plotbounds: ", plotbounds
                  print, "scanstart, scanend: ", scanstart, scanend
                  goto, errorExit
               ENDELSE
             ENDELSE
          END
...
IF doFixed THEN BEGIN
   nrayswide = plotbounds[1]-plotbounds[0]+1
...
IF doFixed THEN xoff = raywidth*(thestart-plotbounds[0]) $
...
IF KEYWORD_SET( label_by_raynum ) THEN BEGIN
   leftlbl=STRING(theStart+1, FORMAT='(I0)')
   rightlbl=STRING(theEnd+1, FORMAT='(I0)')
ENDIF ELSE BEGIN
   leftlbl = 'A'
   rightlbl = 'B'
ENDELSE
...
IF doFixed THEN BEGIN
  ...
  ; dealing with window bounds
   xlblposright = $
      (((theEnd-plotbounds[0])*raywidth + raywidth/2) < (xsize-18)) > 6
   xlblposleft = ((theStart-plotbounds[0])*raywidth > 6) < (xsize-18)
  ; deal with potential label overlaps
  IF xlblposright LE (xlblposleft+20) THEN BEGIN
      IF theStart GT 25 THEN BEGIN
         yoffleft = 15
...
xsectimg[*,*] = 128b         ; re-init image array. now using upper half of byte
for k = 0, nrays-1 DO BEGIN
   ; first, start at the at-surface bin, if available
   IF (N_ELEMENTS(surfBin) GT 0) THEN binstart=sfcbinnum[k] ELSE binstart=0
   for l = 0, nbins - 1 DO BEGIN

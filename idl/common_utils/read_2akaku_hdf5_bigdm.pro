...
FUNCTION read_2akaku_hdf5_bigdm, file, DEBUG=debug, READ_ALL=read_all, SCAN=scan2read
   ...
   if n_elements(file) eq 0 then begin
      ...
      file=dialog_pickfile(FILTER=filters, $
          ...
          PATH='/data/gpmgv/netcdf/geo_match/GPM/2AKu/NS/ITE109/1_21/2014')
   ...
   CASE prodname OF
      ...
      '2AKu' : snames=['NS']         ; ditto, 2A-Ku
       ELSE  : BEGIN
                 h5g_close, group_id
                 h5f_close, file_id
                 message, "Illegal product type '" + prodname + $
                        "', must be '2AKa' or '2AKu'"
               END
   ENDCASE

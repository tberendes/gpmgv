...
; All Rights Reserved.
;
; fprep_dpr_geo_match_profiles_v88.pro
; - Morris/SAIC/GPM_GV  June 2018
;
; DESCRIPTION
; -----------
; A one-off version of fprep_dpr_geo_match_profiles.pro, to read specially-
; prepared version 8_8 matchup files with DPR phase variables included in the
; netCDF files, for Ali Tokay's study.
;
...
; 06/19/18 - Morris/NASA/GSFC (SAIC), GPM GV
; - Added PTRphase=ptr_phase, PTRAGLphaseHgt=ptr_phaseHeightAGL, and
;   PTRNS_phase=ptr_phaseNearSurface keywords/parameters and processing for
;   one-off version 8_8 files.
;
...
FUNCTION fprep_dpr_geo_match_profiles_v88, ncfilepr, heights_in, $
...
if (cpstatus eq 'OK') then begin
 ...
 ; 2nd call to read_dpr_geo_match_netcdf()
   ...
   IF error EQ 0 THEN BEGIN
      status = read_dpr_geo_match_netcdf_phase_v88( ncfile1, matchupmeta=mygeometa, $
                                              DIMS_ONLY=1 )
   ENDIF ELSE BEGIN
      help,!error_state,/st
      Catch, /Cancel
 ...
 ; and metadata in structures
  ...
  phase = intarr(nfp)
  phaseHeightAGL = fltarr(nfp)
  phaseNearSurface = intarr(nfp)
 ...
 ; still unpopulated as of 1_21.  Read it at/after 1_21 anyway.
  ...
  ycorner=fltarr(4,nfp,nswp)
   ...
   ; surface-level DPR rainrate variables and misc. footprint characteristics:
    ...
    max_dbz250raw=maxzraw250, pridx_long=pr_index, phase_int=phase,            $
    AGLphaseHgt=phaseHeightAGL, NS_phase_int=phaseNearSurface )
   ENDIF ELSE BEGIN
      help,!error_state,/st
      Catch, /Cancel
      message, 'err='+STRING(error)+", "+!error_state.msg, /INFO
...
; convert 3-digit DPR phase to corresponding GR HID rain and snow category

  idxmiss = WHERE(phase EQ 255, nidxmiss)
  idxsnow = WHERE(phase/100 EQ 0, nidxsnow)
  idxmix = WHERE(phase/100 EQ 1, nidxmix)
  idxrain = WHERE(phase/100 EQ 2, nidxrain)
  IF nidxsnow GT 0 THEN phase[idxsnow] = 4  ; assign as DS (Dry Snow?)
  IF nidxmix GT 0 THEN phase[idxmix] = 5    ; assign as Wet Snow
  IF nidxrain GT 0 THEN phase[idxrain] = 2  ; assign as Rain
  IF nidxmiss GT 0 THEN phase[idxmiss] = 0

  idxmiss = WHERE(phaseNearSurface EQ 255, nidxmiss)
  idxsnow = WHERE(phaseNearSurface/100 EQ 0, nidxsnow)
  idxmix = WHERE(phaseNearSurface/100 EQ 1, nidxmix)
  idxrain = WHERE(phaseNearSurface/100 EQ 2, nidxrain)
  IF nidxsnow GT 0 THEN phaseNearSurface[idxsnow] = 4  ; assign as DS (Dry Snow?)
  IF nidxmix GT 0 THEN phaseNearSurface[idxmix] = 5    ; assign as Wet Snow
  IF nidxrain GT 0 THEN phaseNearSurface[idxrain] = 2  ; assign as Rain
  IF nidxmiss GT 0 THEN phaseNearSurface[idxmiss] = 0
...
IF ( nswp GT 1 ) THEN BEGIN
  ...
  phaseApp = phase
  phaseHeightAGLApp = phaseHeightAGL
  phaseNearSurfaceApp = phaseNearSurface
   ...
   FOR iswp=1, nswp-1 DO BEGIN
      ...
      phase = [phaseApp, phase]
      phaseHeightAGL = [phaseHeightAGLApp, phaseHeightAGL]
      phaseNearSurface = [phaseNearSurfaceApp, phaseNearSurface]
   ENDFOR
ENDIF
...
ENDIF
  ...
  phase = phase[idxpractual2d]
  phaseHeightAGL = phaseHeightAGL[idxpractual2d]
  phaseNearSurface = phaseNearSurface[idxpractual2d]
...
ENDIF
  ...
  phase = REFORM( phase, countactual, nswp )
  phaseHeightAGL = REFORM( phaseHeightAGL, countactual, nswp )
  phaseNearSurface = REFORM( phaseNearSurface, countactual, nswp )
...
IF PTR_VALID(ptr_yCorner) THEN *ptr_yCorner = ycornew
     ...
     IF PTR_VALID(ptr_phase) THEN *ptr_phase = phase
     IF PTR_VALID(ptr_phaseHeightAGL) THEN *ptr_phaseHeightAGL = phaseHeightAGL
     IF PTR_VALID(ptr_phaseNearSurface) THEN *ptr_phaseNearSurface = phaseNearSurface

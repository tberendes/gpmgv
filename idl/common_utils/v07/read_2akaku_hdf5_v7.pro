...
; All Rights Reserved.
;
; read_2akaku_hdf5_v7.pro         Bob Morris, GPM GV/SAIC   June 2013
;
; DESCRIPTION
; -----------
...
; - Modified for GPM V7 format, removed TRMM 2APR for consistency since
;   NS scan is no longer available in 2AKu and 2AKa
;
; EMAIL QUESTIONS OR COMMENTS AT:
;       https://pmm.nasa.gov/contact
;-

FUNCTION read_2akaku_hdf5_v7, file, DEBUG=debug, READ_ALL=read_all, SCAN=scan2read
   ...
   CASE prodname OF
      '2AKa' : snames=['HS', 'FS']   ; prefixes used in 2A-Ka products
      '2AKu' : snames=['FS']         ; ditto, 2A-Ku DPR
      '2AKaX' : snames=['HS', 'FS']   ; prefixes used in 2A-Ka products
      '2AKuX' : snames=['FS']         ; ditto, 2A-Ku DPR
       ELSE  : BEGIN
                 h5g_close, group_id
                 h5f_close, file_id
                 message, "Illegal product type '" + prodname + $
                        "', must be '2AKa', '2AKaX', '2AKu', or '2AKuX'"
               END
   ENDCASE
   ...
   for isw = 0, filestruc.NUMBEROFSWATHS-1 do begin
      ...
      IF STRMATCH(onescan, sname) EQ 0b THEN BEGIN
         ...
         CASE sname OF
            ...
            'FS' : FS = { Swath : sname+": UNREAD" }  ;case should never happen!
         ENDCASE
         continue
      ENDIF
      ENDIF

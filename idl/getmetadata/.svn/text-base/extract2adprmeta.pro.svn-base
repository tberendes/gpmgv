...
; DESCRIPTION:
; ------------
; Reads rain type, bright band height and rain flag fields from NS swath
...
function extract2ADPRmeta, file_2aDPR, Instrument, dist, unlout, RainType=rainType
...
if ( count100 EQ 0 ) then begin
   print, "ERROR in extract2aDPRmeta(): can't find points <= 100km in dist array provided."
   status = 'extract2A23meta: ERROR IN DIST ARRAY'
   goto, errorExit
endif
;print, "In extract2ADPRmeta.pro: siteID, siteLong, siteLat, orbit = ", $
...
;
   ...
   CASE Instrument OF
     'DPR' : dpr_data = read_2adpr_hdf5(file_2adpr, SCAN=DPR_scantype)
      'Ku' : dpr_data = read_2akaku_hdf5(file_2adpr, SCAN=DPR_scantype)

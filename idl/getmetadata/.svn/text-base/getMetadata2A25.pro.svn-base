...
;
;-------------------------------------------------------------------------------
; getMetadata2A25.pro    Bob Morris, GPM GV (SAIC)    October 2006
;
; DESCRIPTION
;
...
; a run of this program.  See get2A23-25Meta.sh script for how the delimited
; file is created in SQL.
;
; For each file/orbit/site, the 2A25 HDF file is unzipped and extract2a25meta
...
; "extract2a25meta.pro" is a highly-modified version of Liang's access2A25.pro
;  routine.
; "coordinateBtoA.pro" is slightly modified from Liang's routine (using
;  modified commons).
; "read_2a25_ppi.pro"  is a highly-modified version of Liang's routine.
...
; Morris - Jul 8 2008  - Modified call to now-function access2a25(), and deal
;                        with returned status.
; Morris - Jul 10 2008 - Modified to call function extract2a25meta() in place
;                        of access2a25() to compute metadata within 100km.
; Morris - Apr 13 2012 - Cleaned up, edited comments.
...
; FILES
;
; tmpdirname/file2a23sites.YYMMDD.txt (INPUT) - lists 2A25 files to process,
...
;
; 2A25.[YY]YYMMDD.ORBIT#.V.sub-GPMGV1.hdf.gz (INPUT) - HDF data files to be
...
; OR
; 2A-CS-CONUS.TRMM.PR.2A25.YYYYMMDD-Shhmmss-Ehhmmss.ORBIT#.V.HDF - as above,
;    but with the GPM-era PPS filename convention, and preceded by the full
;    path to the file.
;
; tmpdirname/2A25_METADATA.YYMMDD.txt (OUTPUT) - delimited text file listing
...
metadatafile = tmpdirname+"/2A25_METADATA."+DATESTAMP+".unl"
GET_LUN, UNLUNIT
...
;  check whether origFile23Name is the full, existing pathname to the 2A23 file
   ...
   ;  add the well-known path to get the fully-qualified file name
      ...
      IF FILE_TEST(file23path, /DIRECTORY) EQ 0 THEN BEGIN
         print, "In getMetadata2A25, file 2A23 path not found: "+file23path
         goto, errorExit
      ENDIF
...
;  check whether origFile25Name is the full, existing pathname to the 2A25 file
   ...
   ENDIF ELSE BEGIN
      ...
      IF FILE_TEST(file25path, /DIRECTORY) EQ 0 THEN BEGIN
         print, "In getMetadata2A25, file 2A25 path not found: "+file25path
         goto, errorExit
      ENDIF
...
;     Get an uncompressed copy of the found file
      cpstatus = uncomp_file( found2a25, file_2do )
...
;        the overpass metadata
         ...
         status = extract2A25meta( file_2do, dist, unlunit )

         if ( status NE 'OK' ) then begin
           print, "In getmetadata2a25.pro, error in processing ", file_2do
         endif
